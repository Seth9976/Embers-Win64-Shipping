// 函数: sub_141e5b650
// 地址: 0x141e5b650
// 来自: E:\Embers\Embers\Binaries\Win64\Embers-Win64-Shipping.exe

int64_t r15
r15.b = 0
int32_t rax_1 = arg1[1].d - *(arg1 + 0x34)
int32_t rbp = 1
int64_t r11 = sx.q(arg3)
int32_t rdi_6

if (rax_1 == 1)
    rdi_6 = arg5
label_141e5b7f0:
    int32_t r8_4 = arg1[1].d - *(arg1 + 0x34)
    
    if (r8_4 u>= 4)
        uint32_t rax_16 = r8_4 u>> 1
        uint64_t rflags_1
        int32_t temp0_2
        temp0_2, rflags_1 = _bit_scan_reverse(rax_16 + 8)
        int32_t rcx_8
        
        if (rax_16 == 0xfffffff8)
            rcx_8 = 0x20
        else
            rcx_8 = 0x1f - temp0_2
        
        uint64_t rflags_2
        char temp0_3
        temp0_3, rflags_2 = _bit_scan_reverse(rax_16 + 7)
        char r9
        
        if (rax_16 == 0xfffffff9)
            r9 = 0x20
        else
            r9 = 0x1f - temp0_3
        
        rbp = 1 << ((not.d(rcx_8 << 0x1a s>> 0x1f)).b & (0x20 - r9))
    
    int32_t rax_19
    
    if (r8_4 s> 0)
        rax_19 = arg1[9].d
    
    if (r8_4 s> 0 && (rax_19 == 0 || rax_19 s< rbp))
        arg1[9].d = rbp
        sub_141e684e0(arg1)
    else
        void* r9_1 = &arg1[7]
        int32_t rax_22 = (arg1[9].d - 1) & r11.d
        *(arg4 + 0x8c) = rax_22
        void* rdx_10 = *(r9_1 + 8)
        int64_t r8_5 = sx.q(rax_22)
        void* rax_23 = r9_1
        
        if (rdx_10 != 0)
            rax_23 = rdx_10
        
        arg4[0x11].d = *(rax_23 + (((sx.q(arg1[9].d) - 1) & r8_5) << 2))
        void* rax_25 = *(r9_1 + 8)
        
        if (rax_25 != 0)
            r9_1 = rax_25
        
        *(r9_1 + (((sx.q(arg1[9].d) - 1) & r8_5) << 2)) = rdi_6
else
    int32_t r13_1 = -1
    int32_t rbx_1
    
    if (rax_1 != 0)
        void* r8 = &arg1[7]
        void* rcx = arg1[8]
        
        if (rcx != 0)
            r8 = rcx
        
        rbx_1 = *(r8 + (((sx.q(arg1[9].d) - 1) & r11) << 2))
    
    if (rax_1 == 0 || rbx_1 == 0xffffffff)
    label_141e5b719:
        rbx_1 = -1
    else
        int64_t rcx_1 = *arg1
        
        while (true)
            int64_t rdi_2 = sx.q(rbx_1) * 0x12
            int16_t* rdx_3
            
            if (arg4[1].d == 0)
                rdx_3 = &data_142d40450
            else
                rdx_3 = *arg4
            
            int16_t* const rcx_2
            
            if (*(rcx_1 + (rdi_2 << 3) + 8) == 0)
                rcx_2 = &data_142d40450
            else
                rcx_2 = *(rcx_1 + (rdi_2 << 3))
            
            if (sub_140a54510(rcx_2, rdx_3) == 0)
                break
            
            rcx_1 = *arg1
            rbx_1 = *(rcx_1 + (rdi_2 << 3) + 0x88)
            
            if (rbx_1 == 0xffffffff)
                goto label_141e5b719
    
    r15.b = rbx_1 != 0xffffffff
    
    if (rbx_1 == 0xffffffff)
        rdi_6 = arg5
    else
        int64_t* rdi_5 = sx.q(rbx_1) * 0x90 + *arg1
        sub_1405ae180(&rdi_5[3])
        int64_t rcx_4 = *rdi_5
        
        if (rcx_4 != 0)
            sub_140a74f90(rcx_4)
        
        memmove(rdi_5, arg4, 0x88)
        int64_t rcx_6 = sx.q(arg5)
        void* r10_1 = &arg1[2]
        
        if (*(arg1 + 0x34) != 0)
            *(*arg1 + sx.q(arg1[6].d) * 0x90) = rcx_6.d
        
        int32_t* rax_9 = rcx_6 * 0x90 + *arg1
        *rax_9 = 0xffffffff
        
        if (*(arg1 + 0x34) s> 0)
            r13_1 = arg1[6].d
        
        rax_9[1] = r13_1
        *(arg1 + 0x34) += 1
        rdi_6 = rbx_1
        arg1[6].d = rcx_6.d
        void* rax_10 = *(r10_1 + 0x10)
        
        if (rax_10 != 0)
            r10_1 = rax_10
        
        int32_t temp2_1
        int32_t temp3_1
        temp2_1:temp3_1 = sx.q(rcx_6.d)
        int64_t rcx_7 = sx.q((temp3_1 + (temp2_1 & 0x1f)) s>> 5)
        *(r10_1 + (rcx_7 << 2)) &= not.d(rol.d(1, rcx_6.b))
    
    if (rbx_1 == 0xffffffff)
        r11 = zx.q(arg3)
        goto label_141e5b7f0

*arg2 = rdi_6

if (arg6 != 0)
    *arg6 = r15.b

return arg2
