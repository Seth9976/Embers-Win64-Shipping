// 函数: sub_1416319a0
// 地址: 0x1416319a0
// 来自: E:\Embers\Embers\Binaries\Win64\Embers-Win64-Shipping.exe

void var_368
int64_t rax_1 = __security_cookie ^ &var_368
float zmm3[0x4] = *arg3
float zmm0[0x4] = arg3[1]
float temp0[0x4] = _mm_shuffle_ps(zmm3, zmm3, 0xff)
float var_258 = zmm3[0]
float var_308[0x4] = zmm0
int32_t var_190 = 0
int128_t var_12c_1
__builtin_memset(&var_12c_1, 0, 0x24)
int128_t var_fc_1
__builtin_memset(&var_fc_1, 0, 0x24)
zmm0 = data_142d3f7e0
int32_t var_18c
__builtin_memset(&var_18c, 0, 0x10)
uint64_t r12 = zx.q(var_190)
float var_178[0x4] = zmm0
float var_2b8[0x4]
int128_t* var_200 = &var_2b8
float* var_1f8 = &var_258
float temp0_1[0x4] = _mm_shuffle_ps(zmm3, zmm3, 0x55)
int128_t var_168
__builtin_memset(&var_168, 0, 0x24)
float var_254 = temp0_1[0]
float temp0_2[0x4] = _mm_shuffle_ps(zmm3, zmm3, 0xaa)
zmm3 ^= 0x80000000
float var_250 = temp0_2[0]
float temp0_3[0x4] = _mm_shuffle_ps(zmm3, zmm3, 0xe1)
temp0_3[0] = (temp0_1 ^ 0x80000000)[0]
float var_24c = temp0[0]
float temp0_4[0x4] = _mm_shuffle_ps(temp0_3, temp0_3, 0xc6)
temp0_4[0] = (temp0_2 ^ 0x80000000)[0]
void* var_220 = arg1
float temp0_5[0x4] = _mm_shuffle_ps(temp0_4, temp0_4, 0x27)
temp0_5[0] = temp0[0]
int64_t var_208 = arg2
float temp0_6[0x4] = _mm_shuffle_ps(temp0_5, temp0_5, 0x39)
var_2b8 = temp0_6
var_2b8 = temp0_6
zmm3 = *arg10
int128_t zmm11

if (zmm3[0] >= 0f)
    zmm11 = *(arg1 + 0x1c)
else
    zmm11 = *(arg1 + 0x10)

float zmm2[0x4] = (*arg10)[1]
int128_t zmm6

if (zmm2[0] >= 0f)
    zmm6 = *(arg1 + 0x20)
else
    zmm6 = *(arg1 + 0x14)

float zmm1[0x4] = (*arg10)[2]
int32_t var_274 = zmm6.d
int128_t zmm12

if (zmm1[0] >= 0f)
    zmm12 = *(arg1 + 0x24)
else
    zmm12 = *(arg1 + 0x18)

int32_t var_270 = zmm12.d
float var_320 = (zmm3 ^ 0x80000000)[0]
float var_31c = (zmm2 ^ 0x80000000)[0]
float var_318 = (zmm1 ^ 0x80000000)[0]
int64_t var_138 = zmm11.d.q
int32_t var_130 = var_270
void var_2d8
int32_t* rax_5
uint128_t zmm6_1
uint128_t zmm11_1
uint128_t zmm12_1
uint128_t zmm13
rax_5, zmm6_1, zmm11_1, zmm12_1, zmm13 = sub_14170d470(&var_208, &var_2d8, &var_320)
uint128_t zmm2_1 = var_308[1]
uint128_t zmm14 = zx.o(0)
uint128_t zmm5 = var_308[0]
uint128_t zmm3_1 = var_308[2]
uint128_t zmm8 = zmm2_1
uint128_t zmm1_1 = zx.o(*rax_5)
int32_t rcx_1 = rax_5[2]
zmm11_1.d = zmm11_1.d f- zmm1_1.d
int32_t var_2c8 = zmm2_1.d
int32_t var_2a4 = zmm2_1.d
uint32_t result = zx.d(arg9)
uint128_t zmm7 = zmm5
uint128_t zmm9 = zmm3_1
zmm6_1.d = zmm6_1.d f- _mm_shuffle_ps(zmm1_1, zmm1_1, 0x55).d
int64_t var_108 = zmm1_1.q
uint128_t zmm10
zmm10.d = zmm5.d f- zmm11_1.d
uint64_t var_268 = zmm1_1.q
zmm12_1.d = zmm12_1.d f- rcx_1
int32_t var_2cc = zmm5.d
uint128_t zmm15
zmm15.d = zmm2_1.d f- zmm6_1.d
int32_t var_2a8 = zmm5.d
int32_t var_2c4 = zmm3_1.d
uint32_t result_2 = zmm3_1.d
zmm6_1.d = zmm3_1.d f- zmm12_1.d
uint128_t zmm0_1
zmm0_1.d = zmm10.d f* zmm10.d
int32_t var_100 = rcx_1
zmm2_1.d = zmm15.d f* zmm15.d
int64_t var_298 = 0
int32_t var_290 = 0x3f800000
zmm1_1.d = zmm6_1.d f* zmm6_1.d
zmm2_1.d = zmm2_1.d f+ zmm0_1.d
zmm2_1.d = zmm2_1.d f+ zmm1_1.d
int32_t rdi

if (zmm2_1.d f>= 9.99999997e-07f || (result.b != 0 && zmm2_1.d f>= 9.99999997e-07f))
    rdi.b = 0
else
    rdi.b = 1

int32_t rcx_2 = 0
int64_t rsi
rsi.b = 0
int32_t var_2f8 = 0x7f7fffff
int32_t var_2dc = 0
char rbx = rdi.b
char var_328 = rbx
int32_t var_2ec
int64_t var_240
uint32_t result_1
uint128_t zmm4_1

if (rdi.b != 0)
label_14163272e:
    float* rdi_3 = arg6
    *rdi_3 = zmm14.d
    
    if (not(zmm14.d f<= zmm13.d))
        zmm3_1 = zmm13
        zmm4_1 = zmm13
        *arg8 = var_298
        arg8[1].d = var_290
        
        if (r12.d s> 0)
            float (* rcx_18)[0x4] = &var_178
            uint64_t i_2 = zx.q(r12.d)
            int32_t* rax_25 = &var_100
            uint64_t i
            
            do
                zmm2_1 = *rcx_18
                rcx_18 = &(*rcx_18)[1]
                zmm0_1.d = zmm2_1.d f* rax_25[-2]
                zmm1_1.d = zmm2_1.d f* rax_25[-1]
                zmm2_1.d = zmm2_1.d f* *rax_25
                rax_25 = &rax_25[3]
                zmm13.d = zmm13.d f+ zmm0_1.d
                zmm3_1.d = zmm3_1.d f+ zmm1_1.d
                zmm4_1.d = zmm4_1.d f+ zmm2_1.d
                i = i_2
                i_2 -= 1
            while (i != 1)
        
        zmm0_1.d = zmm14.d f* *arg4
        zmm1_1.d = zmm14.d f* *(arg4 + 4)
        zmm14.d = zmm14.d f* *(arg4 + 8)
        zmm0_1.d = zmm0_1.d f+ zmm5.d
        zmm1_1.d = zmm1_1.d f+ var_308[1]
        zmm14.d = zmm14.d f+ var_308[2]
        zmm0_1.d = zmm0_1.d f+ zmm13.d
        zmm1_1.d = zmm1_1.d f+ zmm3_1.d
        zmm14.d = zmm14.d f+ zmm4_1.d
        *arg7 = (_mm_unpacklo_ps(zmm0_1, zmm1_1.q)).q
        arg7[1].d = zmm14.d
    else if (result.b != 0)
        var_178[0].q = 0
        var_178[2].q = 0
        var_308[0].q = 0
        var_308[2].q = 0
        sub_14083ad30(&var_178, 8)
        
        if (var_308[3] s< 8)
            sub_14083ad30(&var_308, 8)
        
        float rax_33
        float zmm0_2[0x4]
        
        if (var_2dc == 0)
            int32_t var_2d0_3 = 0x3f800000
            rax_33 = 1f
            zmm0_2 = _mm_unpacklo_ps(zmm13, zmm13.q)
        label_1416329e0:
            zmm2_1 = var_138:4.d
            zmm1_1 = var_138.d
            *rdi_3 = 0f
            *arg8 = zmm0_2.q
            zmm0_2 = var_130
            arg8[1].d = rax_33
            zmm1_1 = _mm_unpacklo_ps(zmm1_1, zmm2_1.q)
            result = zmm0_2[0]
            *arg7 = zmm1_1.q
        else
            if (r12.d s> 0)
                int64_t rdi_4 = 0
                uint64_t i_3 = zx.q(r12.d)
                uint64_t i_1
                
                do
                    int64_t rbx_2 = sx.q(var_178[2])
                    int32_t rax_26 = (rbx_2 + 1).d
                    var_178[2] = rax_26
                    
                    if (rax_26 s> var_178[3])
                        sub_14083a7e0(&var_178)
                    
                    int64_t rcx_22 = var_178[0].q
                    int64_t rdx_10 = rbx_2 * 3
                    int32_t rax_27 = *(&var_130 + rdi_4)
                    zmm1_1.d = zmm8.d f+ *(&var_108:4 + rdi_4)
                    *(rcx_22 + (rdx_10 << 2)) = *(&var_138 + rdi_4)
                    zmm0_2 = zmm7
                    zmm0_2[0] = zmm0_2[0] f+ *(&var_108 + rdi_4)
                    *(rcx_22 + (rdx_10 << 2) + 8) = rax_27
                    int64_t rbx_3 = sx.q(var_308[2])
                    int32_t var_31c_2 = zmm1_1.d
                    var_320 = zmm0_2[0]
                    zmm0_2 = zmm9
                    zmm0_2[0] = zmm0_2[0] f+ *(&var_100 + rdi_4)
                    int32_t rax_28 = (rbx_3 + 1).d
                    var_308[2] = rax_28
                    var_318 = zmm0_2[0]
                    
                    if (rax_28 s> var_308[3])
                        sub_14083a7e0(&var_308)
                    
                    int64_t rcx_24 = var_308[0].q
                    int64_t rdx_12 = rbx_3 * 3
                    rdi_4 += 0xc
                    *(rcx_24 + (rdx_12 << 2)) = var_320.q
                    *(rcx_24 + (rdx_12 << 2) + 8) = var_318
                    i_1 = i_3
                    i_3 -= 1
                while (i_1 != 1)
                rdi_3 = arg6
            
            var_190.q = &var_2b8
            int64_t var_188_1 = arg2
            int128_t* var_180_1 = arg3
            int32_t rax_32 = sub_1415640e0(&var_178, &var_308, &var_220, &var_190, &var_2ec, 
                &var_320, &var_240, &var_208)
            zmm0_2 = zx.o(var_320.q)
            rax_33 = var_318
            
            if (rax_32 == 2)
                goto label_1416329e0
            
            *arg8 = zmm0_2.q
            arg8[1].d = rax_33
            result = result_1
            *rdi_3 = (var_2ec ^ data_142d3f780)[0]
            *arg7 = var_240
        int64_t rcx_26 = var_308[0].q
        arg7[1].d = result
        
        if (rcx_26 != 0)
            sub_140a74f90(rcx_26)
        
        int64_t rcx_27 = var_178[0].q
        
        if (rcx_27 != 0)
            sub_140a74f90(rcx_27)
    
    result.b = 1
else
    while (true)
        var_2dc = rcx_2 + 1
        
        if (rcx_2 s>= 0x20)
            result = zx.d(arg9)
        else
            void* rax_7 = var_220
            zmm0_1.d = zmm15.d f* zmm15.d
            zmm2_1.d = zmm10.d f* zmm10.d
            zmm1_1.d = zmm6_1.d f* zmm6_1.d
            zmm2_1.d = zmm2_1.d f+ zmm0_1.d
            zmm2_1.d = zmm2_1.d f+ zmm1_1.d
            zmm4_1 = _mm_rsqrt_ss(zmm2_1.d, zmm2_1.d)
            zmm3_1.d = zmm2_1.d f* 0.5f
            zmm0_1.d = zmm4_1.d f* zmm4_1.d
            zmm1_1.d = zmm3_1.d f* zmm0_1.d
            zmm2_1.d = 0.5f f- zmm1_1.d
            zmm0_1.d = zmm4_1.d f* zmm2_1.d
            zmm4_1.d = zmm4_1.d f+ zmm0_1.d
            zmm1_1.d = zmm4_1.d f* zmm4_1.d
            zmm3_1.d = zmm3_1.d f* zmm1_1.d
            zmm5.d = 0.5f f- zmm3_1.d
            zmm0_1.d = zmm4_1.d f* zmm5.d
            zmm4_1.d = zmm4_1.d f+ zmm0_1.d
            zmm6_1.d = zmm6_1.d f* zmm4_1.d
            zmm0_1.d = zmm4_1.d f* zmm10.d
            var_2ec = zmm4_1.d
            var_2ec = zmm6_1.d
            int32_t var_248_1 = zmm0_1.d
            zmm15.d = zmm15.d f* zmm4_1.d
            
            if (zmm0_1.d f>= zmm13.d)
                zmm1_1 = *(rax_7 + 0x1c)
            else
                zmm1_1 = *(rax_7 + 0x10)
            
            int32_t var_2f4_1 = zmm1_1.d
            
            if (zmm15.d f>= zmm13.d)
                zmm1_1 = *(rax_7 + 0x20)
            else
                zmm1_1 = *(rax_7 + 0x14)
            
            int32_t var_2f0_1 = zmm1_1.d
            
            if (zmm6_1.d f>= zmm13.d)
                zmm1_1 = *(rax_7 + 0x24)
            else
                zmm1_1 = *(rax_7 + 0x18)
            
            zmm10 = var_2b8[1]
            zmm11_1 = var_2b8[2]
            zmm7 = var_2b8[0]
            zmm2_1 = var_2b8[3]
            zmm5 = zmm0_1 ^ 0x80000000
            zmm12_1 = zmm6_1 ^ 0x80000000
            int32_t var_2c0_1 = zmm1_1.d
            int32_t var_270_1 = zmm1_1.d
            zmm3_1 = zmm10
            zmm6_1.d = zmm11_1.d f* zmm5.d
            zmm9.d = zmm10.d f* zmm12_1.d
            zmm8 = zmm15 ^ 0x80000000
            zmm0_1.d = zmm11_1.d f* zmm8.d
            zmm4_1.d = zmm7.d f* zmm8.d
            zmm9.d = zmm9.d f- zmm0_1.d
            zmm0_1.d = zmm7.d f* zmm12_1.d
            zmm6_1.d = zmm6_1.d f- zmm0_1.d
            zmm0_1.d = zmm10.d f* zmm5.d
            zmm9.d = zmm9.d f+ zmm9.d
            zmm4_1.d = zmm4_1.d f- zmm0_1.d
            zmm0_1 = zmm11_1
            zmm1_1.d = zmm2_1.d f* zmm9.d
            zmm6_1.d = zmm6_1.d f+ zmm6_1.d
            zmm11_1.d = zmm11_1.d f* zmm9.d
            zmm1_1.d = zmm1_1.d f+ zmm5.d
            zmm10.d = zmm10.d f* zmm9.d
            zmm0_1.d = zmm0_1.d f* zmm6_1.d
            zmm4_1.d = zmm4_1.d f+ zmm4_1.d
            zmm3_1.d = zmm3_1.d f* zmm4_1.d
            zmm3_1.d = zmm3_1.d f- zmm0_1.d
            zmm0_1.d = zmm7.d f* zmm4_1.d
            zmm7.d = zmm7.d f* zmm6_1.d
            zmm3_1.d = zmm3_1.d f+ zmm1_1.d
            zmm11_1.d = zmm11_1.d f- zmm0_1.d
            zmm1_1.d = zmm2_1.d f* zmm6_1.d
            zmm2_1.d = zmm2_1.d f* zmm4_1.d
            zmm7.d = zmm7.d f- zmm10.d
            zmm1_1.d = zmm1_1.d f+ zmm8.d
            zmm2_1.d = zmm2_1.d f+ zmm12_1.d
            zmm11_1.d = zmm11_1.d f+ zmm1_1.d
            zmm7.d = zmm7.d f+ zmm2_1.d
            uint64_t var_1f0 = (_mm_unpacklo_ps(zmm3_1, zmm11_1.q)).q
            int32_t var_1e8_1 = zmm7.d
            float var_218
            sub_141758e70(arg2, &var_218, &var_1f0, zmm13)
            int32_t var_210
            zmm10 = var_210
            zmm12_1 = var_250
            int32_t var_214
            zmm6_1 = var_214
            zmm8 = var_254
            zmm3_1 = var_218
            zmm9 = var_258
            zmm0_1.d = zmm6_1.d f* zmm12_1.d
            zmm7.d = zmm10.d f* zmm8.d
            zmm5.d = zmm3_1.d f* zmm12_1.d
            zmm7.d = zmm7.d f- zmm0_1.d
            zmm0_1.d = zmm10.d f* zmm9.d
            zmm5.d = zmm5.d f- zmm0_1.d
            zmm2_1 = var_24c
            zmm5.d = zmm5.d f+ zmm5.d
            zmm7.d = zmm7.d f+ zmm7.d
            zmm0_1.d = zmm3_1.d f* zmm8.d
            zmm11_1 = zmm8
            (&var_18c)[sx.q(r12.d)] = r12.d
            r12 = sx.q(var_190)
            zmm4_1.d = zmm6_1.d f* zmm9.d
            zmm1_1.d = zmm2_1.d f* zmm7.d
            uint64_t rcx_5 = r12 * 3
            zmm4_1.d = zmm4_1.d f- zmm0_1.d
            zmm8.d = zmm8.d f* zmm7.d
            zmm1_1.d = zmm1_1.d f+ zmm3_1.d
            zmm0_1 = zmm12_1
            zmm12_1.d = zmm12_1.d f* zmm7.d
            zmm7 = var_2cc
            zmm4_1.d = zmm4_1.d f+ zmm4_1.d
            zmm0_1.d = zmm0_1.d f* zmm5.d
            zmm11_1.d = zmm11_1.d f* zmm4_1.d
            zmm11_1.d = zmm11_1.d f- zmm0_1.d
            zmm0_1.d = zmm9.d f* zmm4_1.d
            zmm9.d = zmm9.d f* zmm5.d
            zmm11_1.d = zmm11_1.d f+ zmm1_1.d
            zmm1_1.d = zmm2_1.d f* zmm5.d
            zmm12_1.d = zmm12_1.d f- zmm0_1.d
            zmm0_1 = var_2f4_1
            zmm9.d = zmm9.d f- zmm8.d
            zmm8 = var_2c8
            zmm2_1.d = zmm2_1.d f* zmm4_1.d
            zmm1_1.d = zmm1_1.d f+ zmm6_1.d
            zmm6_1 = var_248_1
            zmm2_1.d = zmm2_1.d f+ zmm10.d
            zmm12_1.d = zmm12_1.d f+ zmm1_1.d
            zmm1_1 = var_2f0_1
            zmm9.d = zmm9.d f+ zmm2_1.d
            zmm2_1.d = zmm0_1.d f- zmm11_1.d
            *(&var_138 + (rcx_5 << 2)) = (_mm_unpacklo_ps(zmm0_1, zmm1_1.q)).q
            zmm3_1.d = zmm1_1.d f- zmm12_1.d
            (&var_130)[rcx_5] = var_270_1
            *(&var_108 + (rcx_5 << 2)) = (_mm_unpacklo_ps(zmm11_1, zmm12_1.q)).q
            zmm10.d = var_2c0_1.d f- zmm9.d
            zmm4_1.d = zmm7.d f- zmm2_1.d
            int32_t var_2f4_2 = zmm2_1.d
            uint32_t var_1d0_1 = zmm9.d
            zmm9 = var_2c4
            zmm2_1.d = zmm8.d f- zmm3_1.d
            int32_t var_2f0_2 = zmm3_1.d
            zmm3_1.d = zmm9.d f- zmm10.d
            zmm0_1.d = zmm6_1.d f* zmm4_1.d
            (&var_100)[rcx_5] = var_1d0_1
            zmm5.d = zmm15.d f* zmm2_1.d
            var_320 = zmm4_1.d
            zmm4_1 = var_2ec
            int32_t var_31c_1 = zmm2_1.d
            zmm5.d = zmm5.d f+ zmm0_1.d
            zmm1_1.d = zmm4_1.d f* zmm3_1.d
            var_318 = zmm3_1.d
            zmm5.d = zmm5.d f+ zmm1_1.d
            int128_t var_158_1
            int32_t var_148_1
            
            if (zmm5.d f<= zmm13.d)
                r12 = zx.q(r12.d + 1)
                *(&var_168 + (rcx_5 << 2)) = var_320.q
                *(&var_168:8 + (rcx_5 << 2)) = var_318
                var_190 = r12.d
            label_141632348:
                zmm11_1 = var_2f8
                
                if (rbx == 0)
                    zmm12_1 = 0x358637bd
                else
                    zmm12_1 = 0x358637bd
                    
                    if (not(zmm5.d f< zmm13.d))
                        zmm5.d = zmm5.d f* zmm5.d
                        zmm5.d = zmm5.d f+ 9.99999997e-07f
                        rdi.b = zmm5.d f>= zmm11_1.d
            else
                zmm3_1 = *(arg4 + 4)
                zmm11_1 = *arg4
                zmm12_1 = *(arg4 + 8)
                zmm2_1.d = zmm6_1.d f* zmm11_1.d
                zmm0_1.d = zmm15.d f* zmm3_1.d
                zmm1_1.d = zmm4_1.d f* zmm12_1.d
                zmm2_1.d = zmm2_1.d f+ zmm0_1.d
                zmm2_1.d = zmm2_1.d f+ zmm1_1.d
                
                if (zmm2_1.d f>= zmm13.d)
                    result.b = 0
                    break
                
                zmm1_1 = zmm14
                zmm0_1.d = zmm5.d f/ zmm2_1.d
                zmm14.d = zmm14.d f- zmm0_1.d
                
                if (zmm14.d f<= zmm1_1.d)
                    goto label_141632348
                
                if (zmm14.d f> arg5)
                    result.b = 0
                    break
                
                zmm1_1 = zx.o(var_2a8.q)
                zmm0_1 = _mm_shuffle_ps(zmm1_1, zmm1_1, 0x55)
                var_240 = zmm1_1.q
                zmm8.d = zmm14.d f* zmm3_1.d
                r12 = zx.q(r12.d + 1)
                result_1 = result_2
                zmm7.d = zmm14.d f* zmm11_1.d
                var_190 = r12.d
                zmm8.d = zmm8.d f+ var_308[1]
                zmm11_1 = 0x7f7fffff
                zmm7.d = zmm7.d f+ var_308[0]
                zmm9.d = zmm14.d f* zmm12_1.d
                var_328 = 0
                var_290 = zmm4_1.d
                zmm4_1.d = zmm8.d f- zmm0_1.d
                var_2c8 = zmm8.d
                zmm9.d = zmm9.d f+ var_308[2]
                zmm3_1.d = zmm7.d f- zmm1_1.d
                var_2cc = zmm7.d
                zmm1_1.d = var_168:4.d.d f+ zmm4_1.d
                var_2a8 = zmm7.d
                int32_t var_2a4_1 = zmm8.d
                var_2c4 = zmm9.d
                zmm2_1.d = zmm9.d f- result_1
                zmm0_1.d = var_168.d.d f+ zmm3_1.d
                result_2 = zmm9.d
                var_168:4.d = zmm1_1.d
                zmm1_1.d = var_168:0xc.d.d f+ zmm3_1.d
                var_298.d = zmm6_1.d
                var_298:4.d = zmm15.d
                var_168.d = zmm0_1.d
                zmm0_1.d = var_168:8.d.d f+ zmm2_1.d
                var_2f8 = 0x7f7fffff
                var_168:0xc.d = zmm1_1.d
                zmm1_1.d = var_158_1:4.d.d f+ zmm2_1.d
                var_168:8.d = zmm0_1.d
                zmm0_1.d = var_158_1.d.d f+ zmm4_1.d
                var_158_1:4.d = zmm1_1.d
                zmm1_1.d = var_158_1:0xc.d.d f+ zmm4_1.d
                var_158_1.d = zmm0_1.d
                zmm0_1.d = var_158_1:8.d.d f+ zmm3_1.d
                var_158_1:0xc.d = zmm1_1.d
                zmm1_1.d = zmm7.d f- var_2f4_2
                var_158_1:8.d = zmm0_1.d
                zmm0_1.d = var_148_1.d f+ zmm2_1.d
                zmm2_1.d = zmm8.d f- var_2f0_2
                var_148_1 = zmm0_1.d
                zmm0_1.d = zmm9.d f- zmm10.d
                *(&var_168 + (rcx_5 << 2)) = (_mm_unpacklo_ps(zmm1_1, zmm2_1.q)).q
                *(&var_168:8 + (rcx_5 << 2)) = zmm0_1.d
                zmm12_1 = 0x358637bd
            
            if (rdi.b != 0)
                int64_t* rax_22
                rax_22, zmm7, zmm8, zmm9, zmm13, zmm14 =
                    sub_1416f4530(&var_208, &var_168, &var_190, &var_178, &var_138, &var_108)
                r12 = zx.q(var_190)
                zmm1_1 = zx.o(*rax_22)
                int32_t rax_23 = rax_22[1].d
                zmm15 = _mm_shuffle_ps(zmm1_1, zmm1_1, 0x55)
                zmm6_1 = rax_23
                zmm10 = zmm1_1.d
            else
                int32_t rcx_6 = r12.d
                int32_t rax_13
                
                if (rcx_6 == 1)
                    int64_t rax_18 = sx.q(var_18c)
                    int64_t rcx_13 = rax_18 * 3
                    var_178[rax_18] = 0x3f800000
                    zmm0_1 = zx.o(*(&var_168 + (rcx_13 << 2)))
                    rax_13 = *(&var_168:8 + (rcx_13 << 2))
                else if (rcx_6 == 2)
                    uint64_t var_1b4
                    uint64_t* rax_17
                    rax_17, zmm7, zmm8, zmm9 =
                        sub_1416e69d0(&var_1b4, &var_168, &var_18c, &var_190, &var_178)
                    r12 = zx.q(var_190)
                    zmm0_1 = zx.o(*rax_17)
                    rax_13 = rax_17[1].d
                else if (rcx_6 == 3)
                    uint64_t var_1a8
                    uint64_t* rax_16
                    rax_16, zmm7, zmm8, zmm9, zmm11_1, zmm12_1, zmm13, zmm14 =
                        sub_1416fa750(&var_1a8, &var_168, &var_190, &var_178)
                    r12 = zx.q(var_190)
                    zmm0_1 = zx.o(*rax_16)
                    rax_13 = rax_16[1].d
                else if (rcx_6 == 4)
                    uint64_t var_19c
                    uint64_t* rax_15
                    rax_15, zmm7, zmm8, zmm9, zmm11_1, zmm12_1, zmm13, zmm14 =
                        sub_1416f9c90(&var_19c, &var_168, &var_190, &var_178)
                    r12 = zx.q(var_190)
                    zmm0_1 = zx.o(*rax_15)
                    rax_13 = rax_15[1].d
                else
                    int32_t var_2d0_1 = 0
                    rax_13 = 0
                    zmm0_1 = _mm_unpacklo_ps(zmm13, zmm13.q)
                
                int32_t var_180
                int64_t rdi_1 = sx.q(var_180)
                int64_t r11_1 = sx.q(var_18c)
                int64_t var_188
                int64_t r10_1 = sx.q(var_188.d)
                int64_t r9_4 = sx.q(var_188:4.d)
                int64_t rbx_1 = rdi_1 * 3
                uint64_t var_2e8_1 = zmm0_1.q
                int64_t r15_1 = r11_1 * 3
                int64_t r14_1 = r10_1 * 3
                rsi = r9_4 * 3
                int32_t rdx_5 = *(&var_168:8 + (r14_1 << 2))
                int32_t rcx_14 = *(&var_168:8 + (rsi << 2))
                int32_t r8_5 = *(&var_168:8 + (r15_1 << 2))
                zmm10 = var_2e8_1.d
                zmm15 = var_2e8_1:4.d
                int32_t rax_19 = *(&var_168:8 + (rbx_1 << 2))
                zmm6_1 = rax_13
                var_168:0xc.q = *(&var_168 + (r14_1 << 2))
                var_148_1:4.q = *(&var_168 + (rbx_1 << 2))
                zmm0_1 = var_178[r11_1]
                var_158_1:8.q = *(&var_168 + (rsi << 2))
                zmm1_1 = var_178[r10_1]
                var_168.q = *(&var_168 + (r15_1 << 2))
                zmm3_1 = var_178[rdi_1]
                int32_t rdi_2 = (&var_130)[r15_1]
                var_178[2] = var_178[r9_4].d
                zmm2_1 = zx.o(*(&var_138 + (r14_1 << 2)))
                var_178[0] = zmm0_1.d
                zmm0_1 = zx.o(*(&var_138 + (rbx_1 << 2)))
                var_178[1] = zmm1_1.d
                zmm1_1 = zx.o(*(&var_138 + (rsi << 2)))
                var_178[3] = zmm3_1.d
                zmm3_1 = zx.o(*(&var_138 + (r15_1 << 2)))
                var_158_1:4.d = rdx_5
                int32_t rdx_6 = (&var_130)[r14_1]
                var_148_1 = rcx_14
                int32_t rcx_15 = (&var_130)[rsi]
                var_148_1 = rax_19
                int32_t rax_20 = (&var_130)[rbx_1]
                var_12c_1.q = zmm2_1.q
                zmm2_1 = zx.o(*(&var_108 + (r14_1 << 2)))
                int128_t var_11c_1
                var_11c_1:8.q = zmm0_1.q
                zmm0_1 = zx.o(*(&var_108 + (rbx_1 << 2)))
                var_12c_1:0xc.q = zmm1_1.q
                zmm1_1 = zx.o(*(&var_108 + (rsi << 2)))
                var_130 = rdi_2
                rdi = (&var_100)[r15_1]
                var_138 = zmm3_1.q
                zmm3_1 = zx.o(*(&var_108 + (r15_1 << 2)))
                var_12c_1:8.d = rdx_6
                int32_t rdx_7 = (&var_100)[r14_1]
                var_11c_1:4.d = rcx_15
                int32_t rcx_16 = (&var_100)[rsi]
                int32_t var_10c_1 = rax_20
                int32_t rax_21 = (&var_100)[rbx_1]
                var_fc_1.q = zmm2_1.q
                int128_t var_ec_1
                var_ec_1:8.q = zmm0_1.q
                var_18c.o = data_142e11d00
                var_100 = rdi
                zmm2_1.d = zmm15.d f* zmm15.d
                var_168:8.d = r8_5
                var_fc_1:0xc.q = zmm1_1.q
                zmm0_1.d = zmm10.d f* zmm10.d
                var_fc_1:8.d = rdx_7
                zmm1_1.d = zmm6_1.d f* zmm6_1.d
                var_ec_1:4.d = rcx_16
                zmm2_1.d = zmm2_1.d f+ zmm0_1.d
                var_108 = zmm3_1.q
                int32_t var_dc_1 = rax_21
                zmm2_1.d = zmm2_1.d f+ zmm1_1.d
                zmm2_1.d f- zmm12_1.d
                rdi.b = zmm2_1.d f< zmm12_1.d
                var_2f8 = zmm2_1.d
                rsi.b = zmm2_1.d f>= zmm11_1.d
            
            result = zx.d(arg9)
            
            if (rdi.b == 0 && rsi.b == 0)
                rcx_2 = var_2dc
                rbx = var_328
                continue
        
        zmm5 = var_308[0]
        goto label_14163272e
__security_check_cookie(rax_1 ^ &var_368)
return result
