// 函数: __sse2_sin2@@16
// 地址: 0x142c95780
// 来自: E:\Embers\Embers\Binaries\Win64\Embers-Win64-Shipping.exe

uint32_t r9 = 0
double zmm15[0x2]
double var_b8[0x2] = zmm15
int32_t zmm14[0x4]
int32_t var_38[0x4] = zmm14
int32_t zmm13[0x4]
int32_t var_28[0x4] = zmm13
uint128_t zmm12
uint128_t var_a8 = zmm12
uint32_t zmm11[0x4]
uint32_t var_98[0x4] = zmm11
uint32_t zmm10[0x4] = arg1
uint32_t zmm2[0x4] = data_144293a80
uint32_t temp0[0x4] = _mm_andnot_ps(zmm2, zmm10)
double zmm7[0x2] = data_144293b00
uint32_t temp0_1[0x4] = _mm_and_ps(zmm2, zmm10)
double temp0_2[0x2] = _mm_mul_pd(zmm7, temp0_1)
double zmm3[0x2] = data_144293b40
double temp0_3[0x2] = _mm_add_pd(temp0_2, zmm3)
zmm7 = _mm_slli_epi64(temp0_3, 0x3f)
double temp0_5[0x2] = _mm_sub_pd(temp0_3, zmm3)
double temp0_6[0x2] = _mm_mul_pd(data_144293c00, temp0_5)
double temp0_7[0x2] = _mm_mul_pd(data_144293c40, temp0_5)
arg1 = _mm_sub_pd(temp0_1, temp0_6)
double temp0_9[0x2] = _mm_mul_pd(data_144293c80, temp0_5)
arg1 = _mm_sub_pd(arg1, temp0_7)
double temp0_11[0x2] = _mm_mul_pd(data_144293cc0, temp0_5)
arg1 = _mm_sub_pd(_mm_sub_pd(arg1, temp0_9), temp0_11)
uint32_t zmm9[0x4] = _mm_mul_pd(arg1, arg1)
double temp0_15[0x2] = __cmppd_xmmpd_mempd_immb(temp0_1, data_144293ac0, 6)
arg1 ^= zmm7
double temp0_16[0x2] = _mm_mul_pd(data_144293f40, zmm9)
uint64_t result = zx.q(_mm_movemask_pd(temp0_15))
double temp0_30[0x2] = _mm_mul_pd(
    _mm_mul_pd(zmm9, 
        __addpd_xmmpd_mempd(
            _mm_mul_pd(
                __addpd_xmmpd_mempd(
                    _mm_mul_pd(
                        __addpd_xmmpd_mempd(
                            _mm_mul_pd(
                                __addpd_xmmpd_mempd(
                                    _mm_mul_pd(
                                        __addpd_xmmpd_mempd(
                                            _mm_mul_pd(
                                                __addpd_xmmpd_mempd(temp0_16, data_144293f00), 
                                                zmm9), 
                                            data_144293ec0), 
                                        zmm9), 
                                    data_144293e80), 
                                zmm9), 
                            data_144293e40), 
                        zmm9), 
                    data_144293e00), 
                zmm9), 
            data_144293dc0)), 
    arg1)
uint32_t var_140[0x4]
uint32_t (* var_10)[0x4] = &var_140
arg1 = _mm_add_pd(arg1, temp0_30) ^ temp0
double var_178[0x2]

if (result.d != 0)
    zmm7 = _mm_srli_epi64(data_144280200 & temp0_1, 0x34)
    uint64_t r10_1 = zx.q(zmm7[0].d)
    double zmm5[0x2] = data_144293780
    uint32_t var_168_1[0x4] = arg1
    uint64_t rax = zx.q(_mm_extract_epi16(zmm7, 4))
    var_178 = temp0_15
    uint32_t temp0_35[0x4] = _mm_and_ps(zmm5, temp0_1)
    uint64_t r8_2 = zx.q((r10_1 * 3).d << 3)
    uint32_t var_158_1[0x4] = zmm10
    double zmm6[0x2] = zx.o(*(&data_144283500 + r8_2))
    zmm10 = zx.o(*(r8_2 + 0x144283508))
    zmm12 = zx.o(*(r8_2 + 0x144283510))
    zmm3 = __paddq_xmmdq_memdq(data_144280210 & temp0_1, data_144280220)
    arg1 = _mm_cmpeq_pd(temp0_35, zmm5, 0)
    uint64_t rcx_3 = zx.q((rax * 3).d << 3)
    zmm12 = zmm12.q | *(rcx_3 + 0x144283510) << 0x40
    double zmm4[0x2] = _mm_srli_epi64(zmm3, 0x20)
    zmm13 = data_144280230
    r9 = _mm_movemask_pd(arg1)
    zmm5 = _mm_srli_epi64(zmm12, 0x20)
    zmm3 &= zmm13
    zmm10 = zmm10[0].q | *(rcx_3 + 0x144283508) << 0x40
    arg1 = _mm_mul_epu32(zmm4, zmm5)
    zmm11 = zmm10
    zmm5 = _mm_mul_epu32(zmm5, zmm3)
    zmm10 &= zmm13
    double zmm8[0x2] = _mm_mul_epu32(zmm3, zmm10)
    zmm5 = _mm_srli_epi64(zmm5, 0x20)
    zmm12 = _mm_mul_epu32(zmm12 & zmm13, zmm4)
    zmm10 = _mm_mul_epu32(zmm10, zmm4)
    arg1 = _mm_add_epi64(arg1, zmm5)
    zmm11 = _mm_srli_epi64(zmm11, 0x20)
    zmm9 = _mm_mul_epu32(zmm3, zmm11)
    zmm11 = _mm_mul_epu32(zmm11, zmm4)
    zmm7 = _mm_add_epi64(zmm13 & zmm8, arg1)
    zmm6 = zmm6[0] | *(&data_144283500 + rcx_3) << 0x40
    zmm12 = _mm_srli_epi64(zmm12, 0x20)
    zmm8 = _mm_srli_epi64(zmm8, 0x20)
    zmm14 = zmm13 & zmm6
    zmm12 = _mm_add_epi64(zmm12, zmm7)
    zmm10 = _mm_add_epi64(zmm10, zmm8)
    zmm15 = _mm_mul_epu32(zmm3, zmm14)
    zmm4 = _mm_mul_epu32(zmm4, zmm14)
    zmm5 = _mm_add_epi64(zmm13 & zmm9, zmm10)
    arg1 = _mm_srli_epi64(zmm12, 0x20)
    zmm9 = _mm_srli_epi64(zmm9, 0x20)
    zmm6 = _mm_srli_epi64(zmm6, 0x20)
    arg1 = _mm_add_epi64(arg1, zmm5)
    zmm11 = _mm_add_epi64(zmm11, zmm9)
    zmm3 = _mm_mul_epu32(zmm3, zmm6)
    zmm8 = _mm_add_epi64(zmm13 & zmm15, zmm11)
    zmm10 = _mm_srli_epi64(arg1, 0x20)
    zmm15 = _mm_srli_epi64(zmm15, 0x20)
    zmm10 = _mm_add_epi64(zmm10, zmm8)
    zmm3 = _mm_add_epi64(zmm3 & zmm13, _mm_add_epi64(zmm4, zmm15))
    zmm7 = _mm_slli_epi64(_mm_add_epi64(_mm_srli_epi64(zmm10, 0x20), zmm3), 0x20)
    arg1 = _mm_slli_epi64(arg1, 0x20)
    zmm7 = _mm_add_epi64(zmm7, zmm10 & zmm13)
    zmm14 = data_144280240 & temp0_1
    zmm11 = data_144280260
    zmm4 = _mm_srli_epi64(zmm7, 0xc) | (data_144280250 ^ zmm14)
    double temp0_77[0x2] = _mm_add_pd(zmm11, zmm4)
    arg1 = _mm_add_epi64(arg1, zmm12 & zmm13)
    zmm3 = data_1442802b0 & zmm7
    zmm13 = data_144280280
    zmm5 = data_144280290 & arg1
    zmm12 = _mm_sub_pd(temp0_77, zmm11)
    zmm13 ^= zmm14
    zmm3 = _mm_slli_epi64(zmm3, 0x28) | _mm_srli_epi64(arg1, 0x18)
    zmm14 ^= data_1442802a0
    zmm5 = _mm_slli_epi64(zmm5, 0x1c)
    double temp0_83[0x2] = _mm_sub_pd(zmm4, zmm12)
    double temp0_84[0x2] = _mm_sub_pd(zmm3 | zmm14, zmm14)
    arg1 = _mm_add_pd(temp0_83, temp0_84)
    double temp0_86[0x2] = _mm_sub_pd(temp0_83, arg1)
    double temp0_87[0x2] = _mm_sub_pd(zmm5 | zmm13, zmm13)
    double temp0_88[0x2] = _mm_add_pd(temp0_84, temp0_86)
    zmm7 = _mm_and_ps(data_1442802f0, arg1)
    double temp0_90[0x2] = _mm_add_pd(temp0_87, temp0_88)
    arg1 = _mm_sub_pd(arg1, zmm7)
    double temp0_92[0x2] = __mulpd_xmmpd_mempd(temp0_90, data_1442802c0)
    zmm15 = data_1442802d0
    zmm4 = data_1442802e0
    double temp0_93[0x2] = _mm_mul_pd(zmm15, zmm7)
    double temp0_94[0x2] = _mm_mul_pd(zmm15, arg1)
    double temp0_95[0x2] = _mm_mul_pd(zmm7, zmm4)
    double temp0_96[0x2] = _mm_mul_pd(zmm4, arg1)
    double temp0_99[0x2] =
        _mm_add_pd(_mm_add_pd(temp0_94, temp0_95), _mm_add_pd(temp0_92, temp0_96))
    zmm6 = __andps_xmmxud_memxud(temp0_77, data_144280270)
    zmm5 = data_144280310
    int32_t rdx_2 = zmm6[0].d
    zmm3 = _mm_and_ps(data_144280300, temp0_1)
    zmm13 = data_144293980
    zmm9 = _mm_add_pd(temp0_99, temp0_93)
    double temp0_103[0x2] = _mm_cmpeq_pd(zmm5, zmm3, 1)
    double temp0_104[0x2] = _mm_cmpeq_pd(zmm3, zmm5, 2)
    double temp0_105[0x2] = _mm_sub_pd(temp0_93, zmm9)
    uint64_t rdx_3 = zx.q(rdx_2 << 5)
    zmm3 = _mm_and_ps(temp0_104, temp0_1)
    zmm5 = zx.o(*(&data_14428f700 + rdx_3))
    uint32_t temp0_107[0x4] = _mm_and_ps(zmm9, temp0_103)
    uint32_t temp0_108 = _mm_extract_epi16(zmm6, 4)
    double temp0_109[0x2] = _mm_add_pd(temp0_99, temp0_105)
    result = zx.q(temp0_108 << 5)
    zmm3 = _mm_or_ps(zmm3, temp0_107)
    zmm12 = *(rdx_3 + 0x14428f708) | *(result + 0x14428f708) << 0x40
    double temp0_111[0x2] = _mm_mul_pd(zmm3, zmm12)
    arg1 = zx.o(*(rdx_3 + 0x14428f710))
    zmm4 = _mm_and_ps(temp0_103, temp0_109)
    arg1 = arg1[0].q | *(result + 0x14428f710) << 0x40
    zmm10 = _mm_add_pd(arg1, temp0_111)
    double temp0_115[0x2] = _mm_add_pd(temp0_111, _mm_sub_pd(arg1, zmm10))
    double temp0_116[0x2] = _mm_mul_pd(zmm3, zmm3)
    zmm5 = zmm5[0] | *(&data_14428f700 + result) << 0x40
    zmm11 = _mm_mul_pd(zmm3, zmm5)
    double temp0_118[0x2] = _mm_add_pd(zmm5, zmm12)
    uint32_t zmm1[0x4] = _mm_add_pd(zmm10, zmm11)
    zmm13 = _mm_mul_pd(zmm13, temp0_116)
    zmm10 = _mm_sub_pd(zmm10, zmm1)
    zmm13 = __addpd_xmmpd_mempd(zmm13, *"HDUUUU")
    zmm11 = _mm_add_pd(zmm11, zmm10)
    zmm13 = _mm_mul_pd(zmm13, temp0_116)
    double temp0_125[0x2] = _mm_add_pd(temp0_115, zmm11)
    zmm13 = _mm_mul_pd(zmm13, zmm3)
    double temp0_127[0x2] = _mm_mul_pd(zmm3, arg1)
    zmm13 = _mm_mul_pd(zmm13, temp0_118)
    double temp0_129[0x2] = _mm_sub_pd(temp0_118, temp0_127)
    double temp0_130[0x2] = _mm_add_pd(temp0_125, zmm13)
    double temp0_131[0x2] = _mm_mul_pd(zmm4, temp0_129)
    zmm10 = var_158_1
    zmm2 = var_178
    uint32_t temp0_142[0x4] = _mm_and_ps(
        _mm_add_pd(zmm1, 
            _mm_add_pd(
                _mm_add_pd(temp0_130, 
                    _mm_mul_pd(
                        _mm_mul_pd(temp0_116, 
                            __addpd_xmmpd_mempd(
                                _mm_mul_pd(
                                    __addpd_xmmpd_mempd(_mm_mul_pd(data_144293a40, temp0_116), 
                                        data_144293a00), 
                                    temp0_116), 
                                data_1442939c0)), 
                        arg1)), 
                _mm_add_pd(zx.o(*(rdx_3 + 0x14428f718))[0] | *(result + 0x14428f718) << 0x40, 
                temp0_131))) ^ temp0, 
        zmm2)
    arg1 = _mm_or_ps(__andnps_xmmxud_memxud(zmm2, var_168_1), temp0_142)

if (r9 != 0)
    var_140 = zmm10
    uint32_t var_100_1[0x4] = arg1
    
    if (r9 != 0)
        result = 0
        var_178[1] = arg2
        uint64_t rbx = 0
        var_178[0] = arg3
        
        do
            if (test_bit(r9, rbx.d))
                result = __common_dcos_cout_rare(&var_140[rbx * 2], &var_140[rbx * 2 + 0x10])
            
            rbx = zx.q(rbx.d + 1)
        while (rbx.d s< 2)
        
        var_178[1]
        var_178[0]

return result
