// 函数: sub_1423e68b0
// 地址: 0x1423e68b0
// 来自: E:\Embers\Embers\Binaries\Win64\Embers-Win64-Shipping.exe

int64_t* rbx = *(arg1 + 0xc38)
uint64_t r13 = arg2
void* rdi = &rbx[sx.q(*(arg1 + 0xc40))]
int64_t result

if (rbx == rdi)
label_1423e6ae9:
    result.b = 0
else
    while (true)
        void* rcx = *(*rbx + 0x278)
        
        if (rcx != 0)
            arg2.b = 1
            result = sub_142434f20(rcx, arg2.b)
            
            if (result != 0)
                int32_t rcx_1 = 0
                int32_t var_94_1 = 1
                void* r15_1 = result + 0x28
                int32_t var_98_1 = 0
                int32_t r10_1 = *(r15_1 + 0x28)
                int32_t r8_1 = 0
                void* var_90_1 = r15_1 + 0x10
                int32_t var_88_1 = 0xffffffff
                int64_t var_84_1 = 0
                
                if (r10_1 != 0)
                    void* rax = *(r15_1 + 0x20)
                    void* r9_1 = r15_1 + 0x10
                    
                    if (rax != 0)
                        r9_1 = rax
                    
                    int32_t temp0_1
                    int32_t temp1_1
                    temp0_1:temp1_1 = sx.q(r10_1 - 1)
                    int32_t rdx_2 = *r9_1
                    
                    if (rdx_2 != 0)
                    label_1423e6998:
                        int32_t rax_7 = neg.d(rdx_2) & rdx_2
                        uint64_t rflags_1
                        int32_t temp0_2
                        temp0_2, rflags_1 = _bit_scan_reverse(rax_7)
                        int32_t var_94_2 = rax_7
                        int32_t rax_8
                        
                        if (rax_7 == 0)
                            rax_8 = 0x20
                        else
                            rax_8 = 0x1f - temp0_2
                        
                        var_84_1.d = r8_1 - rax_8 + 0x1f
                        
                        if (r8_1 - rax_8 + 0x1f s> r10_1)
                            var_84_1.d = r10_1
                    else
                        while (true)
                            int64_t rdx_3 = sx.q(rcx_1)
                            r8_1 += 0x20
                            rcx_1 += 1
                            var_84_1:4.d = r8_1
                            var_98_1 = rcx_1
                            
                            if (rdx_3.d s>= (temp1_1 + (temp0_1 & 0x1f)) s>> 5)
                                break
                            
                            rdx_2 = *(r9_1 + (rdx_3 << 2) + 4)
                            int32_t var_88_2 = 0xffffffff
                            
                            if (rdx_2 != 0)
                                goto label_1423e6998
                        
                        var_84_1.d = r10_1
                
                arg2 = zx.q(*(r15_1 + 0x28))
                int128_t var_38_1 = 0xffffffff
                int32_t r12_1 = 0xffffffff << (arg2.b & 0x1f)
                int128_t var_48_1 = var_98_1.o
                int32_t r8_4 = arg2.d s>> 5
                int32_t r9_3 = arg2.d & 0xffffffe0
                int64_t var_58_1 = _mm_unpackhi_pd(0xffffffff, 0xffffffff)
                int32_t var_88_3 = r12_1
                var_84_1.d = arg2.d
                int128_t var_78 = r15_1.o
                int96_t var_68_1 = var_48_1:8.12
                
                if (arg2.d != r10_1)
                    void* rax_10 = *(r15_1 + 0x20)
                    void* r11_1 = r15_1 + 0x10
                    
                    if (rax_10 != 0)
                        r11_1 = rax_10
                    
                    int32_t temp2_1
                    int32_t temp3_1
                    temp2_1:temp3_1 = sx.q(r10_1 - 1)
                    arg2 = zx.q(*(r11_1 + (sx.q(r8_4) << 2))) & zx.q(r12_1)
                    
                    if (arg2.d != 0)
                    label_1423e6a72:
                        int32_t rax_17 = neg.d(arg2.d) & arg2.d
                        uint64_t rflags_2
                        int32_t temp0_4
                        temp0_4, rflags_2 = _bit_scan_reverse(rax_17)
                        int32_t rax_18
                        
                        if (rax_17 == 0)
                            rax_18 = 0x20
                        else
                            rax_18 = 0x1f - temp0_4
                        
                        var_84_1.d = r9_3 - rax_18 + 0x1f
                        
                        if (r9_3 - rax_18 + 0x1f s> r10_1)
                            var_84_1.d = r10_1
                    else
                        while (true)
                            int64_t rcx_6 = sx.q(r8_4)
                            r9_3 += 0x20
                            r8_4 += 1
                            
                            if (rcx_6.d s>= (temp3_1 + (temp2_1 & 0x1f)) s>> 5)
                                break
                            
                            arg2 = zx.q(*(r11_1 + (rcx_6 << 2) + 4))
                            var_88_3 = 0xffffffff
                            
                            if (arg2.d != 0)
                                goto label_1423e6a72
                        
                        var_84_1.d = r10_1
                
                while (true)
                    int32_t var_5c
                    int64_t rcx_8 = sx.q(var_5c)
                    result = var_78.q
                    
                    if (rcx_8.d == (var_88_3.q u>> 0x20).d && var_68_1.q == r15_1 + 0x10
                            && result == r15_1)
                        goto label_1423e6adc
                    
                    if (*(*(*result + rcx_8 * 0x10) + 0x20) == r13)
                        result.b = 1
                        break
                    
                    var_68_1:8.d &= not.d(var_78:0xc.d)
                    sub_14059bdd0(&var_78:8)
                
                break
        
    label_1423e6adc:
        rbx = &rbx[1]
        
        if (rbx == rdi)
            goto label_1423e6ae9

return result
