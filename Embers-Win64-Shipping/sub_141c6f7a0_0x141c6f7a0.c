// 函数: sub_141c6f7a0
// 地址: 0x141c6f7a0
// 来自: E:\Embers\Embers\Binaries\Win64\Embers-Win64-Shipping.exe

int32_t r8 = 0
int32_t var_94 = 1
int32_t var_98 = 0
int32_t rdi = *(arg1 + 0x50)
void* var_90 = arg1 + 0x38
int32_t rcx = 0
int32_t var_88 = 0xffffffff
int64_t var_84 = 0

if (rdi != 0)
    void* rax_1 = *(arg1 + 0x48)
    void* r9_1 = arg1 + 0x38
    
    if (rax_1 != 0)
        r9_1 = rax_1
    
    int32_t temp0_1
    int32_t temp1_1
    temp0_1:temp1_1 = sx.q(rdi - 1)
    int32_t rdx_3 = *r9_1
    
    if (rdx_3 != 0)
    label_141c6f848:
        int32_t rax_8 = neg.d(rdx_3) & rdx_3
        uint64_t rflags_1
        int32_t temp0_2
        temp0_2, rflags_1 = _bit_scan_reverse(rax_8)
        int32_t var_94_1 = rax_8
        int32_t rax_9
        
        if (rax_8 == 0)
            rax_9 = 0x20
        else
            rax_9 = 0x1f - temp0_2
        
        var_84.d = rcx - rax_9 + 0x1f
        
        if (rcx - rax_9 + 0x1f s> rdi)
            var_84.d = rdi
    else
        while (true)
            int64_t rdx_4 = sx.q(r8)
            rcx += 0x20
            r8 += 1
            var_84:4.d = rcx
            var_98 = r8
            
            if (rdx_4.d s>= (temp1_1 + (temp0_1 & 0x1f)) s>> 5)
                break
            
            rdx_3 = *(r9_1 + (rdx_4 << 2) + 4)
            var_88 = 0xffffffff
            
            if (rdx_3 != 0)
                goto label_141c6f848
        
        var_84.d = rdi

int32_t rdx_6 = *(arg1 + 0x50)
double zmm2[0x2] = var_88.o
double var_38[0x2] = zmm2
int32_t r12 = 0xffffffff << (rdx_6.b & 0x1f)
uint128_t var_48 = var_98.o
int32_t r8_2 = rdx_6 s>> 5
int32_t r9_3 = rdx_6 & 0xffffffe0
int64_t var_58 = (_mm_unpackhi_pd(zmm2, zmm2[0])).q
int32_t var_88_1 = r12
var_84.d = rdx_6
uint128_t var_78 = (arg1 + 0x28).o
int96_t var_68 = var_48:8.12

if (rdx_6 != rdi)
    void* rax_11 = *(arg1 + 0x48)
    void* r10_1 = arg1 + 0x38
    
    if (rax_11 != 0)
        r10_1 = rax_11
    
    int32_t temp2_1
    int32_t temp3_1
    temp2_1:temp3_1 = sx.q(rdi - 1)
    int32_t rdx_10 = *(r10_1 + (sx.q(r8_2) << 2)) & r12
    
    if (rdx_10 != 0)
    label_141c6f912:
        int32_t rax_18 = neg.d(rdx_10) & rdx_10
        uint64_t rflags_2
        int32_t temp0_4
        temp0_4, rflags_2 = _bit_scan_reverse(rax_18)
        int32_t r11_1
        
        if (rax_18 == 0)
            r11_1 = 0x20
        else
            r11_1 = 0x1f - temp0_4
        
        var_84.d = r9_3 - r11_1 + 0x1f
        
        if (r9_3 - r11_1 + 0x1f s> rdi)
            var_84.d = rdi
    else
        while (true)
            int64_t rcx_5 = sx.q(r8_2)
            r9_3 += 0x20
            r8_2 += 1
            
            if (rcx_5.d s>= (temp3_1 + (temp2_1 & 0x1f)) s>> 5)
                break
            
            rdx_10 = *(r10_1 + (rcx_5 << 2) + 4)
            var_88_1 = 0xffffffff
            
            if (rdx_10 != 0)
                goto label_141c6f912
        
        var_84.d = rdi

while (true)
    int32_t var_5c
    int64_t rax_20 = sx.q(var_5c)
    void** rdx_11 = var_78.q
    
    if (rax_20.d == (var_88_1.q u>> 0x20).d && var_68.q == arg1 + 0x38 && rdx_11 == arg1 + 0x28)
        uint64_t result = zx.q(*(arg1 + 0x30) - *(arg1 + 0x5c))
        
        if (result.d s> 0)
            int64_t* rcx_11 = *(arg1 + 0x130)
            result = (*(*rcx_11 + 0x50))(rcx_11, *(arg1 + 0x180))
            
            if (result.b == 0)
                void* rax_25 = *(arg1 + 0x88)
                var_88_1.q = rax_25
                int32_t rcx_12
                
                if (rax_25 == 0)
                    rcx_12 = 0
                else
                    rcx_12 = *(rax_25 + 0x14)
                
                var_90:4.d = rcx_12
                int32_t rcx_13
                
                if (rax_25 == 0)
                    rcx_13 = 0
                else
                    rcx_13 = *(rax_25 + 0x958)
                
                int32_t var_94_2 = rcx_13
                uint128_t zmm0
                
                if (rax_25 == 0)
                    zmm0 = zx.o(0)
                else
                    zmm0 = _mm_cvtepi32_ps(zx.o(*(rax_25 + 0xc)))
                
                int64_t* rcx_14 = *(arg1 + 0x130)
                int64_t r9_5 = *(arg1 + 0x180)
                var_98 = 1
                var_90.d = (int.q(zmm0.d)).d
                int64_t* arg_8
                result =
                    (*(*rcx_14 + 0x10))(rcx_14, &arg_8, &var_98, r9_5, var_98, var_90, var_88_1)
                
                if (arg1 + 0x138 != result)
                    int64_t* rcx_15 = *(arg1 + 0x138)
                    *(arg1 + 0x138) = *result
                    *result = 0
                    
                    if (rcx_15 != 0)
                        result = (**rcx_15)(rcx_15, 1)
                
                int64_t* rcx_16 = arg_8
                
                if (rcx_16 != 0)
                    return (**rcx_16)(rcx_16, 1)
        
        return result
    
    sub_141c6fa70(arg1, *rdx_11 + rax_20 * 0x38 + 8)
    var_68:8.d &= not.d(var_78:0xc.d)
    sub_14059bdd0(&var_78:8)
