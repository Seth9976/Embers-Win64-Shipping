// 函数: sub_140bb6570
// 地址: 0x140bb6570
// 来自: E:\Embers\Embers\Binaries\Win64\Embers-Win64-Shipping.exe

if (arg1[1].d == *(arg1 + 0x34))
    return 0

int64_t r11 = *arg2
uint32_t r8_1 = (r11 u>> 4).d
int32_t rbx = 0
int64_t rsi_1 = sx.q(arg1[9].d) - 1
int32_t rdx_1 = (0x9e3779b9 - r8_1) ^ r8_1 << 8
void* rdi = &arg1[7]
int32_t rcx_2 = neg.d(rdx_1 + r8_1) ^ rdx_1 u>> 0xd
int32_t r8_4 = (r8_1 - rdx_1 - rcx_2) ^ rcx_2 u>> 0xc
int32_t rdx_4 = (rdx_1 - r8_4 - rcx_2) ^ r8_4 << 0x10
int32_t rcx_5 = (rcx_2 - rdx_4 - r8_4) ^ rdx_4 u>> 5
int32_t r8_7 = (r8_4 - rdx_4 - rcx_5) ^ rcx_5 u>> 3
int32_t rdx_7 = (rdx_4 - r8_7 - rcx_5) ^ r8_7 << 0xa
void* rax_15 = arg1[8]

if (rax_15 != 0)
    rdi = rax_15

int32_t i = *(rdi + (((sx.q(rcx_5 - rdx_7 - r8_7) ^ zx.q(rdx_7) u>> 0xf) & rsi_1) << 2))

if (i != 0xffffffff)
    void* r10_1 = *arg1
    
    do
        int64_t i_1 = sx.q(i)
        int64_t rax_19 = i_1 * 2
        
        if (*(r10_1 + (rax_19 << 3)) == r11)
            int64_t r8_8 = i_1 * 2
            int64_t rax_21 = sx.q(*(r10_1 + (r8_8 << 3) + 0xc)) & rsi_1
            void* rcx_11 = rdi + (rax_21 << 2)
            
            for (int32_t j = *(rdi + (rax_21 << 2)); j != 0xffffffff; j = *rcx_11)
                if (j == i)
                    *rcx_11 = *(r10_1 + (r8_8 << 3) + 8)
                    break
                
                rcx_11 = r10_1 + 8 + (sx.q(j) << 4)
            
            rbx = 1
            void* r11_2 = &arg1[2]
            
            if (*(arg1 + 0x34) != 0)
                *(*arg1 + sx.q(arg1[6].d) * 0x10) = i
            
            int64_t rcx_16 = *arg1
            int32_t rax_26 = -1
            *(rcx_16 + (r8_8 << 3)) = 0xffffffff
            
            if (*(arg1 + 0x34) s> 0)
                rax_26 = arg1[6].d
            
            *(rcx_16 + (r8_8 << 3) + 4) = rax_26
            *(arg1 + 0x34) += 1
            arg1[6].d = i
            void* rax_27 = *(r11_2 + 0x10)
            
            if (rax_27 != 0)
                r11_2 = rax_27
            
            int32_t temp0_1
            int32_t temp1_1
            temp0_1:temp1_1 = sx.q(i)
            int64_t rcx_17 = sx.q((temp1_1 + (temp0_1 & 0x1f)) s>> 5)
            *(r11_2 + (rcx_17 << 2)) &= not.d(rol.d(1, i.b))
            break
        
        i = *(r10_1 + (rax_19 << 3) + 8)
    while (i != 0xffffffff)

return zx.q(rbx)
