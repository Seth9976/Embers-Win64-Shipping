// 函数: sub_1419fe140
// 地址: 0x1419fe140
// 来自: E:\Embers\Embers\Binaries\Win64\Embers-Win64-Shipping.exe

int64_t* r8 = arg1[1]
int16_t* rcx = *r8

if (&rcx[1] u<= r8[1])
    *arg2 = *rcx
    int64_t* rax_2 = arg1[1]
    *rax_2 += 2
else if ((*(arg1 + 0x29) & 0x20) != 0)
    sub_140b54000(arg1, arg2, arg3)
else
    (*(*arg1 + 0x150))(arg1, arg3, 2)

int64_t* rcx_2 = arg1[1]
int16_t* r8_1 = *rcx_2

if (&r8_1[1] u<= rcx_2[1])
    arg2[1] = *r8_1
    int64_t* rax_6 = arg1[1]
    *rax_6 += 2
else if ((*(arg1 + 0x29) & 0x20) != 0)
    sub_140b54000(arg1, &arg2[1], arg3)
else
    (*(*arg1 + 0x150))(arg1, &arg2[1], 2)

int64_t* rcx_4 = arg1[1]
int16_t* r8_2 = *rcx_4

if (&r8_2[1] u<= rcx_4[1])
    arg2[2] = *r8_2
    int64_t* rax_10 = arg1[1]
    *rax_10 += 2
else if ((*(arg1 + 0x29) & 0x20) != 0)
    sub_140b54000(arg1, &arg2[2], arg3)
else
    (*(*arg1 + 0x150))(arg1, &arg2[2], 2)

int64_t* rcx_6 = arg1[1]
int16_t* r8_3 = *rcx_6

if (&r8_3[1] u<= rcx_6[1])
    arg2[3] = *r8_3
    int64_t* rax_14 = arg1[1]
    *rax_14 += 2
else if ((*(arg1 + 0x29) & 0x20) != 0)
    sub_140b54000(arg1, &arg2[3], arg3)
else
    (*(*arg1 + 0x150))(arg1, &arg2[3], 2)

int64_t* rcx_8 = arg1[1]
int16_t* r8_4 = *rcx_8

if (&r8_4[1] u<= rcx_8[1])
    arg2[4] = *r8_4
    int64_t* rax_18 = arg1[1]
    *rax_18 += 2
else if ((*(arg1 + 0x29) & 0x20) != 0)
    sub_140b54000(arg1, &arg2[4], arg3)
else
    (*(*arg1 + 0x150))(arg1, &arg2[4], 2)

int64_t* rcx_10 = arg1[1]
int16_t* r8_5 = *rcx_10

if (&r8_5[1] u<= rcx_10[1])
    arg2[5] = *r8_5
    int64_t* rax_22 = arg1[1]
    *rax_22 += 2
else if ((*(arg1 + 0x29) & 0x20) != 0)
    sub_140b54000(arg1, &arg2[5], arg3)
else
    (*(*arg1 + 0x150))(arg1, &arg2[5], 2)

int64_t* rcx_12 = arg1[1]
int16_t* r8_6 = *rcx_12

if (&r8_6[1] u<= rcx_12[1])
    arg2[6] = *r8_6
    int64_t* rax_26 = arg1[1]
    *rax_26 += 2
else if ((*(arg1 + 0x29) & 0x20) != 0)
    sub_140b54000(arg1, &arg2[6], arg3)
else
    (*(*arg1 + 0x150))(arg1, &arg2[6], 2)

int64_t* rcx_14 = arg1[1]
int16_t* r8_7 = *rcx_14

if (&r8_7[1] u<= rcx_14[1])
    arg2[7] = *r8_7
    int64_t* rax_30 = arg1[1]
    *rax_30 += 2
else if ((*(arg1 + 0x29) & 0x20) != 0)
    sub_140b54000(arg1, &arg2[7], arg3)
else
    (*(*arg1 + 0x150))(arg1, &arg2[7], 2)

int64_t* rcx_16 = arg1[1]
int16_t* r8_8 = *rcx_16

if (&r8_8[1] u<= rcx_16[1])
    arg2[8] = *r8_8
    int64_t* rax_34 = arg1[1]
    *rax_34 += 2
else if ((*(arg1 + 0x29) & 0x20) != 0)
    sub_140b54000(arg1, &arg2[8], arg3)
else
    (*(*arg1 + 0x150))(arg1, &arg2[8], 2)

int64_t* rcx_18 = arg1[1]
int16_t* r8_9 = *rcx_18

if (&r8_9[1] u<= rcx_18[1])
    arg2[9] = *r8_9
    int64_t* rax_38 = arg1[1]
    *rax_38 += 2
else if ((*(arg1 + 0x29) & 0x20) != 0)
    sub_140b54000(arg1, &arg2[9], arg3)
else
    (*(*arg1 + 0x150))(arg1, &arg2[9], 2)

int64_t* rcx_20 = arg1[1]
int16_t* r8_10 = *rcx_20

if (&r8_10[1] u<= rcx_20[1])
    arg2[0xa] = *r8_10
    int64_t* rax_42 = arg1[1]
    *rax_42 += 2
else if ((*(arg1 + 0x29) & 0x20) != 0)
    sub_140b54000(arg1, &arg2[0xa], arg3)
else
    (*(*arg1 + 0x150))(arg1, &arg2[0xa], 2)

int64_t* rcx_22 = arg1[1]
int16_t* r8_11 = *rcx_22

if (&r8_11[1] u<= rcx_22[1])
    arg2[0xb] = *r8_11
    int64_t* rax_46 = arg1[1]
    *rax_46 += 2
    return arg1

if ((*(arg1 + 0x29) & 0x20) != 0)
    sub_140b54000(arg1, &arg2[0xb], arg3)
    return arg1

(*(*arg1 + 0x150))(arg1, &arg2[0xb], 2)
return arg1
