// 函数: sub_14064b0a0
// 地址: 0x14064b0a0
// 来自: E:\Embers\Embers\Binaries\Win64\Embers-Win64-Shipping.exe

int64_t* var_98 = nullptr

if (*(arg2 + 0x20) == 0)
    int64_t* r8_1 = *(arg2 + 0x80)
    *(arg2 + 0x80) = r8_1[4]
    sub_140d30490(arg2, &var_98, r8_1)
else
    sub_140d30460(arg2, *(arg2 + 0x18), &var_98)

int64_t var_70

if (*(arg2 + 0x20) == 0)
    int64_t* r8_3 = *(arg2 + 0x80)
    *(arg2 + 0x80) = r8_3[4]
    sub_140d30490(arg2, &var_70, r8_3)
else
    sub_140d30460(arg2, *(arg2 + 0x18), &var_70)

int32_t var_a0 = 0

if (*(arg2 + 0x20) == 0)
    int64_t* r8_5 = *(arg2 + 0x80)
    *(arg2 + 0x80) = r8_5[4]
    sub_140d30490(arg2, &var_a0, r8_5)
else
    sub_140d30460(arg2, *(arg2 + 0x18), &var_a0)

int32_t var_a4 = 0

if (*(arg2 + 0x20) == 0)
    int64_t* r8_7 = *(arg2 + 0x80)
    *(arg2 + 0x80) = r8_7[4]
    sub_140d30490(arg2, &var_a4, r8_7)
else
    sub_140d30460(arg2, *(arg2 + 0x18), &var_a4)

int32_t var_a8 = 0

if (*(arg2 + 0x20) == 0)
    int64_t* r8_9 = *(arg2 + 0x80)
    *(arg2 + 0x80) = r8_9[4]
    sub_140d30490(arg2, &var_a8, r8_9)
else
    sub_140d30460(arg2, *(arg2 + 0x18), &var_a8)

uint128_t var_28

if (*(arg2 + 0x20) == 0)
    int64_t* r8_11 = *(arg2 + 0x80)
    *(arg2 + 0x80) = r8_11[4]
    sub_140d30490(arg2, &var_28, r8_11)
else
    sub_140d30460(arg2, *(arg2 + 0x18), &var_28)

int64_t var_80

if (*(arg2 + 0x20) == 0)
    int64_t* r8_13 = *(arg2 + 0x80)
    *(arg2 + 0x80) = r8_13[4]
    sub_140d30490(arg2, &var_80, r8_13)
else
    sub_140d30460(arg2, *(arg2 + 0x18), &var_80)

int64_t var_90

if (*(arg2 + 0x20) == 0)
    int64_t* r8_15 = *(arg2 + 0x80)
    *(arg2 + 0x80) = r8_15[4]
    sub_140d30490(arg2, &var_90, r8_15)
else
    sub_140d30460(arg2, *(arg2 + 0x18), &var_90)

int32_t arg_20 = 0

if (*(arg2 + 0x20) == 0)
    int64_t* r8_17 = *(arg2 + 0x80)
    *(arg2 + 0x80) = r8_17[4]
    sub_140d30490(arg2, &arg_20, r8_17)
else
    sub_140d30460(arg2, *(arg2 + 0x18), &arg_20)

int32_t arg_10 = 0
int64_t r8_19

if (*(arg2 + 0x20) == 0)
    int64_t* r8_20 = *(arg2 + 0x80)
    *(arg2 + 0x80) = r8_20[4]
    r8_19 = sub_140d30490(arg2, &arg_10, r8_20)
else
    r8_19 = sub_140d30460(arg2, *(arg2 + 0x18), &arg_10)

int64_t rax_11 = *(arg2 + 0x20)
int64_t zmm1 = var_70
int32_t r9 = var_a4
int64_t rdi
rdi.b = rax_11 != 0
int512_t zmm2
zmm2.o = var_a0
bool cond:1 = arg_10 != 0
int64_t* rcx_10 = var_98
int32_t var_88
int32_t var_50 = var_88
int32_t var_78
int32_t var_40 = var_78
uint64_t var_58 = var_90
int32_t var_68
int32_t var_30 = var_68
int32_t rax_14
rax_14.b = cond:1
char var_b0 = rax_14.b
uint64_t var_48 = var_80
uint128_t zmm0 = var_28
*(arg2 + 0x20) = rdi + rax_11
int64_t var_38 = zmm1
uint128_t var_18 = zmm0
int32_t var_b8 = arg_20.d
uint64_t* var_c0 = &var_58
uint64_t* var_c8 = &var_48
uint128_t* var_d0 = &var_18
return sub_14062bc30(rcx_10, &var_38, r8_19, r9, var_a8.d)
