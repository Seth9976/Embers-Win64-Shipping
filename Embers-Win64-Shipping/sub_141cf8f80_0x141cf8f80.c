// 函数: sub_141cf8f80
// 地址: 0x141cf8f80
// 来自: E:\Embers\Embers\Binaries\Win64\Embers-Win64-Shipping.exe

if (arg1 != arg2)
    int32_t r11_1 = arg1[3].d
    int32_t r8_1 = 0
    int32_t rcx = 0
    int32_t var_78_1 = 0
    int32_t var_74_1 = 1
    void* var_70_1 = &arg1[2]
    int32_t var_68_1 = 0xffffffff
    int64_t var_64_1 = 0
    
    if (r11_1 != 0)
        void* r9_1 = arg1[2]
        
        if ((r9_1.b & 1) != 0)
            r9_1 = (r9_1 s>> 1) + &arg1[2]
        
        int32_t temp0_1
        int32_t temp1_1
        temp0_1:temp1_1 = sx.q(r11_1 - 1)
        int32_t rdx_2 = *r9_1
        
        if (rdx_2 != 0)
        label_141cf9038:
            int32_t rax_7 = neg.d(rdx_2) & rdx_2
            uint64_t rflags_1
            int32_t temp0_2
            temp0_2, rflags_1 = _bit_scan_reverse(rax_7)
            int32_t var_74_2 = rax_7
            int32_t rax_8
            
            if (rax_7 == 0)
                rax_8 = 0x20
            else
                rax_8 = 0x1f - temp0_2
            
            var_64_1.d = rcx - rax_8 + 0x1f
            
            if (rcx - rax_8 + 0x1f s> r11_1)
                var_64_1.d = r11_1
        else
            while (true)
                int64_t rdx_3 = sx.q(r8_1)
                rcx += 0x20
                r8_1 += 1
                var_64_1:4.d = rcx
                var_78_1 = r8_1
                
                if (rdx_3.d s>= (temp1_1 + (temp0_1 & 0x1f)) s>> 5)
                    break
                
                rdx_2 = *(r9_1 + (rdx_3 << 2) + 4)
                int32_t var_68_2 = 0xffffffff
                
                if (rdx_2 != 0)
                    goto label_141cf9038
            
            var_64_1.d = r11_1
    
    int32_t rdx_5 = arg1[3].d
    int32_t r9_4 = rdx_5 s>> 5
    int32_t r12_1 = 0xffffffff << (rdx_5.b & 0x1f)
    int32_t r10_2 = rdx_5 & 0xffffffe0
    int32_t var_68_3 = r12_1
    int64_t* r8_2 = arg1
    var_64_1.d = rdx_5
    int128_t var_50 = var_78_1.o
    int64_t var_40_1 = 0xffffffff
    
    if (rdx_5 != r11_1)
        void* r11_2 = arg1[2]
        
        if ((r11_2.b & 1) != 0)
            r11_2 = (r11_2 s>> 1) + &arg1[2]
        
        int32_t temp2_1
        int32_t temp3_1
        temp2_1:temp3_1 = sx.q(r11_1 - 1)
        int32_t rdx_9 = *(r11_2 + (sx.q(r9_4) << 2)) & r12_1
        
        if (rdx_9 != 0)
        label_141cf90f2:
            int32_t rax_16 = neg.d(rdx_9) & rdx_9
            uint64_t rflags_2
            int32_t temp0_3
            temp0_3, rflags_2 = _bit_scan_reverse(rax_16)
            int32_t r15_1
            
            if (rax_16 == 0)
                r15_1 = 0x20
            else
                r15_1 = 0x1f - temp0_3
            
            var_64_1.d = r10_2 - r15_1 + 0x1f
            
            if (r10_2 - r15_1 + 0x1f s> r11_1)
                var_64_1.d = r11_1
        else
            while (true)
                int64_t rcx_5 = sx.q(r9_4)
                r10_2 += 0x20
                r9_4 += 1
                
                if (rcx_5.d s>= (temp3_1 + (temp2_1 & 0x1f)) s>> 5)
                    break
                
                rdx_9 = *(r11_2 + (rcx_5 << 2) + 4)
                var_68_3 = 0xffffffff
                
                if (rdx_9 != 0)
                    goto label_141cf90f2
            
            var_64_1.d = r11_1
    
    while (true)
        int64_t rcx_7 = sx.q(var_40_1:4.d)
        
        if (rcx_7.d == (var_68_3.q u>> 0x20).d && var_50:8.q == &arg1[2] && r8_2 == arg1)
            break
        
        void* rax_18 = *r8_2
        
        if ((rax_18.b & 1) != 0)
            rax_18 = (rax_18 s>> 1) + r8_2
        
        sub_140a1d5c0((rcx_7 << 5) + rax_18)
        var_40_1.d &= not.d(var_50:4.d)
        sub_141d02a20(&var_50)
        r8_2 = arg1
    
    sub_140a2c970(arg1, arg2)
    arg1[1].d = arg2[1].d
    *(arg1 + 0xc) = *(arg2 + 0xc)
    arg2[1] = 0
    
    if (&arg1[2] != &arg2[2])
        sub_140a2c970(&arg1[2], &arg2[2])
        arg1[3].d = arg2[3].d
        *(arg1 + 0x1c) = *(arg2 + 0x1c)
        arg2[3] = 0
    
    arg1[4].d = arg2[4].d
    *(arg1 + 0x24) = *(arg2 + 0x24)
    *(arg2 + 0x24) = 0
    arg2[4].d = 0xffffffff

return arg1
