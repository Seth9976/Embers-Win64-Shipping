// 函数: sub_1429d77c0
// 地址: 0x1429d77c0
// 来自: E:\Embers\Embers\Binaries\Win64\Embers-Win64-Shipping.exe

void* r8 = *(arg1 + 8)
void* rcx = *(arg1 + 0x10)
int32_t rdx_4 = ((*(r8 + 0x38) - *(r8 + 0x30)) s>> 2).d + ((*(r8 + 0x170) - *(r8 + 0x168)) s>> 2).d
    + ((*(r8 + 0xb0) - *(r8 + 0xa8)) s>> 2).d
int32_t rbx = rdx_4 + (*(r8 + 4) << 1)
sub_14058d4e0(rcx + 0x150, sx.q(*(rcx + 8) * 2))
int64_t rbx_1 = sx.q(rbx)
sub_14058d4e0(*(arg1 + 0x10) + 0x168, rbx_1)
sub_1429cd2b0(*(arg1 + 0x10) + 0x180, rbx_1)

if (*(arg1 + 0x54) != 0)
    sub_1429d7bd0(arg1)
    sub_1429d78e0(arg1)
    sub_1429d7d40(arg1)
else
    sub_1429d7d40(arg1)
    sub_1429d7bd0(arg1)
    sub_1429d78e0(arg1)

void* r9 = *(arg1 + 0x10)
int64_t r8_1 = *(r9 + 0x150)
int32_t rcx_11 = *(r9 + 8) * 2
int64_t rbx_2 = sx.q(*(r8_1 + (sx.q(rcx_11 - 2) << 2)) + *(r8_1 + (sx.q(rcx_11 - 1) << 2)))
sub_14058d4e0(r9 + 0x168, rbx_2)
return sub_1429cd2b0(*(arg1 + 0x10) + 0x180, rbx_2) __tailcall
