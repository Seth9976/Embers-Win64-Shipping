...
if (i_1 s> 0)
    ...
    int64_t r11_1 = *arg2[1]
    uint32_t zmm13[0x4] = data_143ef8480
    int64_t i
    
    do
        *(r8 + (sx.q(rdx[1]) << 2)) = *(arg1 + 0x1c)
        float zmm5[0x4] = *(arg1 + 0x40)
        result = sx.q(*rdx)
        float temp0_1[0x4] = __mulps_xmmps_memps(data_143ef8490, *(arg1 + 0x20))
        float temp0_2[0x4] = _mm_rcp_ps(zmm5)
        void* rcx_2 = result * 3
        float zmm2[0x4] = *(r11_1 + (rcx_2 << 2) + 4)
        float temp0_3[0x4] =
            _mm_unpacklo_ps(*(r11_1 + (rcx_2 << 2)), (*(r11_1 + (rcx_2 << 2) + 8))[0].q)
