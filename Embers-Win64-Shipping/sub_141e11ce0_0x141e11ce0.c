// 函数: sub_141e11ce0
// 地址: 0x141e11ce0
// 来自: E:\Embers\Embers\Binaries\Win64\Embers-Win64-Shipping.exe

__builtin_memset(arg1, 0, 0x14)
*(arg1 + 0x18) = 0
*(arg1 + 0x20) = 0
*(arg1 + 0x28) = 0
*(arg1 + 0x2c) = 0x3f800000
*(arg1 + 0x30) = 0xfffffffe
*(arg1 + 0x38) = 0xfffffffe
__builtin_memset(arg1 + 0x40, 0, 0x11)
__builtin_memset(arg1 + 0x58, 0, 0x20)
*(arg1 + 0x78) = 1
*(arg1 + 0x7c) = 0xffffffff
*(arg1 + 0x80) = 0
*(arg1 + 0x88) = 0
*(arg1 + 0x8c) = 0xffffffff
__builtin_memset(arg1 + 0x90, 0, 0x20)
*(arg1 + 0xb0) = 0x7f7fffff
*(arg1 + 0xb4) = 0x7f7fffff
*(arg1 + 0xb8) = 0
__builtin_memset(arg1 + 0xc8, 0, 0x40)
sub_140d3a3a0(arg1 + 0x108, nullptr)
*(arg1 + 0x110) = 0xffffffff
__builtin_memset(arg1 + 0x118, 0, 0x14)
*(arg1 + 0x12c) = 0x3f800000
sub_141ddd6c0(arg1 + 0x130, 0x3e4ccccd)
*(arg1 + 0x160) = 0
*(arg1 + 0x164) = 0
*(arg1 + 0x16c) = 0
*(arg1 + 0x174) = 0xffffffff
*(arg1 + 0x17c) = 0
*(arg1 + 0x184) = 0
*(arg1 + 0x188) = 0
__builtin_memset(arg1 + 0x190, 0, 0x18)
*(arg1 + 0x1a8) = 0xffffffff
return arg1
