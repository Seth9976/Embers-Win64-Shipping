// 函数: sub_142432740
// 地址: 0x142432740
// 来自: E:\Embers\Embers\Binaries\Win64\Embers-Win64-Shipping.exe

int64_t r15
r15.b = 0
int32_t rax_1 = arg1[1].d - *(arg1 + 0x34)
int64_t r12 = sx.q(arg3)
int32_t rdi_3

if (rax_1 == 1)
    rdi_3 = arg5
label_142432874:
    
    if (sub_14242f1d0(arg1, arg1[1].d - *(arg1 + 0x34), 0) == 0)
        void* r9 = &arg1[7]
        int32_t rax_11 = (arg1[9].d - 1) & r12.d
        *(arg4 + 0x24) = rax_11
        void* rdx_9 = *(r9 + 8)
        int64_t r8_1 = sx.q(rax_11)
        void* rax_12 = r9
        
        if (rdx_9 != 0)
            rax_12 = rdx_9
        
        *(arg4 + 0x20) = *(rax_12 + (((sx.q(arg1[9].d) - 1) & r8_1) << 2))
        void* rax_14 = *(r9 + 8)
        
        if (rax_14 != 0)
            r9 = rax_14
        
        *(r9 + (((sx.q(arg1[9].d) - 1) & r8_1) << 2)) = rdi_3
else
    int32_t rbx_1
    
    if (rax_1 != 0)
        void* r8 = &arg1[7]
        void* rcx = arg1[8]
        
        if (rcx != 0)
            r8 = rcx
        
        rbx_1 = *(r8 + (((sx.q(arg1[9].d) - 1) & r12) << 2))
    
    if (rax_1 == 0 || rbx_1 == 0xffffffff)
    label_1424327db:
        rbx_1 = -1
    else
        int64_t rdx_3 = *arg1
        
        while (true)
            int64_t rdi_1 = sx.q(rbx_1) * 0x28
            
            if (sub_14094dfb0(rdi_1 + rdx_3, arg4) != 0)
                break
            
            rdx_3 = *arg1
            rbx_1 = *(rdi_1 + rdx_3 + 0x20)
            
            if (rbx_1 == 0xffffffff)
                goto label_1424327db
    
    r15.b = rbx_1 != 0xffffffff
    
    if (rbx_1 == 0xffffffff)
        rdi_3 = arg5
    else
        int64_t rcx_3 = sx.q(rbx_1) * 5
        int64_t rax_5 = *arg1
        int64_t* rdi_2 = *(rax_5 + (rcx_3 << 3) + 0x10)
        
        if (rdi_2 != 0)
            rdi_2[1].d -= 1
            
            if (rdi_2[1].d == 1)
                (**rdi_2)(rdi_2)
                int32_t temp1_1 = *(rdi_2 + 0xc)
                *(rdi_2 + 0xc) -= 1
                
                if (temp1_1 == 1)
                    (*(*rdi_2 + 8))(rdi_2, 1)
        
        memmove(rax_5 + (rcx_3 << 3), arg4, 0x20)
        sub_1405c3590(arg1, arg5, 1)
        rdi_3 = rbx_1
    
    if (rbx_1 == 0xffffffff)
        goto label_142432874

*arg2 = rdi_3

if (arg6 != 0)
    *arg6 = r15.b

return arg2
