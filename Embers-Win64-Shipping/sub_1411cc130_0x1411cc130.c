// 函数: sub_1411cc130
// 地址: 0x1411cc130
// 来自: E:\Embers\Embers\Binaries\Win64\Embers-Win64-Shipping.exe

void var_128
int64_t rax_1 = __security_cookie ^ &var_128
void*** rdx_2 = (*(arg2 + 0x30) + 7) & 0xfffffffffffffff8
uint128_t zmm7 = zx.o(arg1[0x3a].d)
void* rax_2 = &rdx_2[5]
uint128_t zmm6 = _mm_cvtepi32_ps(zx.o(*(arg1 + 0x1d4)))
zmm7 = _mm_cvtepi32_ps(zmm7)

if (rax_2 u> *(arg2 + 0x38))
    zmm6 = sub_140b0e3d0(arg2 + 0x30, 0x30)
    rdx_2 = (*(arg2 + 0x30) + 7) & 0xfffffffffffffff8
    rax_2 = &rdx_2[5]

*(arg2 + 0x30) = rax_2
void**** rax_3 = *(arg2 + 8)
*(arg2 + 0x14) += 1
*rax_3 = rdx_2
*(arg2 + 8) = &rdx_2[1]
rdx_2[1] = 0
*rdx_2 = &data_142d54998
*(rdx_2 + 0x1c) = zmm7.d
rdx_2[4].d = zmm6.d
rdx_2[2] = 0
rdx_2[3].d = 0
*(rdx_2 + 0x24) = 0x3f800000
int128_t var_108
__builtin_memset(&var_108, 0, 0x30)
int64_t var_ac
__builtin_memset(&var_ac, 0, 0x43)
int128_t var_c0 = zx.o(0)
int16_t var_68 = 0
sub_140fdc870(arg2, &var_108)
void* rcx_2 = arg1[0x36]
int64_t var_d8 = data_1439b6188
int64_t var_d0 = data_14395da18
int64_t var_c8 = data_14395d9e8
var_108.q = data_1439c9228
int64_t rax_9 = 0

if (rcx_2 != 0)
    int64_t rdx_6 = sx.q(*(rcx_2 + 0x10c))
    int64_t* r14_1 = *(arg1[0x37] + 0x10)
    int64_t rax_11 = r14_1[3]
    
    if (*(rax_11 + (rdx_6 << 3)) == 0)
        sub_1419ccf30(r14_1, rdx_6.d)
        rax_11 = r14_1[3]
    
    rax_9 = *(rax_11 + (rdx_6 << 3))

void* rcx_4 = arg1[0x38]
var_108:8.q = rax_9
int64_t rax_12 = 0

if (rcx_4 != 0)
    int64_t rdx_7 = sx.q(*(rcx_4 + 0x10c))
    int64_t* r15_2 = *(arg1[0x39] + 0x10)
    int64_t rax_14 = r15_2[3]
    
    if (*(rax_14 + (rdx_7 << 3)) == 0)
        sub_1419ccf30(r15_2, rdx_7.d)
        rax_14 = r15_2[3]
    
    rax_12 = *(rax_14 + (rdx_7 << 3))

int128_t var_e8
var_e8.q = rax_12
int32_t var_b0 = 0
sub_1419870b0(arg2, &var_108, 2)
void* rax_15 = arg1[0x36]
int64_t r8 = 0

if (rax_15 != 0)
    int64_t rdx_9 = sx.q(*(rax_15 + 0x10c))
    int64_t* r15_3 = *(arg1[0x37] + 0x10)
    int64_t rax_17 = r15_3[3]
    
    if (*(rax_17 + (rdx_9 << 3)) == 0)
        sub_1419ccf30(r15_3, rdx_9.d)
        rax_17 = r15_3[3]
    
    r8 = *(rax_17 + (rdx_9 << 3))

sub_1411bcae0(arg2, &arg1[0x36], r8, &arg1[2])
void* rax_18 = arg1[0x38]
int64_t r8_1 = 0

if (rax_18 != 0)
    int64_t rdx_11 = sx.q(*(rax_18 + 0x10c))
    int64_t* r15_4 = *(arg1[0x39] + 0x10)
    int64_t rax_20 = r15_4[3]
    
    if (*(rax_20 + (rdx_11 << 3)) == 0)
        sub_1419ccf30(r15_4, rdx_11.d)
        rax_20 = r15_4[3]
    
    r8_1 = *(rax_20 + (rdx_11 << 3))

sub_1411bc520(arg2, &arg1[0x38], r8_1, &arg1[0x1c])
void*** rcx_13 = (*(arg2 + 0x30) + 7) & 0xfffffffffffffff8
int32_t rsi_1 = *(*arg1 + 8)
void* rax_22 = &rcx_13[4]

if (rax_22 u> *(arg2 + 0x38))
    sub_140b0e3d0(arg2 + 0x30, 0x28)
    rcx_13 = (*(arg2 + 0x30) + 7) & 0xfffffffffffffff8
    rax_22 = &rcx_13[4]

*(arg2 + 0x30) = rax_22
int64_t* rax_23 = *(arg2 + 8)
*(arg2 + 0x14) += 1
*rax_23 = rcx_13
*(arg2 + 8) = &rcx_13[1]
rcx_13[1] = 0
*rcx_13 = &data_142d54988
rcx_13[2].d = 0
*(rcx_13 + 0x14) = rsi_1
rcx_13[3].d = 1
__security_check_cookie(rax_1 ^ &var_128)
return &data_142d54988
