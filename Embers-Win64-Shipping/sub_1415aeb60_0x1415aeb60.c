// 函数: sub_1415aeb60
// 地址: 0x1415aeb60
// 来自: E:\Embers\Embers\Binaries\Win64\Embers-Win64-Shipping.exe

void var_298
int64_t rax_1 = __security_cookie ^ &var_298
int32_t r15 = 0
int64_t* r13 = arg3
int64_t* r12 = arg2
int64_t* r14 = arg1
uint64_t* var_270 = arg6
int64_t var_240
int64_t* var_278 = &var_240
float* var_198 = arg5
int32_t* var_1a8 = arg7
var_240 = 0
int64_t var_238 = 0
char rax_3
float zmm6[0x4]
float zmm7[0x4]
float zmm8[0x4]
float zmm9[0x4]
int128_t zmm10
int128_t zmm11
int128_t zmm12
int128_t zmm13
int128_t zmm15
rax_3, zmm6, zmm7, zmm8, zmm9, zmm10, zmm11, zmm12, zmm13, zmm15 =
    sub_1416d3990(arg1, arg2, arg3, arg4, var_278)
int32_t rdi_1

if (rax_3 != 0)
    int128_t var_1f8
    __builtin_memset(&var_1f8, 0, 0x18)
    float (* rdi_2)[0x4] = var_1f8:8.q
    int32_t i = 0
    float (* rbx_1)[0x4] = var_1f8.q
    int64_t* var_250
    int64_t var_1e8
    float zmm1_1[0x4]
    
    if (var_238.d s> 0)
        do
            char rax_5
            rax_5, zmm6, zmm7, zmm8, zmm9, zmm10, zmm11, zmm12, zmm13, zmm15 =
                sub_141743d50(sx.q(i) * 0x38 + var_240, *r14, *r12)
            
            if (rax_5 != 0)
                int64_t var_248
                var_248.d = i
                var_250 = &var_240
                
                if (var_1e8 == rdi_2)
                    sub_141701fc0(&var_1f8, rdi_2, &var_250)
                    rdi_2 = var_1f8:8.q
                    rbx_1 = var_1f8.q
                else
                    *rdi_2 = var_250.o
                    rdi_2 = &rdi_2[1]
                    var_1f8:8.q = rdi_2
                
                int64_t r8_4 = (rdi_2 - rbx_1) s>> 4
                
                if (r8_4 s>= 2)
                    zmm1_1 = rdi_2[-1]
                    int64_t j_1 = r8_4 - 1
                    
                    if (j_1 s> 0)
                        int64_t r11_1 = sx.q(_mm_bsrli_si128(zmm1_1, 8)[0]) * 0x38
                        int64_t j
                        
                        do
                            j = (j_1 - 1) s>> 1
                            int64_t rdx_2 = *rbx_1[j][0].q
                            
                            if ((*(r11_1 + rdx_2 + 0x18))[0] f>=
                                    *(sx.q(rbx_1[j][2]) * 0x38 + rdx_2 + 0x18))
                                break
                            
                            rbx_1[j_1] = rbx_1[j]
                            j_1 = j
                        while (j s> 0)
                    
                    rbx_1[j_1] = zmm1_1
            
            i += 1
        while (i s< var_238.d)
    
    int64_t rcx_7 = (rdi_2 - rbx_1) s>> 4
    float (* rdx_4)[0x4]
    
    if (rcx_7 == 0)
        rdx_4 = var_240
    else
        rdx_4 = sx.q((*rbx_1)[2]) * 0x38 + var_240
    
    float zmm0_1[0x4] = *rdx_4
    int64_t* rsi_1 = nullptr
    zmm1_1 = rdx_4[1]
    var_250 = nullptr
    float var_58_1[0x4] = zmm6
    float var_68_1[0x4] = zmm7
    float var_78_1[0x4] = zmm8
    float var_88_1[0x4] = zmm9
    int128_t var_98_1 = zmm10
    int128_t var_a8_1 = zmm11
    float var_130[0x4] = zmm0_1
    int64_t var_248_1 = 0
    float var_120_1[0x4] = zmm1_1
    int32_t rdx_5 = 0
    int64_t var_100_1 = rdx_4[3][0].q
    float var_110_1[0x4] = rdx_4[2]
    int512_t zmm6_2
    int512_t zmm7_1
    int512_t zmm8_1
    int512_t zmm9_1
    int512_t zmm10_1
    int512_t zmm11_1
    int512_t zmm12_2
    int512_t zmm13_2
    int512_t zmm15_2
    
    if (rcx_7 == 0)
    label_1415af5a0:
        zmm6_2, zmm7_1, zmm8_1, zmm9_1, zmm10_1, zmm11_1, zmm12_2, zmm13_2, zmm15_2 =
            sub_14154a2a0(*arg1, *arg2, &var_130, var_198, arg6, var_1a8, arg8)
        rdi_1 = 1
    else
        float zmm2_1[0x4] = 0x80000000
        
        while (true)
            if (rdx_5 s< 0x80)
                float rax_11 = (*rbx_1)[2]
                float var_258_1 = rax_11
                
                if (rcx_7 s>= 2)
                    zmm1_1 = rdi_2[-1]
                    int64_t i_2 = 0
                    int64_t i_3 = 0
                    rdi_2[-1] = *rbx_1
                    int64_t r11_4 = (&rdi_2[-1] - rbx_1) s>> 4
                    int64_t rsi_3 = (r11_4 - 1) s>> 1
                    
                    if (rsi_3 s> 0)
                        do
                            int64_t r9_1 = i_3 * 2
                            int64_t rcx_9 = r9_1 * 2
                            int64_t r8_7 = **(rbx_1 + (rcx_9 << 3) + 0x20)
                            i_3 = r9_1 + 1
                            
                            if ((*(sx.q((*rbx_1)[rcx_9 * 2 + 0xa]) * 0x38 + r8_7 + 0x18))[0] f<=
                                    *(sx.q((*rbx_1)[rcx_9 * 2 + 6]) * 0x38 + r8_7 + 0x18))
                                i_3 = r9_1 + 2
                            
                            rbx_1[i_2] = rbx_1[i_3]
                            i_2 = i_3
                        while (i_3 s< rsi_3)
                    
                    if (i_3 == rsi_3 && (r11_4.b & 1) == 0)
                        rbx_1[i_2] = rbx_1[r11_4 - 1]
                        i_2 = r11_4 - 1
                    
                    if (i_2 s> 0)
                        int64_t r11_6 = sx.q(_mm_bsrli_si128(zmm1_1, 8)[0]) * 0x38
                        int64_t i_1
                        
                        do
                            i_1 = (i_2 - 1) s>> 1
                            int64_t r9_3 = i_1 * 2
                            int64_t rdx_8 = **(rbx_1 + (r9_3 << 3))
                            
                            if ((*(r11_6 + rdx_8 + 0x18))[0] f>=
                                    *(sx.q((*rbx_1)[r9_3 * 2 + 2]) * 0x38 + rdx_8 + 0x18))
                                break
                            
                            rbx_1[i_2] = *(rbx_1 + (r9_3 << 3))
                            i_2 = i_1
                        while (i_1 s> 0)
                    
                    rax_11 = var_258_1
                    r13 = arg3
                    *((i_2 << 4) + rbx_1) = zmm1_1
                
                rdi_2 -= 0x10
                var_1f8:8.q = rdi_2
                int32_t* rsi_5 = sx.q(rax_11) * 0x38 + var_240
                char r10_7 = rsi_5[0xd].b
                
                if (r10_7 != 0)
                label_1415af550:
                    rcx_7 = (rdi_2 - rbx_1) s>> 4
                    
                    if (rcx_7 != 0)
                        rdx_5 += 1
                        r13 = arg3
                        continue
                else
                    void* rcx_13 = *r13
                    zmm6 = rsi_5[5]
                    void* rax_22 = *(rcx_13 + 0x10)
                    zmm11 = *(rcx_13 + 0x28)
                    zmm1_1 = rsi_5[4] ^ zmm2_1
                    float var_190 = (rsi_5[3] ^ zmm2_1)[0]
                    float var_18c_1 = zmm1_1[0]
                    zmm9 = *(rax_22 + 0x14)
                    zmm10 = *(rax_22 + 0x18)
                    zmm9[0] = zmm9[0] f* *(rcx_13 + 0x2c)
                    zmm10.d = zmm10.d f* *(rcx_13 + 0x30)
                    zmm11.d = zmm11.d f* *(rax_22 + 0x10)
                    float var_1c4_1 = zmm9[0]
                    int32_t var_1c0_1 = zmm10.d
                    int32_t var_1c8_1 = zmm11.d
                    float var_188_1 = (zmm6 ^ zmm2_1)[0]
                    float var_140[0x4]
                    float* rax_23
                    float zmm6_1
                    float zmm14_1
                    int32_t zmm15_1
                    rax_23, zmm6_1, zmm7, zmm8, zmm9, zmm10, zmm11, zmm14_1, zmm15_1 =
                        sub_1417092b0(arg4, &var_140, &var_190)
                    uint128_t zmm13_1 = rsi_5[6]
                    uint128_t zmm12_1 = zx.o(*rax_23)
                    int32_t r13_2 = rax_23[2]
                    zmm11.d = zmm11.d f- zmm12_1.d
                    zmm10.d = zmm10.d f- r13_2
                    uint128_t zmm0_2 = _mm_shuffle_ps(zmm12_1, zmm12_1, 0x55)
                    zmm9[0] = zmm9[0] f- zmm0_2.d
                    zmm0_2.d = zmm13_1.d f* 0.00999999978f
                    zmm7[0] = zmm7[0] f* zmm11.d
                    zmm0_2 = __andps_xmmxud_memxud(zmm0_2, data_142d3f770)
                    zmm8[0] = zmm8[0] * zmm9[0]
                    zmm8[0] = zmm8[0] + zmm7[0]
                    int32_t var_180 = zmm11.d
                    float var_17c_1 = zmm9[0]
                    int32_t var_178_1 = zmm10.d
                    zmm8[0] = zmm8[0] + zmm6_1 f* zmm10.d
                    float temp0_7[0x4] = _mm_min_ss(zmm8[0], zmm14_1)
                    zmm14_1 = temp0_7[0]
                    temp0_7[0] = temp0_7[0] f- zmm13_1.d
                    
                    if (__andps_xmmxud_memxud(temp0_7, data_142d3f770)[0] f<= zmm0_2.d)
                        zmm6_2, zmm7_1, zmm8_1, zmm9_1, zmm10_1, zmm11_1, zmm12_2, zmm13_2, 
                            zmm15_2 = sub_14154a2a0(*r14, *r12, rsi_5, var_198, arg6, var_1a8, arg8)
                        rsi_1 = var_250
                        rdi_1 = 0
                        break
                    
                    if (not(zmm14_1 f< zmm13_1.d))
                        int32_t rcx_15 = r14[1].d
                        zmm0_2 = zx.o(*(rsi_5 + 0xc))
                        var_130[0] = *rsi_5
                        var_130[1] = rsi_5[1]
                        var_130[2] = rsi_5[2]
                        var_120_1[1] = rsi_5[5]
                        var_120_1[3] = rsi_5[7]
                        var_110_1[0] = rsi_5[8]
                        var_110_1[1] = rsi_5[9]
                        var_110_1[2] = rsi_5[0xa]
                        var_110_1[3] = rsi_5[0xb]
                        var_100_1.d = rsi_5[0xc]
                        var_120_1[2] = zmm13_1.d
                        var_130[3].q = zmm0_2.q
                        var_100_1:4.b = r10_7
                        int32_t var_218_1 = rcx_15
                        r14[1].d = rcx_15 + 1
                        
                        if (rcx_15 + 1 s> *(r14 + 0xc))
                            sub_14083a7e0(r14)
                            rcx_15 = var_218_1
                        
                        int64_t rcx_17 = *r14
                        int64_t rdx_11 = sx.q(rcx_15) * 3
                        *(rcx_17 + (rdx_11 << 2)) = var_1c8_1.q
                        *(rcx_17 + (rdx_11 << 2) + 8) = var_1c0_1
                        int64_t r14_1 = sx.q(r12[1].d)
                        int32_t rax_37 = (r14_1 + 1).d
                        r12[1].d = rax_37
                        
                        if (rax_37 s> *(r12 + 0xc))
                            sub_14083a7e0(r12)
                        
                        int64_t rax_38 = *r12
                        int64_t rcx_19 = r14_1 * 3
                        var_248_1.d = 0
                        *(rax_38 + (rcx_19 << 2)) = zmm12_1.q
                        *(rax_38 + (rcx_19 << 2) + 8) = r13_2
                        rsi_5[0xd].b = 1
                        
                        if (r15 s< 0 && r15 != 0)
                            sub_1405c5570(&var_250, 0)
                        
                        sub_1415c5980(&var_240, var_258_1, &var_180, &var_250)
                        int64_t r13_3 = sx.q(var_238.d)
                        int32_t r8_11 = var_248_1.d
                        int32_t rax_39 = r8_11 + r13_3.d
                        var_238.d = rax_39
                        
                        if (rax_39 s> var_238:4.d)
                            sub_1407c3b60(&var_240)
                            r8_11 = var_248_1.d
                        
                        if (r8_11 s>= 3)
                            int64_t rsi_6 = 0
                            int64_t rax_40 = sx.q(r8_11)
                            
                            if (r8_11 s<= 0)
                                r14 = arg1
                            else
                                void* rdi_3 = r13_3 * 0x38
                                int32_t rbx_2 = r13_3.d
                                int32_t r11_7 = 1 - r13_3.d
                                int32_t var_1d0_1 = r11_7
                                
                                do
                                    int64_t* rdx_15 = var_250
                                    int64_t rcx_23 = var_240
                                    int64_t r15_1 = sx.q(rdx_15[rsi_6].d)
                                    int64_t r14_2 = sx.q(*(&rdx_15[rsi_6] + 4))
                                    int32_t* r9_5 = rdi_3 + rcx_23
                                    int64_t r10_9 = r15_1 * 0x38 + rcx_23
                                    int32_t r11_8 = rbx_2 + 1
                                    
                                    if (rbx_2 + r11_7 s>= r8_11)
                                        r11_8 = r13_3.d
                                    
                                    int64_t r8_12 = *r12
                                    int32_t rax_45 = rbx_2
                                    
                                    if (rbx_2 s<= r13_3.d)
                                        rax_45 = r8_11 + r13_3.d
                                    
                                    int32_t rax_47 = *(r10_9 + (r14_2 << 2))
                                    int64_t rdx_20 = *arg1
                                    int64_t rax_49 =
                                        sx.q(*(r10_9 + (sx.q((r14_2 + 1).d s% 3) << 2)))
                                    int32_t var_1d8_1 = rax_49.d
                                    int64_t rcx_26 = rax_49 * 3
                                    zmm10.d =
                                        (*(rdx_20 + (rcx_26 << 2))).d f- *(r8_12 + (rcx_26 << 2))
                                    zmm6 = *(rdx_20 + (rcx_26 << 2) + 4)
                                    zmm6[0] = zmm6[0] f- *(r8_12 + (rcx_26 << 2) + 4)
                                    zmm11.d = (*(rdx_20 + (rcx_26 << 2) + 8)).d f-
                                        *(r8_12 + (rcx_26 << 2) + 8)
                                    int64_t rcx_27 = sx.q(rax_47) * 3
                                    zmm2_1 = *(rdx_20 + (rcx_27 << 2) + 8)
                                    zmm2_1[0] = zmm2_1[0] f- *(r8_12 + (rcx_27 << 2) + 8)
                                    float zmm5_1[0x4] = *(rdx_20 + (rcx_27 << 2))
                                    zmm7 = *(rdx_20 + (rdx_11 << 2) + 4)
                                    zmm7[0] = zmm7[0] f- *(r8_12 + (rdx_11 << 2) + 4)
                                    zmm1_1 = *(rdx_20 + (rdx_11 << 2) + 8)
                                    zmm1_1[0] = zmm1_1[0] f- *(r8_12 + (rdx_11 << 2) + 8)
                                    float zmm3_1[0x4] = *(rdx_20 + (rdx_11 << 2))
                                    zmm2_1[0] = zmm2_1[0] f- zmm11.d
                                    zmm3_1[0] = zmm3_1[0] f- *(r8_12 + (rdx_11 << 2))
                                    float zmm4_1 = *(rdx_20 + (rcx_27 << 2) + 4)
                                    zmm7[0] = zmm7[0] - zmm6[0]
                                    zmm5_1[0] = zmm5_1[0] f- *(r8_12 + (rcx_27 << 2))
                                    zmm4_1 = zmm4_1 f- *(r8_12 + (rcx_27 << 2) + 4)
                                    zmm1_1[0] = zmm1_1[0] f- zmm11.d
                                    zmm3_1[0] = zmm3_1[0] f- zmm10.d
                                    zmm7[0] = zmm7[0] * zmm2_1[0]
                                    zmm5_1[0] = zmm5_1[0] f- zmm10.d
                                    zmm4_1 = zmm4_1 - zmm6[0]
                                    zmm3_1[0] = zmm3_1[0] * zmm2_1[0]
                                    zmm1_1[0] = zmm1_1[0] * zmm5_1[0]
                                    zmm1_1[0] = zmm1_1[0] * zmm4_1
                                    zmm3_1[0] = zmm3_1[0] - zmm1_1[0]
                                    zmm7[0] = zmm7[0] * zmm5_1[0]
                                    zmm1_1[0] = zmm1_1[0] - zmm7[0]
                                    zmm3_1[0] = zmm3_1[0] * zmm4_1
                                    zmm7[0] = zmm7[0] - zmm3_1[0]
                                    zmm3_1[0] = zmm3_1[0] * zmm3_1[0]
                                    zmm1_1[0] = zmm1_1[0] * zmm1_1[0]
                                    zmm7[0] = zmm7[0] * zmm7[0]
                                    zmm3_1[0] = zmm3_1[0] + zmm1_1[0]
                                    zmm3_1[0] = zmm3_1[0] + zmm7[0]
                                    
                                    if (zmm3_1[0] == 1f)
                                        goto label_1415af3f9
                                    
                                    float var_230_1
                                    float var_228_1
                                    
                                    if (zmm3_1[0] f>= zmm15_1)
                                        float temp0_10[0x4] = _mm_rsqrt_ss(zmm3_1[0], zmm3_1[0])
                                        zmm2_1 = 0x3f000000
                                        zmm3_1[0] = zmm3_1[0] * 0.5f
                                        temp0_10[0] = temp0_10[0] * temp0_10[0]
                                        zmm3_1[0] = zmm3_1[0] * temp0_10[0]
                                        zmm2_1[0] = 0.5f - zmm3_1[0]
                                        temp0_10[0] = temp0_10[0] * zmm2_1[0]
                                        temp0_10[0] = temp0_10[0] + temp0_10[0]
                                        temp0_10[0] = temp0_10[0] * temp0_10[0]
                                        zmm3_1[0] = zmm3_1[0] * temp0_10[0]
                                        temp0_10[0] = temp0_10[0] * (0.5f - zmm3_1[0])
                                        temp0_10[0] = temp0_10[0] + temp0_10[0]
                                        zmm1_1[0] = zmm1_1[0] * temp0_10[0]
                                        zmm3_1[0] = zmm3_1[0] * temp0_10[0]
                                        zmm7[0] = zmm7[0] * temp0_10[0]
                                    label_1415af3f9:
                                        var_228_1 = zmm7[0]
                                        float var_22c_2 = zmm3_1[0]
                                        var_230_1 = zmm1_1[0]
                                    else
                                        var_230_1 = data_143dbb1f8[0]
                                        var_228_1 = data_143dbb200[0]
                                        float var_22c_1 = data_143dbb1fc[0]
                                    
                                    *(r9_5 + 0xc) = var_230_1.q
                                    r9_5[5] = var_228_1
                                    zmm0_1 = r9_5[4]
                                    zmm2_1 = r9_5[3]
                                    zmm1_1 = r9_5[5]
                                    zmm2_1[0] = zmm2_1[0] * zmm2_1[0]
                                    zmm0_1[0] = zmm0_1[0] * zmm0_1[0]
                                    zmm1_1[0] = zmm1_1[0] * zmm1_1[0]
                                    zmm2_1[0] = zmm2_1[0] + zmm0_1[0]
                                    zmm2_1[0] = zmm2_1[0] + zmm1_1[0]
                                    float rax_52
                                    
                                    if (zmm2_1[0] >= 9.99999975e-05f)
                                        *r9_5 = var_1d8_1
                                        r9_5[1] = rax_47
                                        r9_5[2] = var_218_1
                                        r9_5[8] = rax_45 - 1
                                        rax_52.b = 1
                                        r9_5[7] = r15_1.d
                                        r9_5[9] = r11_8
                                        r9_5[0xa] = r14_2.d
                                        r9_5[0xb] = 2
                                        r9_5[0xc] = 1
                                        zmm6[0] = zmm6[0] f* r9_5[4]
                                        zmm10.d = zmm10.d f* r9_5[3]
                                        zmm11.d = zmm11.d f* r9_5[5]
                                        r9_5[0xd].b = 0
                                        zmm6[0] = zmm6[0] f+ zmm10.d
                                        zmm6[0] = zmm6[0] f+ zmm11.d
                                        r9_5[6] = zmm6[0]
                                    else
                                        rax_52.b = 0
                                    
                                    *(r10_9 + (r14_2 << 2) + 0x1c) = rbx_2
                                    *(r10_9 + (r14_2 << 2) + 0x28) = 0
                                    
                                    if (rax_52.b == 0)
                                        rbx_1 = var_1f8.q
                                        goto label_1415af569
                                    
                                    zmm0_1 = r9_5[6]
                                    r12 = arg2
                                    r14 = arg1
                                    
                                    if (not(zmm0_1[0] f< zmm13_1.d) && not(zmm0_1[0] > zmm14_1))
                                        char rax_56
                                        rax_56, zmm13_1, zmm14_1, zmm15_1 =
                                            sub_141743d50(r9_5, *r14, *r12)
                                        
                                        if (rax_56 != 0)
                                            int32_t var_148_1 = rbx_2
                                            int64_t* var_150 = &var_240
                                            sub_14175ce20(&var_1f8, &var_150)
                                    
                                    r8_11 = var_248_1.d
                                    rbx_2 += 1
                                    r11_7 = var_1d0_1
                                    rdi_3 += 0x38
                                    rsi_6 += 1
                                while (rsi_6 s< rax_40)
                                
                                rdi_2 = var_1f8:8.q
                                rbx_1 = var_1f8.q
                            
                            zmm2_1 = 0x80000000
                            r15 = var_248_1:4.d
                            goto label_1415af550
            
        label_1415af569:
            rsi_1 = var_250
            goto label_1415af5a0
    
    zmm15_2.o = zmm15
    zmm13_2.o = zmm13
    zmm12_2.o = zmm12
    zmm11_1.o = var_a8_1
    zmm10_1.o = var_98_1
    zmm9_1.o = var_88_1
    zmm8_1.o = var_78_1
    zmm7_1.o = var_68_1
    zmm6_2.o = var_58_1
    
    if (rsi_1 != 0)
        sub_140a74f90(rsi_1)
    
    if (rbx_1 != 0)
        float (* rax_61)[0x4] = rbx_1
        
        if (((var_1e8 - rbx_1) & 0xfffffffffffffff0) u>= 0x1000)
            rbx_1 = *(rbx_1 - 8)
            
            if (rax_61 - rbx_1 - 8 u> 0x1f)
                _invalid_parameter_noinfo_noreturn()
                noreturn
        
        j_sub_140a74f90(rbx_1)
else
    *arg5 = 0
    *arg7 = (_mm_unpacklo_ps(zx.o(0), 0)).q
    arg7[1].d = 0
    *arg8 = (_mm_unpacklo_ps(zx.o(0), 0)).q
    arg8[1].d = 0
    rdi_1 = 2
    int32_t var_200_1 = 0
    int32_t var_200_2 = 0

int64_t rcx_36 = var_240

if (rcx_36 != 0)
    sub_140a74f90(rcx_36)

__security_check_cookie(rax_1 ^ &var_298)
return zx.q(rdi_1)
