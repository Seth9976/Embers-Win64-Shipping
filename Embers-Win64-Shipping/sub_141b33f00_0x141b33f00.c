// 函数: sub_141b33f00
// 地址: 0x141b33f00
// 来自: E:\Embers\Embers\Binaries\Win64\Embers-Win64-Shipping.exe

int64_t arg_10 = arg2
int32_t var_a4 = 1
int32_t var_98 = 0xffffffff
int32_t rbx = *(arg1 + 0x30)
void* var_a0 = arg1 + 0x18
int32_t rcx = 0
int32_t var_a8 = 0
int32_t r8 = 0
int64_t var_94 = 0

if (rbx != 0)
    void* rax_1 = *(arg1 + 0x28)
    void* r9_1 = arg1 + 0x18
    
    if (rax_1 != 0)
        r9_1 = rax_1
    
    int32_t temp0_1
    int32_t temp1_1
    temp0_1:temp1_1 = sx.q(rbx - 1)
    int32_t rdx_2 = *r9_1
    
    if (rdx_2 != 0)
    label_141b33fa8:
        int32_t rax_8 = neg.d(rdx_2) & rdx_2
        uint64_t rflags_1
        int32_t temp0_2
        temp0_2, rflags_1 = _bit_scan_reverse(rax_8)
        int32_t var_a4_1 = rax_8
        int32_t rax_9
        
        if (rax_8 == 0)
            rax_9 = 0x20
        else
            rax_9 = 0x1f - temp0_2
        
        var_94.d = r8 - rax_9 + 0x1f
        
        if (r8 - rax_9 + 0x1f s> rbx)
            var_94.d = rbx
    else
        while (true)
            int64_t rdx_3 = sx.q(rcx)
            r8 += 0x20
            rcx += 1
            var_94:4.d = r8
            var_a8 = rcx
            
            if (rdx_3.d s>= (temp1_1 + (temp0_1 & 0x1f)) s>> 5)
                break
            
            rdx_2 = *(r9_1 + (rdx_3 << 2) + 4)
            int32_t var_98_1 = 0xffffffff
            
            if (rdx_2 != 0)
                goto label_141b33fa8
        
        var_94.d = rbx

int32_t rdx_4 = *(arg1 + 0x30)
int128_t var_48 = 0xffffffff
int32_t r15 = 0xffffffff << (rdx_4.b & 0x1f)
int128_t var_58 = var_a8.o
int32_t r8_3 = rdx_4 s>> 5
int32_t r9_3 = rdx_4 & 0xffffffe0
int64_t var_68 = _mm_unpackhi_pd(0xffffffff, 0xffffffff)
int32_t var_98_2 = r15
var_94.d = rdx_4
int128_t var_88 = (arg1 + 8).o
int128_t var_78 = var_58

if (rdx_4 != rbx)
    void* rax_11 = *(arg1 + 0x28)
    void* r10_1 = arg1 + 0x18
    
    if (rax_11 != 0)
        r10_1 = rax_11
    
    int32_t temp2_1
    int32_t temp3_1
    temp2_1:temp3_1 = sx.q(rbx - 1)
    int32_t rdx_8 = *(r10_1 + (sx.q(r8_3) << 2)) & r15
    
    if (rdx_8 != 0)
    label_141b34072:
        int32_t rax_18 = neg.d(rdx_8) & rdx_8
        uint64_t rflags_2
        int32_t temp0_4
        temp0_4, rflags_2 = _bit_scan_reverse(rax_18)
        int32_t r11_1
        
        if (rax_18 == 0)
            r11_1 = 0x20
        else
            r11_1 = 0x1f - temp0_4
        
        var_94.d = r9_3 - r11_1 + 0x1f
        
        if (r9_3 - r11_1 + 0x1f s> rbx)
            var_94.d = rbx
    else
        while (true)
            int64_t rcx_5 = sx.q(r8_3)
            r9_3 += 0x20
            r8_3 += 1
            
            if (rcx_5.d s>= (temp3_1 + (temp2_1 & 0x1f)) s>> 5)
                break
            
            rdx_8 = *(r10_1 + (rcx_5 << 2) + 4)
            var_98_2 = 0xffffffff
            
            if (rdx_8 != 0)
                goto label_141b34072
        
        var_94.d = rbx

while (true)
    int64_t result = sx.q(var_78:0xc.d)
    int64_t rdx_9 = var_88.q
    
    if (result.d == (var_98_2.q u>> 0x20).d && var_78.q == arg1 + 0x18 && rdx_9 == arg1 + 8)
        return result
    
    void* rbx_3 = result * 0x60 + *rdx_9
    int32_t arg_8
    sub_141b1fb40(rbx_3 + 8, &arg_8, &arg_10)
    int64_t rax_20 = sx.q(arg_8)
    void* rax_22
    
    if (rax_20.d != 0xffffffff)
        rax_22 = *(rbx_3 + 8) + rax_20 * 0x18
    
    int32_t arg_18
    
    if (rax_20.d == 0xffffffff || rax_22 == 0)
        arg_18 = 0xffffffff
        int32_t arg_1c = 0
    else
        arg_18.q = *(rax_22 + 8)
    
    int64_t* rax_24 = sub_140d3c6e0(&arg_18)
    
    if (rax_24 != 0)
        int64_t rdx_12 = *rax_24
        (*(rdx_12 + 0x538))(rax_24, rdx_12)
    
    var_78:8.d &= not.d(var_88:0xc.d)
    sub_14059bdd0(&var_88:8)
