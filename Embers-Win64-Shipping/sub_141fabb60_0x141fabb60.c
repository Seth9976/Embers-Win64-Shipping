// 函数: sub_141fabb60
// 地址: 0x141fabb60
// 来自: E:\Embers\Embers\Binaries\Win64\Embers-Win64-Shipping.exe

int64_t r9 = *arg2
(*(r9 + 8))(arg2, sx.q(*(arg1 + 8)) * 0x18, sx.q(*(arg1 + 0xc)) * 0x18, r9)
int64_t r9_1 = *arg2
(*(r9_1 + 8))(arg2, zx.q(*(arg1 + 0x28) + 0x1f) u>> 5 << 2, zx.q(*(arg1 + 0x2c) + 0x1f) u>> 5 << 2, 
    r9_1)
int64_t rdx_7 = sx.q(*(arg1 + 0x48)) << 2
(*(*arg2 + 8))(arg2, rdx_7, rdx_7)
int64_t r9_2 = *arg2
(*(r9_2 + 8))(arg2, sx.q(*(arg1 + 0x58)) * 0xc, sx.q(*(arg1 + 0x5c)) * 0xc, r9_2)
int64_t r9_3 = *arg2
(*(r9_3 + 8))(arg2, zx.q(*(arg1 + 0x78) + 0x1f) u>> 5 << 2, zx.q(*(arg1 + 0x7c) + 0x1f) u>> 5 << 2, 
    r9_3)
int64_t rdx_15 = sx.q(*(arg1 + 0x98)) << 2
(*(*arg2 + 8))(arg2, rdx_15, rdx_15)
(*(*arg2 + 8))(arg2, sx.q(*(arg1 + 0xa8)) << 4, sx.q(*(arg1 + 0xac)) << 4)
int64_t r9_4 = *arg2
(*(r9_4 + 8))(arg2, zx.q(*(arg1 + 0xc8) + 0x1f) u>> 5 << 2, zx.q(*(arg1 + 0xcc) + 0x1f) u>> 5 << 2, 
    r9_4)
*(arg1 + 0xe8)
jump(*(*arg2 + 8))
