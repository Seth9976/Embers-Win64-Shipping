// 函数: sub_1412241a0
// 地址: 0x1412241a0
// 来自: E:\Embers\Embers\Binaries\Win64\Embers-Win64-Shipping.exe

int64_t r12
r12.b = 0
int32_t rax_1 = arg1[1].d - *(arg1 + 0x34)
int64_t rbx = sx.q(arg3)
int32_t rdi

if (rax_1 == 1)
    rdi = arg5
label_1412242c9:
    
    if (sub_140aa4bc0(arg1, arg1[1].d - *(arg1 + 0x34), 0) == 0)
        void* r9_1 = &arg1[7]
        int32_t rax_8 = (arg1[9].d - 1) & rbx.d
        *(arg4 + 0x34) = rax_8
        void* rdx_8 = *(r9_1 + 8)
        int64_t r8_3 = sx.q(rax_8)
        void* rax_9 = r9_1
        
        if (rdx_8 != 0)
            rax_9 = rdx_8
        
        arg4[6].d = *(rax_9 + (((sx.q(arg1[9].d) - 1) & r8_3) << 2))
        void* rax_11 = *(r9_1 + 8)
        
        if (rax_11 != 0)
            r9_1 = rax_11
        
        *(r9_1 + (((sx.q(arg1[9].d) - 1) & r8_3) << 2)) = rdi
else
    int32_t rbx_1
    
    if (rax_1 != 0)
        void* r8 = &arg1[7]
        void* rcx = arg1[8]
        
        if (rcx != 0)
            r8 = rcx
        
        rbx_1 = *(r8 + (((sx.q(arg1[9].d) - 1) & rbx) << 2))
    
    if (rax_1 == 0 || rbx_1 == 0xffffffff)
    label_141224226:
        rbx_1 = -1
    else
        int64_t rdx_3 = *arg1
        
        while (true)
            int64_t* rcx_1 = sx.q(rbx_1) * 0x38
            
            if (*(rcx_1 + rdx_3) == *arg4)
                break
            
            rbx_1 = *(rcx_1 + rdx_3 + 0x30)
            
            if (rbx_1 == 0xffffffff)
                goto label_141224226
    
    r12.b = rbx_1 != 0xffffffff
    
    if (rbx_1 == 0xffffffff)
        rdi = arg5
    else
        int64_t i = 2
        void* rbp_2 = sx.q(rbx_1) * 0x38 + *arg1
        void* rsi_1 = rbp_2 + 0x18
        
        do
            int64_t* rcx_2 = *(rsi_1 - 8)
            rsi_1 -= 8
            i -= 1
            
            if (rcx_2 != 0)
                (*(*rcx_2 + 0x38))(rcx_2)
        while (i != 0)
        
        memmove(rbp_2, arg4, (i + 0x30).d)
        sub_140afe9e0(arg1, arg5, (i + 1).d)
        rdi = rbx_1
    
    if (rbx_1 == 0xffffffff)
        rbx = zx.q(arg3)
        goto label_1412242c9

*arg2 = rdi

if (arg6 != 0)
    *arg6 = r12.b

return arg2
