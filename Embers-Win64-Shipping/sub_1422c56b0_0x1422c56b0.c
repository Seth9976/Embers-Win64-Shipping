// 函数: sub_1422c56b0
// 地址: 0x1422c56b0
// 来自: E:\Embers\Embers\Binaries\Win64\Embers-Win64-Shipping.exe

int32_t r8 = 0
void* rsi_1 = arg2[1] + 0xa8
int32_t var_a8 = 0
int32_t var_a4 = 1
int32_t rcx = 0
int32_t* var_a0 = rsi_1 + 0x10
int64_t* r13 = *(*arg2 + 0x58)
int32_t result_1 = 0
int32_t r11 = *(rsi_1 + 0x28)
int32_t var_98 = 0xffffffff
int32_t var_94 = 0
int32_t var_90 = 0

if (r11 != 0)
    int32_t* rax_1 = *(rsi_1 + 0x20)
    int32_t* r9_1 = rsi_1 + 0x10
    
    if (rax_1 != 0)
        r9_1 = rax_1
    
    int32_t temp0_1
    int32_t temp1_1
    temp0_1:temp1_1 = sx.q(r11 - 1)
    int32_t rdx_2 = *r9_1
    int32_t var_94_2
    
    if (rdx_2 != 0)
    label_1422c5768:
        int32_t rax_8 = neg.d(rdx_2) & rdx_2
        uint64_t rflags_1
        int32_t temp0_2
        temp0_2, rflags_1 = _bit_scan_reverse(rax_8)
        int32_t var_a4_1 = rax_8
        int32_t rax_9
        
        if (rax_8 == 0)
            rax_9 = 0x20
        else
            rax_9 = 0x1f - temp0_2
        
        int32_t var_94_1 = rcx - rax_9 + 0x1f
        
        if (rcx - rax_9 + 0x1f s> r11)
            var_94_2 = r11
    else
        while (true)
            int64_t rdx_3 = sx.q(r8)
            rcx += 0x20
            r8 += 1
            int32_t var_90_1 = rcx
            var_a8 = r8
            
            if (rdx_3.d s>= (temp1_1 + (temp0_1 & 0x1f)) s>> 5)
                break
            
            rdx_2 = r9_1[rdx_3 + 1]
            int32_t var_98_1 = 0xffffffff
            
            if (rdx_2 != 0)
                goto label_1422c5768
        
        var_94_2 = r11

int32_t rdx_5 = *(rsi_1 + 0x28)
int128_t var_48 = 0xffffffff
int32_t r15 = 0xffffffff << (rdx_5.b & 0x1f)
int128_t var_58 = var_a8.o
int32_t r8_2 = rdx_5 s>> 5
int32_t r9_3 = rdx_5 & 0xffffffe0
int64_t var_68 = _mm_unpackhi_pd(0xffffffff, 0xffffffff)
int32_t var_98_2 = r15
int32_t var_94_3 = rdx_5
int128_t var_88 = rsi_1.o
int128_t var_78 = var_58

if (rdx_5 != r11)
    int32_t* rax_11 = *(rsi_1 + 0x20)
    int32_t* r10_1 = rsi_1 + 0x10
    
    if (rax_11 != 0)
        r10_1 = rax_11
    
    int32_t temp2_1
    int32_t temp3_1
    temp2_1:temp3_1 = sx.q(r11 - 1)
    int32_t rdx_9 = r10_1[sx.q(r8_2)] & r15
    int32_t var_94_5
    
    if (rdx_9 != 0)
    label_1422c5832:
        int32_t rax_18 = neg.d(rdx_9) & rdx_9
        uint64_t rflags_2
        int32_t temp0_4
        temp0_4, rflags_2 = _bit_scan_reverse(rax_18)
        int32_t rbx_1
        
        if (rax_18 == 0)
            rbx_1 = 0x20
        else
            rbx_1 = 0x1f - temp0_4
        
        int32_t var_94_4 = r9_3 - rbx_1 + 0x1f
        
        if (r9_3 - rbx_1 + 0x1f s> r11)
            var_94_5 = r11
    else
        while (true)
            int64_t rcx_5 = sx.q(r8_2)
            r9_3 += 0x20
            r8_2 += 1
            
            if (rcx_5.d s>= (temp3_1 + (temp2_1 & 0x1f)) s>> 5)
                break
            
            rdx_9 = r10_1[rcx_5 + 1]
            var_98_2 = 0xffffffff
            
            if (rdx_9 != 0)
                goto label_1422c5832
        
        var_94_5 = r11

while (true)
    int64_t rax_20 = sx.q(var_78:0xc.d)
    int64_t* rdx_10 = var_88.q
    
    if (rax_20.d == (var_98_2.q u>> 0x20).d && var_78.q == rsi_1 + 0x10 && rdx_10 == rsi_1)
        uint64_t result = *arg2
        int32_t* rcx_11 = *(result + 0x60)
        
        if (rcx_11 != 0)
            result = zx.q(result_1)
            *rcx_11 += result.d
        
        return result
    
    sub_1422c58e0(arg1, *rdx_10 + 8 + rax_20 * 0x60, r13, &result_1)
    var_78:8.d &= not.d(var_88:0xc.d)
    sub_14059bdd0(&var_88:8)
