// 函数: sub_141ab82e0
// 地址: 0x141ab82e0
// 来自: E:\Embers\Embers\Binaries\Win64\Embers-Win64-Shipping.exe

int16_t* const rbp = &data_142d40450
int16_t* const r14

if (arg2[4].d == 0)
    r14 = &data_142d40450
else
    r14 = arg2[3]

int64_t arg_8 = *(sub_141ae26e0() + 0x18)
int16_t* var_28
sub_140b63b70(&arg_8, &var_28)
float zmm0[0x2] = *arg2
zmm0[0] = zmm0[0] * 100f
int16_t* const var_48 = r14
int32_t var_20

if (var_20 != 0)
    rbp = var_28

_mm_cvtps_pd(zmm0)
int64_t var_38
sub_140a2e390(&var_38, u"%s<W:%.1f%%> %s", rbp)
int16_t* rcx_3 = var_28

if (rcx_3 != 0)
    sub_140a74f90(rcx_3)

arg2[4].d = 0

if (*(arg2 + 0x24) != 0)
    sub_1405947f0(&arg2[3], 0)

sub_140a20b00(&var_38, U"(", 1)
float zmm0_1[0x2] = *(arg1 + 0x24)
zmm0_1[0] = zmm0_1[0] * 100f
sub_140a2e390(&var_28, u"Alpha: %.1f%%", _mm_cvtps_pd(zmm0_1))
int32_t r8_3

if (var_20 == 0)
    r8_3 = 0
else
    r8_3 = var_20 - 1

sub_140a20ba0(&var_38, var_28, r8_3)
int16_t* rcx_8 = var_28

if (rcx_8 != 0)
    sub_140a74f90(rcx_8)

sub_140a20b00(&var_38, U")\n", 1)
int64_t rbp_1 = var_38
var_28 = nullptr
int32_t var_30
int32_t var_20_1 = var_30

if (var_30 != 0)
    sub_1405a4c70(&var_28, var_30, 0)
    memcpy(var_28, rbp_1, var_30 * 2)
else
    int32_t var_1c_1 = 0

int64_t result = sub_141e14220(arg2, &var_28, 0)

if ((*(arg1 + 0x1bc) & 8) == 0 || (*(arg1 + 0x1d8) & 2) == 0 || *(arg1 + 0x518) s<= 0)
    result = sub_141e21d10(arg1 + 0x10, arg2)

int64_t rcx_14 = var_38

if (rcx_14 == 0)
    return result

return sub_140a74f90(rcx_14)
