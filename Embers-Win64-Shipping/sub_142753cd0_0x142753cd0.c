...
float zmm2[0x4] = *(sx.q(r8) + *(rdi + (sx.q(rdx) << 3)))
...
if (result.d s> 0)
    ...
    uint32_t zmm3[0x4] = _mm_cmpeq_ps(data_143f87750, _mm_and_ps(data_143f87760, temp0_1), 1)
    float temp0_4[0x4] = _mm_div_ps(temp0, temp0_1)
    uint64_t i
    
    do
        result = result_1
        result_1 += 0x10
        *result = _mm_and_ps(temp0_4 ^ data_143f87690, zmm3) ^ data_143f87690
        i = i_1
        i_1 -= 1
    while (i != 1)
