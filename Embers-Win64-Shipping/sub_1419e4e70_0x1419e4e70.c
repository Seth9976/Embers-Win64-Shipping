// 函数: sub_1419e4e70
// 地址: 0x1419e4e70
// 来自: E:\Embers\Embers\Binaries\Win64\Embers-Win64-Shipping.exe

float r8 = 0f
int32_t var_a4 = 1
float var_a8 = 0f
int32_t rbx = *(arg1 + 0xa8)
int32_t r9 = 0
void* var_a0 = arg1 + 0x90
int32_t var_98 = 0xffffffff
int64_t var_94 = 0

if (rbx != 0)
    void* rax_1 = *(arg1 + 0xa0)
    void* r10_1 = arg1 + 0x90
    
    if (rax_1 != 0)
        r10_1 = rax_1
    
    int32_t temp0_1
    int32_t temp1_1
    temp0_1:temp1_1 = sx.q(rbx - 1)
    int32_t rcx = *r10_1
    
    if (rcx != 0)
    label_1419e4f0c:
        int32_t rax_8 = neg.d(rcx) & rcx
        uint64_t rflags_1
        int32_t temp0_2
        temp0_2, rflags_1 = _bit_scan_reverse(rax_8)
        int32_t var_a4_1 = rax_8
        int32_t rax_9
        
        if (rax_8 == 0)
            rax_9 = 0x20
        else
            rax_9 = 0x1f - temp0_2
        
        var_94.d = r9 - rax_9 + 0x1f
        
        if (r9 - rax_9 + 0x1f s> rbx)
            var_94.d = rbx
    else
        while (true)
            int64_t rcx_1 = sx.q(r8)
            r9 += 0x20
            r8 += 1
            var_94:4.d = r9
            var_a8 = r8
            
            if (rcx_1.d s>= (temp1_1 + (temp0_1 & 0x1f)) s>> 5)
                break
            
            rcx = *(r10_1 + (rcx_1 << 2) + 4)
            var_98 = 0xffffffff
            
            if (rcx != 0)
                goto label_1419e4f0c
        
        var_94.d = rbx

int32_t rdx_3 = *(arg1 + 0xa8)
double zmm2[0x2] = var_98.o
double var_48[0x2] = zmm2
int32_t r12 = 0xffffffff << (rdx_3.b & 0x1f)
int128_t var_58 = var_a8.o
int32_t r8_2 = rdx_3 s>> 5
int32_t r9_3 = rdx_3 & 0xffffffe0
int64_t var_68 = (_mm_unpackhi_pd(zmm2, zmm2[0])).q
int32_t var_98_1 = r12
var_94.d = rdx_3
int128_t var_88 = (arg1 + 0x80).o
double var_78[0x2] = var_58

if (rdx_3 != rbx)
    void* rax_11 = *(arg1 + 0xa0)
    void* r10_2 = arg1 + 0x90
    
    if (rax_11 != 0)
        r10_2 = rax_11
    
    int32_t temp2_1
    int32_t temp3_1
    temp2_1:temp3_1 = sx.q(rbx - 1)
    int32_t rdx_7 = *(r10_2 + (sx.q(r8_2) << 2)) & r12
    
    if (rdx_7 != 0)
    label_1419e4fd3:
        int32_t rax_18 = neg.d(rdx_7) & rdx_7
        uint64_t rflags_2
        int32_t temp0_4
        temp0_4, rflags_2 = _bit_scan_reverse(rax_18)
        int32_t r11_1
        
        if (rax_18 == 0)
            r11_1 = 0x20
        else
            r11_1 = 0x1f - temp0_4
        
        var_94.d = r9_3 - r11_1 + 0x1f
        
        if (r9_3 - r11_1 + 0x1f s> rbx)
            var_94.d = rbx
    else
        while (true)
            int64_t rcx_6 = sx.q(r8_2)
            r9_3 += 0x20
            r8_2 += 1
            
            if (rcx_6.d s>= (temp3_1 + (temp2_1 & 0x1f)) s>> 5)
                break
            
            rdx_7 = *(r10_2 + (rcx_6 << 2) + 4)
            var_98_1 = 0xffffffff
            
            if (rdx_7 != 0)
                goto label_1419e4fd3
        
        var_94.d = rbx

uint32_t rdi_1 = (var_98_1.q u>> 0x20).d

while (true)
    int64_t rcx_8 = sx.q(var_78[1]:4.d)
    int64_t* rax_20 = var_88.q
    
    if (rcx_8.d == rdi_1 && var_78[0] == arg1 + 0x90 && rax_20 == arg1 + 0x80)
        bool cond:3 = *(arg1 + 0xdc) == 0
        *(arg1 + 0xd8) = 0
        
        if (not(cond:3))
            sub_14090a730(arg1 + 0xd0, 0)
        
        return sub_1419e19e0(arg1) __tailcall
    
    double (* rbx_1)[0x2] = *(*rax_20 + rcx_8 * 0x10)
    double (* arg_18)[0x2] = rbx_1
    
    if (rbx_1[5][0].b != 0)
        double (* arg_10)[0x2] = rbx_1
        int128_t zmm0 = rbx_1[3][0].d
        double rax_22 = (*rbx_1)[7]
        zmm2 = *rbx_1
        float zmm4 = zmm0.d
        int128_t zmm3 = *(rbx_1 + 4)
        zmm0.d = zmm0.d f* *(rax_22 i+ 0x298)
        var_98_1.b = 1
        float temp0_5 = _mm_max_ss(zmm4 f* *(rax_22 i+ 0x29c), zmm0.d)
        zmm2[0].d = zmm2[0].d f+ temp0_5
        zmm0.d = zmm3.d f+ temp0_5
        zmm2[0].d = zmm2[0].d f- temp0_5
        zmm3.d = zmm3.d f- temp0_5
        var_a0.d = zmm2[0].d
        var_a0:4.d = zmm0.d
        var_a8 = zmm2[0].d
        int32_t var_a4_2 = zmm3.d
        sub_1419e4ce0(arg1 + 0xe8, &arg_10, &var_a8)
        rbx_1[5][0].b = 0
        int32_t arg_8
        sub_140b91690(arg1 + 0x30, &arg_8, &arg_18, nullptr)
    
    var_78[1].d &= not.d(var_88:0xc.d)
    sub_14059bdd0(&var_88:8)
