...
if (0 s< r9)
    ...
    while (true)
        void* rdi_3 = sx.q(rcx_3) * 0xa0 + *rdx_3
...
uint32_t rdx_6 = (*rsi_1 + 0x1f) u>> 5

if (result != 0)
    result_1 = result

if (rdx_6 u> 8)
    result = memset(result_1, 0, zx.q(rdx_6) << 2)
else if (rdx_6 != 0)
    result = nullptr
    __builtin_memset(result_1, 0, zx.q(rdx_6) << 2)
