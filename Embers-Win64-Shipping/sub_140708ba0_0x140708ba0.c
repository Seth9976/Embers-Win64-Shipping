// 函数: sub_140708ba0
// 地址: 0x140708ba0
// 来自: E:\Embers\Embers\Binaries\Win64\Embers-Win64-Shipping.exe

int16_t* rax = *(arg2 + 0x10)
int64_t* r14 = nullptr
int32_t r9 = 0
int32_t* r8 = *(arg2 + 0x20)
int16_t i_5 = *rax & 0x7fff
*(arg2 + 0x10) = &rax[1]
uint32_t i = zx.d(i_5)

if (i s>= *r8)
    int64_t rax_1 = 0
    int32_t* rcx = nullptr
    
    do
        i -= *(rcx + r8)
        rax_1 += 1
        rcx = rax_1 << 2
        r9 += 1
    while (i s>= *(rcx + r8))

int64_t r10 = *(arg2 + 0x18)
int32_t* result_2 = *(*(arg2 + 0x38) + (sx.q(*(sx.q(i) + *(r10 + (sx.q(r9) << 3)))) << 3))
uint32_t rax_5 = zx.d(rax[1])
uint8_t rax_6 = (rax_5 u>> 0xf).b
int32_t i_1 = rax_5 & 0x7fff
*(arg2 + 0x10) = &rax[2]
int32_t* rbx_1

if ((not.b(rax_6) & 1) == 0)
    rbx_1 = zx.q(*(arg2 + 0x80) * i_1) + *(arg2 + 0x70)
else
    int32_t r9_1 = 0
    
    if (i_1 s>= *r8)
        int64_t rax_7 = 0
        int32_t* rdx_1 = nullptr
        
        do
            i_1 -= *(rdx_1 + r8)
            rax_7 += 1
            rdx_1 = rax_7 << 2
            r9_1 += 1
        while (i_1 s>= *(rdx_1 + r8))
    
    rbx_1 = sx.q(i_1) + *(r10 + (sx.q(r9_1) << 3))

uint32_t rax_9 = zx.d(rax[2])
uint32_t rax_10 = rax_9 u>> 0xf
int32_t i_2 = rax_9 & 0x7fff
uint32_t arg_20 = rax_10
rax_10.b = not.b(rax_10.b)
*(arg2 + 0x10) = &rax[3]
float (* r13_1)[0x4]

if ((rax_10.b & 1) == 0)
    r13_1 = zx.q(*(arg2 + 0x80) * i_2) + *(arg2 + 0x70)
else
    int32_t r9_2 = 0
    
    if (i_2 s>= *r8)
        int64_t rax_11 = 0
        int32_t* rdx_2 = nullptr
        
        do
            i_2 -= *(rdx_2 + r8)
            rax_11 += 1
            rdx_2 = rax_11 << 2
            r9_2 += 1
        while (i_2 s>= *(rdx_2 + r8))
    
    r13_1 = sx.q(i_2) + *(r10 + (sx.q(r9_2) << 3))

uint32_t rax_13 = zx.d(rax[3])
uint32_t rax_14 = rax_13 u>> 0xf
int32_t i_3 = rax_13 & 0x7fff
uint32_t arg_18 = rax_14
rax_14.b = not.b(rax_14.b)
*(arg2 + 0x10) = &rax[4]
uint128_t* r12_1

if ((rax_14.b & 1) == 0)
    r12_1 = zx.q(*(arg2 + 0x80) * i_3) + *(arg2 + 0x70)
else
    int32_t r9_3 = 0
    
    if (i_3 s>= *r8)
        int64_t rax_15 = 0
        int32_t* rdx_3 = nullptr
        
        do
            i_3 -= *(rdx_3 + r8)
            rax_15 += 1
            rdx_3 = rax_15 << 2
            r9_3 += 1
        while (i_3 s>= *(rdx_3 + r8))
    
    r12_1 = sx.q(i_3) + *(r10 + (sx.q(r9_3) << 3))

uint32_t rcx_5 = zx.d(rax[4])
*(arg2 + 0x10) = &rax[5]
uint32_t rax_19 = rcx_5 u>> 0xf
int32_t i_4 = rcx_5 & 0x7fff
uint32_t arg_10 = rax_19
rax_19.b = not.b(rax_19.b)
float (* r15_1)[0x4]

if ((rax_19.b & 1) == 0)
    r15_1 = zx.q(*(arg2 + 0x80) * i_4) + *(arg2 + 0x70)
else
    int32_t r9_4 = 0
    
    if (i_4 s>= *r8)
        int64_t rax_20 = 0
        int64_t rdx_4 = 0
        
        do
            i_4 -= *(r8 + rdx_4)
            rax_20 += 1
            rdx_4 = rax_20 << 2
            r9_4 += 1
        while (i_4 s>= *(r8 + rdx_4))
    
    r15_1 = sx.q(i_4) + *(r10 + (sx.q(r9_4) << 3))

uint32_t var_3d8 = 0xffffffff
int128_t* var_3d0 = nullptr
int32_t var_3c8 = 0
sub_140767bd0(&var_3d8, arg2)
void var_2f8
sub_140742680(&var_2f8, arg2)
void* rax_22 = sub_140d3c6e0(result_2)
void* var_438 = rax_22
void* rax_23
int64_t rax_24
void* rdx_7

if (rax_22 != 0)
    rax_23 = sub_142591550()
    rdx_7 = *(rax_22 + 0x10)
    rax_24 = sx.q(*(rax_23 + 0x38))

if (rax_22 == 0 || rax_24.d s> *(rdx_7 + 0x38)
        || *(*(rdx_7 + 0x30) + (rax_24 << 3)) != rax_23 + 0x30)
    var_438 = nullptr

void* var_590 = nullptr
void* rax_26 = *(result_2 + 0x58)

if (rax_26 != 0)
    r14 = *(**(rax_26 + 0x58) + (sx.q(result_2[0x1a]) << 3))
    
    if (result_2[0x86].b != 0)
        void* rax_29 = sub_140d3c6e0(result_2)
        void* rax_31
        void* rcx_12
        int64_t rdx_8
        
        if (rax_29 != 0)
            rax_31 = sub_142591550() + 0x30
            rcx_12 = *(rax_29 + 0x10)
            rdx_8 = sx.q(*(rax_31 + 8))
        
        void* rax_32
        
        if (rax_29 == 0 || rdx_8.d s> *(rcx_12 + 0x38)
                || *(*(rcx_12 + 0x30) + (rdx_8 << 3)) != rax_31 || *(rax_29 + 0x430) == 0)
            rax_32 = &r14[0x27]
        else
            rax_32 = sub_141f66050(rax_29, result_2[0x1a])
        
        var_590 = rax_32

int64_t* rcx_15

if (rax_26 == 0 || r14 == 0)
    rcx_15 = nullptr
else
    rcx_15 = r14[3]

void* rsi_2 = *(result_2 + 0x70)
float zmm9[0x4] = 0x3f800000
int32_t var_528 = 0
int32_t temp0
int32_t temp1
temp0:temp1 = muls.dp.d(0x55555556, (*(*rcx_15 + 0x58))(rcx_15))
float zmm0[0x4] = 0x3f800000
bool cond:2 = *(arg2 + 0x40) s<= 0
uint64_t result = result_2
zmm0[0] = 1f f/ *(result + 0x190)
float var_4d4 = zmm0[0]

if (not(cond:2))
    int128_t* r8_3 = var_3d0
    float zmm10[0x4] = zx.o(0)
    uint128_t zmm11
    uint128_t var_98_1 = zmm11
    uint128_t zmm12
    uint128_t var_a8_1 = zmm12
    result = zx.q(arg_10.b) << 2
    uint64_t r9_6 = zx.q(arg_18.b) << 2
    uint64_t r10_2 = zx.q(arg_20.b) << 2
    uint64_t r11_5 = zx.q(rax_6) << 2
    uint128_t zmm13
    uint128_t var_b8_1 = zmm13
    float zmm14[0x4]
    float var_c8_1[0x4] = zmm14
    uint128_t zmm15
    uint128_t var_d8_1 = zmm15
    uint64_t result_1 = result
    uint64_t var_3a8_1 = r9_6
    uint64_t var_3b8_1 = r10_2
    uint64_t var_378_1 = r11_5
    bool cond:4_1
    
    do
        zmm15 = *r8_3
        float (* rdx_11)[0x4] = r15_1
        uint128_t* rcx_19 = r12_1
        r15_1 += result
        float (* rax_40)[0x4] = r13_1
        r12_1 += r9_6
        r13_1 += r10_2
        uint64_t rdx_12 = zx.q(*rbx_1)
        float var_320_1 = (*rdx_11)[0]
        zmm0 = _mm_unpacklo_ps(*rax_40, (*rcx_19).q)
        int64_t var_444_1 = zmm0.q
        
        if (rdx_12.d s< 0 || rdx_12.d s> (temp0 u>> 0x1f) - 1 + temp0)
            int32_t var_314_1 = 0
            int64_t var_384_1 = (_mm_unpacklo_ps(zmm9, zmm10[0].q)).q
            int32_t var_37c_1 = 0
            zmm0 = 0.o
            rdx_12 = zx.q(zmm0[0])
        
        uint32_t rbx_5 = (rdx_12 * 3).d
        int32_t rax_43 = (*(*rcx_15 + 0x68))(zmm0, zx.q(rbx_5))
        int64_t r8_4 = *rcx_15
        int32_t rax_45 = (*(r8_4 + 0x68))(rcx_15, zx.q(rbx_5 + 1), r8_4)
        int64_t r8_5 = *rcx_15
        int32_t rax_47 = (*(r8_5 + 0x68))(rcx_15, zx.q(rbx_5 + 2), r8_5)
        int64_t var_188
        int64_t* rax_49
        int512_t zmm8_1
        rax_49, zmm8_1 = sub_1423162c0(&var_188, var_438, rax_43, r14, var_590, 
            ((sx.q(*(rsi_2 + 0x34)) + 4) << 4) + rsi_2)
        zmm8_1.o = zx.o(*rax_49)
        int32_t rax_50 = rax_49[1].d
        int64_t var_134
        int64_t* rax_56
        int512_t zmm7_1
        rax_56, zmm7_1 = sub_1423162c0(&var_134, var_438, rax_45, r14, var_590, 
            ((sx.q(*(rsi_2 + 0x34)) + 4) << 4) + rsi_2)
        zmm7_1.o = zx.o(*rax_56)
        int32_t rax_57 = rax_56[1].d
        int64_t var_17c
        int64_t* rax_63
        int512_t zmm6_1
        rax_63, zmm6_1 = sub_1423162c0(&var_17c, var_438, rax_47, r14, var_590, 
            ((sx.q(*(rsi_2 + 0x34)) + 4) << 4) + rsi_2)
        zmm6_1.o = zx.o(*rax_63)
        int32_t rax_64 = rax_63[1].d
        int64_t var_170
        int64_t* rax_71 = sub_1423162c0(&var_170, var_438, rax_43, r14, var_590, 
            (((sx.q(*(rsi_2 + 0x34)) ^ 1) + 4) << 4) + rsi_2)
        zmm12 = zx.o(*rax_71)
        int32_t rax_72 = rax_71[1].d
        uint64_t var_4e8_1 = zmm12.q
        int64_t var_164
        int64_t* rax_79 = sub_1423162c0(&var_164, var_438, rax_45, r14, var_590, 
            (((sx.q(*(rsi_2 + 0x34)) ^ 1) + 4) << 4) + rsi_2)
        zmm13 = zx.o(*rax_79)
        int32_t rax_80 = rax_79[1].d
        uint64_t var_468_1 = zmm13.q
        int64_t var_158
        int64_t* rax_87
        int32_t r9_13
        uint128_t zmm6_2
        uint128_t zmm7_2
        float zmm8_2[0x4]
        rax_87, r9_13, zmm6_2, zmm7_2, zmm8_2 = sub_1423162c0(&var_158, var_438, rax_47, r14, 
            var_590, (((sx.q(*(rsi_2 + 0x34)) ^ 1) + 4) << 4) + rsi_2)
        zmm9 = var_320_1
        zmm11 = var_444_1:4.d
        zmm14 = var_444_1.d
        uint128_t zmm1 = zx.o(*rax_87)
        int32_t rax_88 = rax_87[1].d
        zmm9[0] = zmm9[0] f* zmm1.d
        uint64_t var_410_1 = zmm13.q
        float var_5a8_1 = zmm9[0]
        zmm0 = zmm1.q:4.d
        zmm9[0] = zmm9[0] * zmm0[0]
        float var_598_1 = zmm0[0]
        zmm0 = rax_88
        uint128_t zmm4_1
        zmm4_1.d = zmm9.d f* zmm0[0]
        float var_5a4_1 = zmm0[0]
        zmm0 = var_410_1:4.d
        uint128_t zmm2_1
        zmm2_1.d = zmm11.d f* zmm0[0]
        float var_5a0_1 = zmm0[0]
        zmm0 = rax_80
        uint64_t var_4b8_2 = zmm1.q
        zmm1.d = zmm11.d f* zmm0[0]
        float var_59c_1 = zmm0[0]
        zmm14[0] = zmm14[0] f* zmm12.d
        uint128_t zmm3_1
        zmm3_1.d = zmm11.d f* zmm13.d
        zmm13.d = zmm14.d f* rax_72
        uint64_t var_548_1 = zmm12.q
        zmm14[0] = zmm14[0] f+ zmm3_1.d
        int32_t var_558_1 = zmm8_2.d
        zmm12.d = zmm14.d f* var_548_1:4.d
        zmm13.d = zmm13.d f+ zmm1.d
        zmm1.d = var_5a8_1.d f+ zmm14[0]
        zmm12.d = zmm12.d f+ zmm2_1.d
        zmm0 = _mm_shuffle_ps(zmm8_2, zmm8_2, 0x55)
        zmm13.d = zmm13.d f+ zmm4_1.d
        int64_t var_4c8_1 = zmm0.q
        int32_t var_5a8_2 = zmm1.d
        zmm12.d = zmm12.d f+ zmm9[0]
        arg_18 = rax_57.d
        zmm1 = zmm7_2
        zmm8_2 = rax_64
        uint32_t var_430_1 = zmm7_2.d
        zmm7_2 = zmm6_2
        zmm3_1 = var_430_1
        zmm6_2 = zmm6_2.d
        zmm7_2 = _mm_shuffle_ps(zmm7_2, zmm7_2, 0x55)
        zmm1 = _mm_shuffle_ps(zmm1, zmm1, 0x55)
        uint64_t var_578_1 = zmm7_2.q
        arg_10 = zmm3_1.d
        float var_57c_1 = zmm8_2[0]
        arg_20 = zmm6_2.d
        uint64_t var_458_1 = zmm1.q
        char var_190
        char var_18f
        
        if (var_190 != 0 || var_18f != 0)
            void* var_2e8
            float* rdx_22 = var_2e8
            zmm9[0] = zmm9[0] * zmm8_2[0]
            zmm14[0] = zmm14[0] f* var_558_1
            int32_t var_2f4
            var_2e8 = &rdx_22[sx.q(var_2f4)]
            zmm2_1.d = zmm11.d f* zmm3_1.d
            zmm3_1 = zmm11
            zmm11.d = zmm11.d f* arg_18
            zmm3_1.d = zmm3_1.d f* zmm1.d
            zmm2_1.d = zmm2_1.d f+ zmm14[0]
            zmm4_1.d = zmm9.d f* zmm6_2.d
            zmm14[0] = zmm14[0] f* rax_50
            zmm1.d = zmm14.d f* zx.o(var_4c8_1)[0]
            zmm4_1.d = zmm4_1.d f+ zmm2_1.d
            zmm8_2 = var_57c_1
            zmm11.d = zmm11.d f+ zmm14[0]
            zmm9[0] = zmm9[0] f* zmm7_2.d
            zmm3_1.d = zmm3_1.d f+ zmm1.d
            zmm9[0] = zmm9[0] f+ zmm11.d
            zmm9[0] = zmm9[0] f+ zmm3_1.d
            zmm3_1 = var_5a8_2
            zmm4_1.d = zmm4_1.d f- zmm3_1.d
            zmm0 = zmm3_1
            zmm9[0] = zmm9[0] f- zmm13.d
            zmm9[0] = zmm9[0] f- zmm12.d
            zmm4_1.d = zmm4_1.d f* zmm15.d
            zmm9[0] = zmm9[0] f* zmm15.d
            zmm0[0] = zmm0[0] f+ zmm4_1.d
            zmm9[0] = zmm9[0] f* zmm15.d
            zmm2_1.d = zmm13.d f+ zmm9[0]
            zmm1.d = zmm12.d f+ zmm9[0]
            *rdx_22 = zmm0[0]
            void* var_2d0
            int32_t* rdx_23 = var_2d0
            int32_t var_2dc
            var_2d0 = &rdx_23[sx.q(var_2dc)]
            *rdx_23 = zmm1.d
            void* var_2b8
            int32_t* rdx_24 = var_2b8
            int32_t var_2c4
            var_2b8 = &rdx_24[sx.q(var_2c4)]
            *rdx_24 = zmm2_1.d
            
            if (var_18f != 0)
                void* var_2a0
                float* rdx_25 = var_2a0
                zmm0[0] = zmm0[0] f- zmm3_1.d
                zmm1.d = zmm1.d f- zmm12.d
                zmm3_1 = var_4d4
                zmm2_1.d = zmm2_1.d f- zmm13.d
                zmm0[0] = zmm0[0] f* zmm3_1.d
                int32_t var_2ac
                var_2a0 = &rdx_25[sx.q(var_2ac)]
                zmm1.d = zmm1.d f* zmm3_1.d
                *rdx_25 = zmm0[0]
                void* var_288
                int32_t* rdx_26 = var_288
                zmm2_1.d = zmm2_1.d f* zmm3_1.d
                int32_t var_294
                var_288 = &rdx_26[sx.q(var_294)]
                *rdx_26 = zmm1.d
                void* var_270
                int32_t* rdx_27 = var_270
                int32_t var_27c
                var_270 = &rdx_27[sx.q(var_27c)]
                *rdx_27 = zmm2_1.d
            
            zmm3_1 = arg_10
            zmm1 = zx.o(var_458_1)
            zmm0 = zx.o(var_4c8_1)
        
        void* var_5b8_6
        char var_18e
        float zmm5_1[0x4]
        
        if (var_18e == 0)
            zmm9 = 0x3f800000
        else
            zmm5_1 = var_558_1
            zmm5_1[0] = zmm5_1[0] f- zmm6_2.d
            zmm4_1.d = zmm3_1.d f- arg_20
            zmm2_1.d = rax_50.d f- zmm8_2[0]
            zmm14 = data_143dbb200
            zmm6_2.d = zmm0.d f- zmm7_2.d
            zmm3_1.d = zmm1.d f- zmm7_2.d
            zmm1.d = arg_18.d f- zmm8_2[0]
            zmm8_2 = zmm5_1
            zmm0 = zmm6_2
            zmm6_2.d = zmm6_2.d f* zmm4_1.d
            zmm5_1[0] = zmm5_1[0] f* zmm3_1.d
            zmm0[0] = zmm0[0] f* zmm1.d
            zmm7_2.d = zmm3_1.d f* zmm2_1.d
            zmm6_2.d = zmm6_2.d f- zmm5_1[0]
            zmm8_2[0] = zmm8_2[0] f* zmm1.d
            zmm7_2.d = zmm7_2.d f- zmm0[0]
            zmm0 = zmm4_1
            zmm0[0] = zmm0[0] f* zmm2_1.d
            zmm1.d = zmm6_2.d f* zmm6_2.d
            zmm8_2[0] = zmm8_2[0] - zmm0[0]
            zmm2_1.d = zmm7_2.d f* zmm7_2.d
            zmm8_2[0] = zmm8_2[0] * zmm8_2[0]
            zmm2_1.d = zmm2_1.d f+ zmm8_2[0]
            zmm2_1.d = zmm2_1.d f+ zmm1.d
            float var_568_1
            float var_560_1
            
            if (not(zmm2_1.d f!= 1f))
                var_568_1 = zmm7_2.d
                float var_564_1 = zmm8_2[0]
                var_560_1 = zmm6_2.d
            else if (zmm2_1.d f>= 9.99999994e-09f)
                zmm3_1.d = zmm2_1[0]
                zmm5_1 = _mm_rsqrt_ss(zmm3_1[0], zmm3_1.d)
                zmm3_1.d = zmm3_1.d f* 0.5f
                zmm5_1[0] = zmm5_1[0] * zmm5_1[0]
                zmm1.d = zmm3_1.d f* zmm5_1[0]
                zmm2_1.d = 0.5f f- zmm1.d
                zmm5_1[0] = zmm5_1[0] f* zmm2_1.d
                zmm5_1[0] = zmm5_1[0] + zmm5_1[0]
                zmm1.d = zmm5_1.d f* zmm5_1[0]
                zmm3_1.d = zmm3_1.d f* zmm1.d
                zmm4_1.d = 0.5f f- zmm3_1.d
                zmm5_1[0] = zmm5_1[0] f* zmm4_1.d
                zmm5_1[0] = zmm5_1[0] + zmm5_1[0]
                float var_5a8_3 = zmm5_1[0]
                zmm5_1[0] = zmm5_1[0] f* zmm7_2.d
                zmm1.d = zmm5_1.d f* zmm8_2[0]
                zmm5_1[0] = zmm5_1[0] f* zmm6_2.d
                var_568_1 = zmm5_1[0]
                int32_t var_564_3 = zmm1.d
                var_560_1 = zmm5_1[0]
            else
                var_568_1 = data_143dbb1f8[0]
                float var_564_2 = data_143dbb1fc[0]
                var_560_1 = zmm14[0]
            
            zmm6_2.d = var_548_1:4.d.d f- var_598_1
            zmm3_1.d = var_5a0_1.d f- var_598_1
            zmm2_1.d = rax_72.d f- var_5a4_1
            zmm1.d = var_59c_1.d f- var_5a4_1
            zmm5_1 = zx.o(var_4e8_1)
            zmm13 = zx.o(var_4b8_2)
            zmm5_1[0] = zmm5_1[0] f- zmm13.d
            zmm4_1.d = var_468_1.d f- zmm13.d
            int64_t var_500 = var_568_1.q
            zmm0 = zmm6_2
            zmm0[0] = zmm0[0] f* zmm1.d
            float var_4f8_1 = var_560_1
            zmm7_2.d = zmm3_1.d f* zmm2_1.d
            zmm8_2 = zmm5_1
            zmm8_2[0] = zmm8_2[0] f* zmm1.d
            zmm7_2.d = zmm7_2.d f- zmm0[0]
            zmm6_2.d = zmm6_2.d f* zmm4_1.d
            zmm0 = zmm4_1
            zmm5_1[0] = zmm5_1[0] f* zmm3_1.d
            zmm0[0] = zmm0[0] f* zmm2_1.d
            zmm6_2.d = zmm6_2.d f- zmm5_1[0]
            zmm2_1.d = zmm7_2.d f* zmm7_2.d
            zmm8_2[0] = zmm8_2[0] - zmm0[0]
            zmm1.d = zmm6_2.d f* zmm6_2.d
            zmm8_2[0] = zmm8_2[0] * zmm8_2[0]
            zmm2_1.d = zmm2_1.d f+ zmm8_2[0]
            zmm2_1.d = zmm2_1.d f+ zmm1.d
            float var_538
            
            if (not(zmm2_1.d f!= 1f))
                var_538 = zmm7_2.d
                float var_534_1 = zmm8_2[0]
                int32_t var_530_1 = zmm6_2.d
            else if (zmm2_1.d f>= 9.99999994e-09f)
                zmm0 = zmm2_1
                zmm3_1 = zmm0
                zmm5_1 = _mm_rsqrt_ss(zmm0[0], zmm3_1.d)
                zmm3_1.d = zmm3_1.d f* 0.5f
                zmm5_1[0] = zmm5_1[0] * zmm5_1[0]
                zmm1.d = zmm3_1.d f* zmm5_1[0]
                zmm2_1.d = 0.5f f- zmm1.d
                zmm5_1[0] = zmm5_1[0] f* zmm2_1.d
                zmm5_1[0] = zmm5_1[0] + zmm5_1[0]
                zmm1.d = zmm5_1.d f* zmm5_1[0]
                zmm3_1.d = zmm3_1.d f* zmm1.d
                zmm4_1.d = 0.5f f- zmm3_1.d
                zmm5_1[0] = zmm5_1[0] f* zmm4_1.d
                zmm5_1[0] = zmm5_1[0] + zmm5_1[0]
                float var_5a8_4 = zmm5_1[0]
                zmm5_1[0] = zmm5_1[0] f* zmm7_2.d
                zmm1.d = zmm5_1.d f* zmm8_2[0]
                zmm5_1[0] = zmm5_1[0] f* zmm6_2.d
                var_538 = zmm5_1[0]
                int32_t var_534_3 = zmm1.d
                float var_530_3 = zmm5_1[0]
            else
                var_538 = data_143dbb1f8[0]
                float var_534_2 = data_143dbb1fc[0]
                float var_530_2 = zmm14[0]
            
            var_5b8_6.d = 0x3f800000
            int32_t var_14c
            int32_t* rax_96
            rax_96, r9_13, zmm9, zmm10 = sub_140ae34d0(&var_14c, &var_538, &var_500, r9_13)
            void* var_258
            float* rdx_29 = var_258
            zmm7_2 = zx.o(var_578_1)
            zmm6_2 = arg_20
            var_500 = *rax_96
            zmm1 = var_500:4.d
            zmm2_1 = rax_96[2]
            int32_t var_264
            var_258 = &rdx_29[sx.q(var_264)]
            *rdx_29 = var_500.d[0]
            void* var_240
            int32_t* rdx_30 = var_240
            int32_t var_24c
            var_240 = &rdx_30[sx.q(var_24c)]
            *rdx_30 = zmm1.d
            void* var_228
            int32_t* rdx_31 = var_228
            int32_t var_234
            var_228 = &rdx_31[sx.q(var_234)]
            *rdx_31 = zmm2_1.d
        
        char var_18c
        char rbx_7 = var_18c
        char var_18d
        
        if (var_18d != 0 || rbx_7 != 0)
            int64_t rdx_32 = r14[0x13]
            uint64_t rcx_45 = zx.q(r14[0x15].d)
            zmm14 = arg_10
            zmm14[0] = zmm14[0] f- var_558_1
            zmm6_2.d = zmm6_2.d f- var_558_1
            zmm8_2 = zx.o(var_4c8_1)
            zmm7_2.d = zmm7_2.d f- zmm8_2[0]
            zmm12.d = var_57c_1.d f- rax_50
            int64_t rax_102 = zx.q(rax_43) * rcx_45
            arg_10 = zmm14[0]
            zmm11.d = var_458_1.d f- zmm8_2[0]
            zmm14 = arg_18
            zmm14[0] = zmm14[0] f- rax_50
            int32_t var_49c_1 = 0
            int32_t var_48c_1 = 0
            arg_18 = zmm14[0]
            zmm4_1 = *(rdx_32 + (rax_102 << 3))
            zmm5_1 = *(rdx_32 + (rax_102 << 3) + 4)
            int64_t rax_104 = zx.q(rax_45) * rcx_45
            int32_t var_348_1 = zmm4_1.d
            float var_344_1 = zmm5_1[0]
            uint128_t var_338_1 = data_142d3f660
            int32_t var_47c_1 = 0
            int64_t var_360_1 = 0
            int64_t var_350_1 = 0
            int64_t var_340_1 = 0x3f800000
            zmm2_1.d = (*(rdx_32 + (rax_104 << 3))).d f- zmm4_1.d
            zmm3_1.d = (*(rdx_32 + (rax_104 << 3) + 4)).d f- zmm5_1[0]
            int64_t rax_106 = zx.q(rax_47) * rcx_45
            int32_t var_368 = zmm2_1.d
            int32_t var_364_1 = zmm3_1.d
            zmm0 = *(rdx_32 + (rax_106 << 3))
            zmm1 = *(rdx_32 + (rax_106 << 3) + 4)
            zmm0[0] = zmm0[0] f- zmm4_1.d
            zmm1.d = zmm1.d f- zmm5_1[0]
            float var_358_1 = zmm0[0]
            int32_t var_354_1 = zmm1.d
            uint32_t var_118[0x4][0x4]
            uint32_t (* rax_107)[0x4]
            float zmm6_3[0x4]
            uint32_t zmm7_3[0x4]
            float zmm8_3[0x4]
            float zmm11_1
            float zmm12_1[0x4]
            float zmm13_1
            rax_107, zmm6_3, zmm7_3, zmm8_3, zmm11_1, zmm12_1, zmm13_1 =
                sub_140631b10(&var_368, &var_118)
            float var_498[0x4]
            var_498[0] = zmm6_2.d[0]
            uint32_t zmm2_2[0x4] = *rax_107
            float var_4a8[0x4]
            var_4a8[0] = arg_10[0]
            float zmm5_2[0x4] = _mm_shuffle_ps(var_498, var_498, 0xe1)
            float zmm4_2[0x4] = _mm_shuffle_ps(var_4a8, var_4a8, 0xe1)
            zmm5_2[0] = zmm7_3[0]
            zmm4_2[0] = zmm11_1
            zmm5_2 = _mm_shuffle_ps(zmm5_2, zmm5_2, 0xc6)
            zmm4_2 = _mm_shuffle_ps(zmm4_2, zmm4_2, 0xc6)
            zmm5_2[0] = zmm12_1[0]
            zmm4_2[0] = arg_18[0]
            zmm5_2 = _mm_shuffle_ps(zmm5_2, zmm5_2, 0xc9)
            zmm4_2 = _mm_shuffle_ps(zmm4_2, zmm4_2, 0xc9)
            uint32_t zmm14_1[0x4] = _mm_shuffle_ps(zmm2_2, zmm2_2, 0x55)
            float zmm0_1[0x4] = _mm_mul_ps(_mm_shuffle_ps(zmm2_2, zmm2_2, 0), zmm4_2)
            zmm14_1 = _mm_mul_ps(zmm14_1, zmm5_2)
            float zmm1_1[0x4] = _mm_shuffle_ps(zmm2_2, zmm2_2, 0xaa)
            zmm14_1 = _mm_add_ps(zmm14_1, zmm0_1)
            float var_488[0x4]
            var_488[0] = var_558_1[0]
            zmm2_2 = _mm_mul_ps(_mm_shuffle_ps(zmm2_2, zmm2_2, 0xff), zmm6_3)
            int32_t var_49c_2 = 0
            float zmm3_2[0x4] = _mm_shuffle_ps(var_488, var_488, 0xe1)
            zmm3_2[0] = zmm8_3[0]
            int32_t var_48c_2 = 0
            zmm3_2 = _mm_shuffle_ps(zmm3_2, zmm3_2, 0xc6)
            zmm3_2[0] = rax_50[0]
            zmm3_2 = _mm_shuffle_ps(zmm3_2, zmm3_2, 0xc9)
            int32_t var_47c_2 = 0
            _mm_add_ps(_mm_add_ps(zmm14_1, _mm_mul_ps(zmm1_1, zmm3_2)), zmm2_2)
            zmm2_2 = rax_107[1]
            zmm0_1 = _mm_shuffle_ps(zmm2_2, zmm2_2, 0)
            zmm1_1 = _mm_shuffle_ps(zmm2_2, zmm2_2, 0xaa)
            zmm0_1 = _mm_mul_ps(zmm0_1, zmm4_2)
            zmm7_3 = _mm_shuffle_ps(zmm2_2, zmm2_2, 0x55)
            zmm1_1 = _mm_mul_ps(zmm1_1, zmm3_2)
            zmm2_2 = _mm_mul_ps(_mm_shuffle_ps(zmm2_2, zmm2_2, 0xff), zmm6_3)
            zmm7_3 = _mm_add_ps(_mm_add_ps(_mm_add_ps(_mm_mul_ps(zmm7_3, zmm5_2), zmm0_1), zmm1_1), 
                zmm2_2)
            zmm2_2 = rax_107[2]
            zmm0_1 = _mm_mul_ps(_mm_shuffle_ps(zmm2_2, zmm2_2, 0), zmm4_2)
            zmm1_1 = _mm_shuffle_ps(zmm2_2, zmm2_2, 0xaa)
            zmm8_3 = _mm_shuffle_ps(zmm2_2, zmm2_2, 0x55)
            zmm1_1 = _mm_mul_ps(zmm1_1, zmm3_2)
            zmm2_2 = _mm_mul_ps(_mm_shuffle_ps(zmm2_2, zmm2_2, 0xff), zmm6_3)
            var_4c8_1.o = zmm7_3
            zmm8_3 = _mm_add_ps(_mm_add_ps(_mm_add_ps(_mm_mul_ps(zmm8_3, zmm5_2), zmm0_1), zmm1_1), 
                zmm2_2)
            zmm2_2 = rax_107[3]
            zmm0_1 = _mm_mul_ps(_mm_shuffle_ps(zmm2_2, zmm2_2, 0), zmm4_2)
            zmm12_1 = _mm_shuffle_ps(zmm2_2, zmm2_2, 0x55)
            zmm1_1 = _mm_shuffle_ps(zmm2_2, zmm2_2, 0xaa)
            zmm12_1 = _mm_mul_ps(zmm12_1, zmm5_2)
            zmm1_1 = _mm_mul_ps(zmm1_1, zmm3_2)
            zmm2_2 = _mm_shuffle_ps(zmm2_2, zmm2_2, 0xff)
            zmm12_1 = _mm_add_ps(zmm12_1, zmm0_1)
            zmm2_2 = _mm_mul_ps(zmm2_2, zmm6_3)
            zmm0_1 = var_5a4_1
            zmm0_1[0] = zmm0_1[0] f- rax_72
            var_458_1.o = zmm8_3
            zmm12_1 = _mm_add_ps(zmm12_1, zmm1_1)
            zmm1_1 = zx.o(var_4e8_1)
            zmm1_1[0]
            float var_5a4_2 = zmm0_1[0]
            zmm0_1 = zx.o(var_468_1)
            zmm0_1[0] = zmm0_1[0] - zmm1_1[0]
            zmm12_1 = _mm_add_ps(zmm12_1, zmm2_2)
            int64_t var_468_2 = zmm0_1.q
            zmm0_1 = var_5a0_1
            zmm0_1[0] = zmm0_1[0] f- var_548_1:4.d
            var_4b8_2.o = zmm12_1
            float var_5a0_2 = zmm0_1[0]
            zmm0_1 = var_59c_1
            zmm0_1[0] = zmm0_1[0] f- rax_72
            float var_59c_2 = zmm0_1[0]
            uint32_t (* rax_108)[0x4]
            float zmm6_4[0x4]
            float zmm7_4[0x4]
            float zmm8_4[0x4]
            rax_108, zmm6_4, zmm7_4, zmm8_4, zmm9, zmm10, zmm11, zmm12, zmm13, zmm14, zmm15 =
                sub_140631b10(&var_368, &var_118)
            zmm5_2[0] = zmm13.d
            float zmm2_3[0x4] = *rax_108
            zmm4_2[0] = var_468_2.d
            var_578_1.o = _mm_shuffle_ps(zmm2_3, zmm2_3, 0x55)
            float zmm5_3[0x4] = _mm_shuffle_ps(zmm5_2, zmm5_2, 0xe1)
            zmm5_3[0] = zmm11.d
            float zmm4_3[0x4] = _mm_shuffle_ps(zmm4_2, zmm4_2, 0xe1)
            zmm4_3[0] = var_5a0_2.d
            zmm5_3 = _mm_shuffle_ps(zmm5_3, zmm5_3, 0xc6)
            zmm5_3[0] = var_5a4_2[0]
            zmm5_3 = _mm_shuffle_ps(zmm5_3, zmm5_3, 0xc9)
            zmm0 = _mm_shuffle_ps(zmm2_3, zmm2_3, 0)
            zmm13 = _mm_mul_ps(var_578_1.o, zmm5_3)
            zmm4_3 = _mm_shuffle_ps(zmm4_3, zmm4_3, 0xc6)
            zmm4_3[0] = var_59c_2.d
            var_498 = zmm5_3
            zmm4_3 = _mm_shuffle_ps(zmm4_3, zmm4_3, 0xc9)
            zmm1 = _mm_shuffle_ps(zmm2_3, zmm2_3, 0xaa)
            zmm0 = _mm_mul_ps(zmm0, zmm4_3)
            zmm2_3 = _mm_mul_ps(_mm_shuffle_ps(zmm2_3, zmm2_3, 0xff), zmm6_4)
            zmm13 = _mm_add_ps(zmm13, zmm0)
            var_4a8 = zmm4_3
            zmm3_2[0] = zx.o(var_4e8_1)[0]
            float zmm3_3[0x4] = _mm_shuffle_ps(zmm3_2, zmm3_2, 0xe1)
            zmm3_3[0] = var_548_1:4.d[0]
            zmm3_3 = _mm_shuffle_ps(zmm3_3, zmm3_3, 0xc6)
            zmm3_3[0] = rax_72[0]
            zmm3_3 = _mm_shuffle_ps(zmm3_3, zmm3_3, 0xc9)
            var_488 = zmm3_3
            zmm13 = _mm_add_ps(_mm_add_ps(zmm13, _mm_mul_ps(zmm1, zmm3_3)), zmm2_3)
            zmm2_3 = rax_108[1]
            zmm11 = _mm_shuffle_ps(zmm2_3, zmm2_3, 0x55)
            zmm0 = _mm_shuffle_ps(zmm2_3, zmm2_3, 0)
            zmm11 = _mm_mul_ps(zmm11, zmm5_3)
            zmm1 = _mm_shuffle_ps(zmm2_3, zmm2_3, 0xaa)
            zmm0 = _mm_mul_ps(zmm0, zmm4_3)
            zmm1 = _mm_mul_ps(zmm1, zmm3_3)
            zmm2_3 = _mm_shuffle_ps(zmm2_3, zmm2_3, 0xff)
            zmm11 = _mm_add_ps(zmm11, zmm0)
            zmm2_3 = _mm_mul_ps(zmm2_3, zmm6_4)
            zmm11 = _mm_add_ps(_mm_add_ps(zmm11, zmm1), zmm2_3)
            zmm2_3 = rax_108[2]
            zmm0 = _mm_shuffle_ps(zmm2_3, zmm2_3, 0)
            zmm1 = _mm_shuffle_ps(zmm2_3, zmm2_3, 0xaa)
            zmm0 = _mm_mul_ps(zmm0, zmm4_3)
            var_3d8.o = zmm11
            zmm11 = _mm_mul_ps(_mm_shuffle_ps(zmm2_3, zmm2_3, 0x55), zmm5_3)
            zmm1 = _mm_mul_ps(zmm1, zmm3_3)
            zmm2_3 = _mm_shuffle_ps(zmm2_3, zmm2_3, 0xff)
            zmm11 = _mm_add_ps(zmm11, zmm0)
            zmm2_3 = _mm_mul_ps(zmm2_3, zmm6_4)
            zmm11 = _mm_add_ps(_mm_add_ps(zmm11, zmm1), zmm2_3)
            zmm2_3 = rax_108[3]
            var_468_2.o = zmm11
            zmm0 = _mm_shuffle_ps(zmm2_3, zmm2_3, 0)
            zmm11 = _mm_mul_ps(_mm_shuffle_ps(zmm2_3, zmm2_3, 0x55), zmm5_3)
            zmm1 = _mm_shuffle_ps(zmm2_3, zmm2_3, 0xaa)
            zmm0 = _mm_mul_ps(zmm0, zmm4_3)
            zmm1 = _mm_mul_ps(zmm1, zmm3_3)
            zmm2_3 = _mm_shuffle_ps(zmm2_3, zmm2_3, 0xff)
            zmm11 = _mm_add_ps(zmm11, zmm0)
            zmm2_3 = _mm_mul_ps(zmm2_3, zmm6_4)
            var_4e8_1.o = _mm_add_ps(_mm_add_ps(zmm11, zmm1), zmm2_3)
            
            if (var_18d != 0)
                zmm2_3 = data_142d4cc00
                int64_t var_578_2 = 0
                zmm6_4 = _mm_shuffle_ps(zmm2_3, zmm2_3, 0xff)
                zmm0 = _mm_shuffle_ps(zmm2_3, zmm2_3, 0xaa)
                zmm1 = _mm_shuffle_ps(zmm2_3, zmm2_3, 0x55)
                zmm0 = _mm_mul_ps(zmm0, zmm8_4)
                int32_t var_570_1 = 0
                zmm1 = _mm_mul_ps(zmm1, zmm7_4)
                zmm6_4 = _mm_mul_ps(zmm6_4, zmm12)
                zmm2_3 = _mm_mul_ps(_mm_shuffle_ps(zmm2_3, zmm2_3, 0), zmm14)
                zmm6_4 = _mm_add_ps(_mm_add_ps(zmm6_4, zmm0), _mm_add_ps(zmm1, zmm2_3))
                zmm6_4[0] = zmm6_4[0] * zmm6_4[0]
                zmm7_4 = _mm_shuffle_ps(zmm6_4, zmm6_4, 0x55)
                zmm8_4 = _mm_shuffle_ps(zmm6_4, zmm6_4, 0xaa)
                zmm1.d = zmm7_4.d f* zmm7_4[0]
                zmm1.d = zmm1.d f+ zmm6_4[0]
                zmm8_4[0] = zmm8_4[0] * zmm8_4[0]
                zmm1.d = zmm1.d f+ zmm8_4[0]
                
                if (zmm1.d f<= 9.99999994e-09f)
                    zmm6_4 = zmm10
                    zmm7_4 = zmm10
                    zmm8_4 = zmm10
                else
                    zmm3_3 = zx.o(0)
                    zmm4_3 = 0x3f000000
                    zmm3_3[0] = zmm1[0]
                    zmm2_3 = 0x3f000000
                    zmm5_3 = _mm_rsqrt_ss(zmm3_3[0], zmm3_3[0])
                    zmm3_3[0] = zmm3_3[0] * 0.5f
                    zmm5_3[0] = zmm5_3[0] * zmm5_3[0]
                    zmm1.d = zmm3_3.d f* zmm5_3[0]
                    zmm2_3[0] = 0.5f f- zmm1.d
                    zmm5_3[0] = zmm5_3[0] * zmm2_3[0]
                    zmm5_3[0] = zmm5_3[0] + zmm5_3[0]
                    zmm1.d = zmm5_3.d f* zmm5_3[0]
                    zmm3_3[0] = zmm3_3[0] f* zmm1.d
                    zmm4_3[0] = 0.5f - zmm3_3[0]
                    zmm5_3[0] = zmm5_3[0] * zmm4_3[0]
                    zmm5_3[0] = zmm5_3[0] + zmm5_3[0]
                    arg_10 = zmm5_3[0]
                    zmm6_4[0] = zmm6_4[0] * zmm5_3[0]
                    zmm7_4[0] = zmm7_4[0] * zmm5_3[0]
                    zmm8_4[0] = zmm8_4[0] * zmm5_3[0]
                
                zmm2_3 = data_142d4cc00
                zmm0 = _mm_unpacklo_ps(zmm6_4, zmm7_4[0].q)
                zmm3_3 = __mulps_xmmps_memps(_mm_shuffle_ps(zmm2_3, zmm2_3, 0xff), var_4e8_1.o)
                int64_t var_578_3 = 0
                zmm1 = __mulps_xmmps_memps(_mm_shuffle_ps(zmm2_3, zmm2_3, 0x55), var_3d8.o)
                int32_t var_570_2 = 0
                int64_t var_520 = zmm0.q
                zmm0 = __mulps_xmmps_memps(_mm_shuffle_ps(zmm2_3, zmm2_3, 0xaa), var_468_2.o)
                zmm2_3 = _mm_mul_ps(_mm_shuffle_ps(zmm2_3, zmm2_3, 0), zmm13)
                zmm3_3 = _mm_add_ps(zmm3_3, zmm0)
                float var_518_1 = zmm8_4[0]
                zmm3_3 = _mm_add_ps(zmm3_3, _mm_add_ps(zmm1, zmm2_3))
                zmm0 = zmm3_3
                zmm1 = _mm_shuffle_ps(zmm3_3, zmm3_3, 0x55)
                zmm0[0] = zmm0[0] * zmm3_3[0]
                zmm1.d = zmm1.d f* zmm1.d
                zmm3_3 = _mm_shuffle_ps(zmm3_3, zmm3_3, 0xaa)
                zmm3_3[0] = zmm3_3[0] * zmm3_3[0]
                zmm1.d = zmm1.d f+ zmm0[0]
                zmm1.d = zmm1.d f+ zmm3_3[0]
                
                if (not(zmm1.d f<= 9.99999994e-09f))
                    zmm3_3 = zx.o(0)
                    zmm5_3 = 0x3f000000
                    zmm3_3[0] = zmm1[0]
                    zmm2_3 = 0x3f000000
                    zmm4_3 = _mm_rsqrt_ss(zmm3_3[0], zmm3_3[0])
                    zmm3_3[0] = zmm3_3[0] * 0.5f
                    zmm4_3[0] = zmm4_3[0] * zmm4_3[0]
                    zmm1.d = zmm3_3.d f* zmm4_3[0]
                    zmm2_3[0] = 0.5f f- zmm1.d
                    zmm4_3[0] = zmm4_3[0] * zmm2_3[0]
                    zmm4_3[0] = zmm4_3[0] + zmm4_3[0]
                    zmm1.d = zmm4_3.d f* zmm4_3[0]
                    zmm3_3[0] = zmm3_3[0] f* zmm1.d
                    zmm5_3[0] = 0.5f - zmm3_3[0]
                    zmm4_3[0] = zmm4_3[0] * zmm5_3[0]
                    zmm4_3[0] = zmm4_3[0] + zmm4_3[0]
                    arg_10 = zmm4_3[0]
                
                var_5b8_6.d = zmm9[0]
                int32_t var_140
                int32_t* rax_110
                rax_110, r9_13, zmm9, zmm10, zmm14 =
                    sub_140ae34d0(&var_140, &var_520, &var_520, r9_13)
                void* var_210
                float* rdx_36 = var_210
                zmm7_4 = var_4c8_1.o
                zmm8_4 = var_458_1.o
                zmm12 = var_4b8_2.o
                var_520 = *rax_110
                zmm1 = var_520:4.d
                zmm2_3 = rax_110[2]
                int32_t var_21c
                var_210 = &rdx_36[sx.q(var_21c)]
                *rdx_36 = var_520.d[0]
                void* var_1f8
                int32_t* rdx_37 = var_1f8
                int32_t var_204
                var_1f8 = &rdx_37[sx.q(var_204)]
                *rdx_37 = zmm1.d
                void* var_1e0
                float* rdx_38 = var_1e0
                int32_t var_1ec
                var_1e0 = &rdx_38[sx.q(var_1ec)]
                *rdx_38 = zmm2_3[0]
                rbx_7 = var_18c
            
            if (rbx_7 != 0)
                zmm2_3 = data_142d4cc20
                int64_t var_578_4 = 0
                zmm6_4 = _mm_shuffle_ps(zmm2_3, zmm2_3, 0xff)
                zmm0 = _mm_shuffle_ps(zmm2_3, zmm2_3, 0xaa)
                zmm1 = _mm_shuffle_ps(zmm2_3, zmm2_3, 0x55)
                zmm0 = _mm_mul_ps(zmm0, zmm8_4)
                int32_t var_570_3 = 0
                zmm1 = _mm_mul_ps(zmm1, zmm7_4)
                zmm6_4 = _mm_mul_ps(zmm6_4, zmm12)
                zmm2_3 = _mm_mul_ps(_mm_shuffle_ps(zmm2_3, zmm2_3, 0), zmm14)
                zmm6_4 = _mm_add_ps(_mm_add_ps(zmm6_4, zmm0), _mm_add_ps(zmm1, zmm2_3))
                zmm6_4[0] = zmm6_4[0] * zmm6_4[0]
                zmm7_4 = _mm_shuffle_ps(zmm6_4, zmm6_4, 0x55)
                zmm8_4 = _mm_shuffle_ps(zmm6_4, zmm6_4, 0xaa)
                zmm1.d = zmm7_4.d f* zmm7_4[0]
                zmm1.d = zmm1.d f+ zmm6_4[0]
                zmm8_4[0] = zmm8_4[0] * zmm8_4[0]
                zmm1.d = zmm1.d f+ zmm8_4[0]
                
                if (zmm1.d f<= 9.99999994e-09f)
                    zmm6_4 = zmm10
                    zmm7_4 = zmm10
                    zmm8_4 = zmm10
                else
                    zmm3_3 = zx.o(0)
                    zmm4_3 = 0x3f000000
                    zmm3_3[0] = zmm1[0]
                    zmm2_3 = 0x3f000000
                    zmm5_3 = _mm_rsqrt_ss(zmm3_3[0], zmm3_3[0])
                    zmm3_3[0] = zmm3_3[0] * 0.5f
                    zmm5_3[0] = zmm5_3[0] * zmm5_3[0]
                    zmm1.d = zmm3_3.d f* zmm5_3[0]
                    zmm2_3[0] = 0.5f f- zmm1.d
                    zmm5_3[0] = zmm5_3[0] * zmm2_3[0]
                    zmm5_3[0] = zmm5_3[0] + zmm5_3[0]
                    zmm1.d = zmm5_3.d f* zmm5_3[0]
                    zmm3_3[0] = zmm3_3[0] f* zmm1.d
                    zmm4_3[0] = 0.5f - zmm3_3[0]
                    zmm5_3[0] = zmm5_3[0] * zmm4_3[0]
                    zmm5_3[0] = zmm5_3[0] + zmm5_3[0]
                    arg_10 = zmm5_3[0]
                    zmm6_4[0] = zmm6_4[0] * zmm5_3[0]
                    zmm7_4[0] = zmm7_4[0] * zmm5_3[0]
                    zmm8_4[0] = zmm8_4[0] * zmm5_3[0]
                
                zmm2_3 = data_142d4cc20
                int64_t var_510 = (_mm_unpacklo_ps(zmm6_4, zmm7_4[0].q)).q
                zmm1 = __mulps_xmmps_memps(_mm_shuffle_ps(zmm2_3, zmm2_3, 0x55), var_4c8_1.o)
                int64_t var_578_5 = 0
                zmm0 = __mulps_xmmps_memps(_mm_shuffle_ps(zmm2_3, zmm2_3, 0xaa), var_458_1.o)
                int32_t var_570_4 = 0
                zmm6_4 = __mulps_xmmps_memps(_mm_shuffle_ps(zmm2_3, zmm2_3, 0xff), var_4b8_2.o)
                float var_2fc_1 = zmm8_4[0]
                zmm2_3 = _mm_shuffle_ps(zmm2_3, zmm2_3, 0)
                float var_508_1 = var_2fc_1
                zmm6_4 = _mm_add_ps(_mm_add_ps(zmm6_4, zmm0), 
                    _mm_add_ps(zmm1, _mm_mul_ps(zmm2_3, zmm14)))
                zmm6_4[0] = zmm6_4[0] * zmm6_4[0]
                zmm7_4 = _mm_shuffle_ps(zmm6_4, zmm6_4, 0x55)
                zmm8_4 = _mm_shuffle_ps(zmm6_4, zmm6_4, 0xaa)
                zmm1.d = zmm7_4.d f* zmm7_4[0]
                zmm1.d = zmm1.d f+ zmm6_4[0]
                zmm8_4[0] = zmm8_4[0] * zmm8_4[0]
                zmm1.d = zmm1.d f+ zmm8_4[0]
                
                if (zmm1.d f<= 9.99999994e-09f)
                    zmm6_4 = zmm10
                    zmm7_4 = zmm10
                    zmm8_4 = zmm10
                else
                    zmm3_3 = zx.o(0)
                    zmm4_3 = 0x3f000000
                    zmm3_3[0] = zmm1[0]
                    zmm2_3 = 0x3f000000
                    zmm5_3 = _mm_rsqrt_ss(zmm3_3[0], zmm3_3[0])
                    zmm3_3[0] = zmm3_3[0] * 0.5f
                    zmm5_3[0] = zmm5_3[0] * zmm5_3[0]
                    zmm1.d = zmm3_3.d f* zmm5_3[0]
                    zmm2_3[0] = 0.5f f- zmm1.d
                    zmm5_3[0] = zmm5_3[0] * zmm2_3[0]
                    zmm5_3[0] = zmm5_3[0] + zmm5_3[0]
                    zmm1.d = zmm5_3.d f* zmm5_3[0]
                    zmm3_3[0] = zmm3_3[0] f* zmm1.d
                    zmm4_3[0] = 0.5f - zmm3_3[0]
                    zmm5_3[0] = zmm5_3[0] * zmm4_3[0]
                    zmm5_3[0] = zmm5_3[0] + zmm5_3[0]
                    arg_10 = zmm5_3[0]
                    zmm6_4[0] = zmm6_4[0] * zmm5_3[0]
                    zmm7_4[0] = zmm7_4[0] * zmm5_3[0]
                    zmm8_4[0] = zmm8_4[0] * zmm5_3[0]
                
                float var_3f0 = zmm6_4[0]
                float var_3ec_1 = zmm7_4[0]
                float var_3e8_1 = zmm8_4[0]
                var_5b8_6.d = zmm9[0]
                int32_t var_128[0x4]
                int32_t* rax_116
                rax_116, zmm9, zmm10 = sub_140ae34d0(&var_128, &var_3f0, &var_510, r9_13)
                void* var_1c8
                float* rdx_40 = var_1c8
                var_510 = *rax_116
                int32_t rax_117 = rax_116[2]
                zmm1 = var_510:4.d
                int32_t var_1d4
                var_1c8 = &rdx_40[sx.q(var_1d4)]
                *rdx_40 = var_510.d[0]
                void* var_1b0
                int32_t* rdx_41 = var_1b0
                int32_t var_1bc
                var_1b0 = &rdx_41[sx.q(var_1bc)]
                *rdx_41 = zmm1.d
                void* var_198
                int32_t* rdx_42 = var_198
                int32_t var_1a4
                var_198 = &rdx_42[sx.q(var_1a4)]
                *rdx_42 = rax_117
        
        rbx_1 += r11_5
        cond:4_1 = var_528 + 1 s< *(arg2 + 0x40)
        result = result_1
        r9_6 = var_3a8_1
        r10_2 = var_3b8_1
        r11_5 = var_378_1
        var_528 += 1
        r8_3 += sx.q(var_3c8) << 2
    while (cond:4_1)

return result
