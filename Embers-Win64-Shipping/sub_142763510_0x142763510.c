...
if (i_2 s> 0)
    float zmm8[0x4] = tanf(temp0[0])
    float zmm7[0x4] = tanf(_mm_shuffle_ps(temp0, temp0, 0x55)[0])
    float zmm6[0x4] = tanf(_mm_shuffle_ps(temp0, temp0, 0xaa)[0])
    float temp0_3[0x4] = _mm_unpacklo_ps(zmm7, tanf(temp0[0]))
