// 函数: sub_1423b1c20
// 地址: 0x1423b1c20
// 来自: E:\Embers\Embers\Binaries\Win64\Embers-Win64-Shipping.exe

void var_d8
int64_t rax_1 = __security_cookie ^ &var_d8
void* i_2 = *(arg1 + 0x120)
void* i = arg1 + 0x110

if (i_2 != 0)
    i = i_2

for (void* r14 = i + (sx.q(*(arg1 + 0x128)) << 3); i != r14; i += 8)
    int64_t* rcx = *i
    int32_t rax_3 = *(rcx + 0xc)
    bool cond:1_1
    
    if (*(arg2 + 8) != 2)
        cond:1_1 = rax_3 == 1
    else
        cond:1_1 = rax_3 == 0
    
    rax_3.b = cond:1_1
    
    if (rax_3.b != 0)
        (*(*rcx + 8))(rcx, arg2, arg1 + 0x130)

char r9 = *(arg1 + 0x54)
int128_t* result
int64_t (* var_b8)(int64_t* arg1, void* arg2, int512_t arg3 @ zmm1)
void*** var_a8
void** const var_98
int64_t (* var_78)(int64_t* arg1, void* arg2)
void*** var_68
void** const var_58
int32_t rax_6
void*** rax_10
void** const* rcx_8

if (r9 != 0)
label_1423b1e20:
    *(arg1 + 0x54) = 1
    var_a8 = nullptr
    var_98 = &data_142da2668
    void* var_90_2 = arg1
    var_b8 = sub_1423ab1c0
    var_78 = nullptr
    var_68 = nullptr
    rax_6 = sub_1423affe0(arg1)
    *(arg1 + 0x60) = 0
label_1423b1e60:
    sub_140a3c510(arg1 + 0x70, &var_78)
    *(arg1 + 0xb0) = rax_6
    result = sub_140a3c510(arg1 + 0xc0, &var_b8)
    
    if (var_78 != 0)
        void** const* rcx_12 = &var_58
        
        if (var_68 != 0)
            rcx_12 = var_68
        
        result = (*rcx_12)[2](rcx_12)
    
    if (var_b8 != 0)
        rax_10 = var_a8
        rcx_8 = &var_98
    label_1423b1ea7:
        
        if (rax_10 != 0)
            rcx_8 = rax_10
        
        result = (*rcx_8)[2](rcx_8)
else
    void* i_3 = *(arg1 + 0x120)
    void* i_1 = arg1 + 0x110
    
    if (i_3 != 0)
        i_1 = i_3
    
    for (void* r8_2 = i_1 + (sx.q(*(arg1 + 0x128)) << 3); i_1 != r8_2; i_1 += 8)
        void* rdx_1 = *i_1
        
        if (*(rdx_1 + 8) == 0)
            int32_t rdx_2 = *(rdx_1 + 0xc)
            int32_t rsi_2
            
            if (rdx_2 == 0)
                rsi_2 = 2
            label_1423b1d2b:
                void* var_90 = arg1
                var_a8 = nullptr
                var_98 = &data_142da2668
                var_68 = nullptr
                var_b8 = sub_1423ab1c0
                var_58 = &data_142da2668
                var_78 = sub_1423ab4f0
                void* var_50_1 = arg1
                rax_6 = sub_1423affe0(arg1)
                *(arg1 + 0x60) = rsi_2
                goto label_1423b1e60
            
            if (rdx_2 == 1)
                rsi_2 = 1
                goto label_1423b1d2b
    
    if (r9 != 0)
        goto label_1423b1e20
    
    void* rcx_1 = *(arg1 + 0x108)
    
    if (*(rcx_1 + 8) != 0)
        goto label_1423b1e20
    
    int32_t rcx_2 = *(rcx_1 + 0xc)
    int32_t rsi_1
    
    if (rcx_2 == 0)
        rsi_1 = 2
    else
        if (rcx_2 != 1)
            goto label_1423b1e20
        
        rsi_1 = rcx_2
    
    void* var_50_2 = arg1
    var_58 = &data_142da2668
    void*** var_68_1 = nullptr
    var_78 = sub_1423ab1c0
    void*** var_a8_1 = nullptr
    var_98 = &data_142da2668
    var_b8 = sub_1423ab510
    void* var_90_1 = arg1
    int32_t rax_7 = sub_1423affe0(arg1)
    *(arg1 + 0x60) = rsi_1
    sub_140a3c510(arg1 + 0x70, &var_b8)
    *(arg1 + 0xb0) = rax_7
    result = sub_140a3c510(arg1 + 0xc0, &var_78)
    
    if (var_b8 != 0)
        void** const* rcx_7 = &var_98
        
        if (var_a8_1 != 0)
            rcx_7 = var_a8_1
        
        result = (*rcx_7)[2](rcx_7)
    
    if (var_78 != 0)
        rax_10 = var_68_1
        rcx_8 = &var_58
        goto label_1423b1ea7
__security_check_cookie(rax_1 ^ &var_d8)
return result
