// 函数: sub_141ca0e20
// 地址: 0x141ca0e20
// 来自: E:\Embers\Embers\Binaries\Win64\Embers-Win64-Shipping.exe

int32_t rbx = *(arg1 + 0x28)
int32_t var_84 = 1
void* var_80 = arg1 + 0x10
int32_t var_78 = 0xffffffff
int32_t rcx = 0
int32_t rdi_4 = (((*(arg1 + 0x7c) + 0x1f) u>> 5) + (*(arg1 + 0x5c) + (*(arg1 + 0xc) << 2)) * 6
    + ((*(arg1 + 0x2c) + 0x1f) u>> 5) + *(arg1 + 0x98) + *(arg1 + 0x48)) << 2
int32_t r8_1 = 0
int32_t var_88 = 0
int64_t var_74 = 0

if (rbx != 0)
    void* rax_7 = *(arg1 + 0x20)
    void* r9_1 = arg1 + 0x10
    
    if (rax_7 != 0)
        r9_1 = rax_7
    
    int32_t temp0_1
    int32_t temp1_1
    temp0_1:temp1_1 = sx.q(rbx - 1)
    int32_t rdx_4 = *r9_1
    
    if (rdx_4 != 0)
    label_141ca0ef9:
        int32_t rax_13 = neg.d(rdx_4) & rdx_4
        uint64_t rflags_1
        int32_t temp0_2
        temp0_2, rflags_1 = _bit_scan_reverse(rax_13)
        int32_t var_84_1 = rax_13
        int32_t rax_14
        
        if (rax_13 == 0)
            rax_14 = 0x20
        else
            rax_14 = 0x1f - temp0_2
        
        var_74.d = r8_1 - rax_14 + 0x1f
        
        if (r8_1 - rax_14 + 0x1f s> rbx)
            var_74.d = rbx
    else
        while (true)
            int64_t rax_10 = sx.q(rcx)
            r8_1 += 0x20
            rcx += 1
            var_74:4.d = r8_1
            var_88 = rcx
            
            if (rax_10.d s>= ((temp0_1 & 0x1f) + temp1_1) s>> 5)
                break
            
            rdx_4 = *(r9_1 + (rax_10 << 2) + 4)
            int32_t var_78_1 = 0xffffffff
            
            if (rdx_4 != 0)
                goto label_141ca0ef9
        
        var_74.d = rbx

int32_t rdx_5 = *(arg1 + 0x28)
var_74.d = rdx_5
int32_t r15 = 0xffffffff << (rdx_5.b & 0x1f)
int128_t var_38 = var_88.o
int32_t r8_4 = rdx_5 s>> 5
int32_t rcx_5 = rdx_5 & 0xffffffe0
int32_t var_78_2 = r15
int128_t var_28 = 0xffffffff
int64_t var_48 = _mm_unpackhi_pd(0xffffffff, 0xffffffff)
int128_t var_68 = arg1.o
int128_t var_58 = var_38

if (rdx_5 != rbx)
    void* rax_16 = *(arg1 + 0x20)
    void* r9_2 = arg1 + 0x10
    
    if (rax_16 != 0)
        r9_2 = rax_16
    
    int32_t temp2_1
    int32_t temp3_1
    temp2_1:temp3_1 = sx.q(rbx - 1)
    int32_t rdx_9 = *(r9_2 + (sx.q(r8_4) << 2)) & r15
    
    if (rdx_9 != 0)
    label_141ca0fc2:
        int32_t rax_23 = neg.d(rdx_9) & rdx_9
        uint64_t rflags_2
        int32_t temp0_4
        temp0_4, rflags_2 = _bit_scan_reverse(rax_23)
        int32_t r11_1
        
        if (rax_23 == 0)
            r11_1 = 0x20
        else
            r11_1 = 0x1f - temp0_4
        
        var_74.d = rcx_5 - r11_1 + 0x1f
        
        if (rcx_5 - r11_1 + 0x1f s> rbx)
            var_74.d = rbx
    else
        while (true)
            int64_t rax_20 = sx.q(r8_4)
            rcx_5 += 0x20
            r8_4 += 1
            
            if (rax_20.d s>= ((temp2_1 & 0x1f) + temp3_1) s>> 5)
                break
            
            rdx_9 = *(r9_2 + (rax_20 << 2) + 4)
            var_78_2 = 0xffffffff
            
            if (rdx_9 != 0)
                goto label_141ca0fc2
        
        var_74.d = rbx

while (true)
    int64_t rax_25 = sx.q(var_58:0xc.d)
    int64_t* rdx_11 = var_68.q
    
    if (rax_25.d == (var_78_2.q u>> 0x20).d && var_58.q == arg1 + 0x10 && rdx_11 == arg1)
        return zx.q(rdi_4)
    
    void* rdx_13 = *rdx_11 + rax_25 * 0x60
    int32_t var_5c
    var_58:8.d &= not.d(var_5c)
    rdi_4 += (((*(rdx_13 + 0x34) + 0x1f) u>> 5) + (*(rdx_13 + 0x14) << 2) + *(rdx_13 + 0x50)) << 2
    sub_14059bdd0(&var_68:8)
