// 函数: sub_141969b00
// 地址: 0x141969b00
// 来自: E:\Embers\Embers\Binaries\Win64\Embers-Win64-Shipping.exe

void* rsi = *(arg1 + 0x10)
int32_t r8 = 0
int32_t rbx = *(rsi + 0x28)
int32_t var_88 = 0
int32_t rcx = 0
int32_t var_84 = 1
int32_t r12_1 = *(rsi + 8) - *(rsi + 0x34)
void* var_80 = rsi + 0x10
int32_t var_78 = 0xffffffff
int32_t var_74 = 0
int32_t var_70 = 0

if (rbx != 0)
    void* rax_1 = *(rsi + 0x20)
    void* r9_1 = rsi + 0x10
    
    if (rax_1 != 0)
        r9_1 = rax_1
    
    int32_t temp0_1
    int32_t temp1_1
    temp0_1:temp1_1 = sx.q(rbx - 1)
    int32_t rdx_3 = *r9_1
    int32_t var_74_2
    
    if (rdx_3 != 0)
    label_141969bb9:
        int32_t rax_7 = neg.d(rdx_3) & rdx_3
        uint64_t rflags_1
        int32_t temp0_2
        temp0_2, rflags_1 = _bit_scan_reverse(rax_7)
        int32_t var_84_1 = rax_7
        int32_t rax_8
        
        if (rax_7 == 0)
            rax_8 = 0x20
        else
            rax_8 = 0x1f - temp0_2
        
        int32_t var_74_1 = rcx - rax_8 + 0x1f
        
        if (rcx - rax_8 + 0x1f s> rbx)
            var_74_2 = rbx
    else
        while (true)
            int64_t rax_4 = sx.q(r8)
            rcx += 0x20
            r8 += 1
            int32_t var_70_1 = rcx
            var_88 = r8
            
            if (rax_4.d s>= ((temp0_1 & 0x1f) + temp1_1) s>> 5)
                break
            
            rdx_3 = *(r9_1 + (rax_4 << 2) + 4)
            int32_t var_78_1 = 0xffffffff
            
            if (rdx_3 != 0)
                goto label_141969bb9
        
        var_74_2 = rbx

int32_t rdx_5 = *(rsi + 0x28)
int32_t var_74_3 = rdx_5
int32_t r15 = 0xffffffff << (rdx_5.b & 0x1f)
int128_t var_38 = var_88.o
int32_t r8_2 = rdx_5 s>> 5
int32_t rcx_5 = rdx_5 & 0xffffffe0
int32_t var_78_2 = r15
int128_t var_28 = 0xffffffff
int64_t var_48 = _mm_unpackhi_pd(0xffffffff, 0xffffffff)
int128_t var_68 = rsi.o
int128_t var_58 = var_38

if (rdx_5 != rbx)
    void* rax_10 = *(rsi + 0x20)
    void* r9_2 = rsi + 0x10
    
    if (rax_10 != 0)
        r9_2 = rax_10
    
    int32_t temp2_1
    int32_t temp3_1
    temp2_1:temp3_1 = sx.q(rbx - 1)
    int32_t rdx_9 = *(r9_2 + (sx.q(r8_2) << 2)) & r15
    int32_t var_74_5
    
    if (rdx_9 != 0)
    label_141969c82:
        int32_t rax_17 = neg.d(rdx_9) & rdx_9
        uint64_t rflags_2
        int32_t temp0_4
        temp0_4, rflags_2 = _bit_scan_reverse(rax_17)
        int32_t r11_1
        
        if (rax_17 == 0)
            r11_1 = 0x20
        else
            r11_1 = 0x1f - temp0_4
        
        int32_t var_74_4 = rcx_5 - r11_1 + 0x1f
        
        if (rcx_5 - r11_1 + 0x1f s> rbx)
            var_74_5 = rbx
    else
        while (true)
            int64_t rax_14 = sx.q(r8_2)
            rcx_5 += 0x20
            r8_2 += 1
            
            if (rax_14.d s>= ((temp2_1 & 0x1f) + temp3_1) s>> 5)
                break
            
            rdx_9 = *(r9_2 + (rax_14 << 2) + 4)
            var_78_2 = 0xffffffff
            
            if (rdx_9 != 0)
                goto label_141969c82
        
        var_74_5 = rbx

while (true)
    int64_t rax_19 = sx.q(var_58:0xc.d)
    int64_t* rdx_11 = var_68.q
    
    if (rax_19.d == (var_78_2.q u>> 0x20).d && var_58.q == rsi + 0x10 && rdx_11 == rsi)
        sub_14196c900(*(arg1 + 0x10), 0)
        int64_t r8_3 = *(arg1 + 8)
        void* const rcx_11 = 0x68
        void* const r9_3 = 0x18
        
        if (r8_3 != arg1 + 0x18)
            rcx_11 = 0x18
        
        *(arg1 + 8) = rcx_11 + arg1
        
        if (r8_3 != arg1 + 0x18)
            r9_3 = 0x68
        
        *(arg1 + 0x10) = r9_3 + arg1
        return zx.q(r12_1)
    
    int64_t* rcx_8 = *(rax_19 * 0xb8 + *rdx_11 + 0xa8)
    
    if (rcx_8 != 0)
        (**rcx_8)(rcx_8, 1)
    
    int32_t var_5c
    var_58:8.d &= not.d(var_5c)
    sub_14059bdd0(&var_68:8)
